...
export async function decideCreatorPublishingPackage(_prev: ApprovalActionState, formData: FormData): Promise<ApprovalActionState> {
  ...
  try {
    ...
    const copy = creatorApprovalSuccessMessage({ decision: result.decision, target_platform: result.target_platform, queue_task_status: result.queue_task_status })
    return { ok: true, ...copy, resultingApprovalStatus: result.resulting_creator_approval_status, targetPlatform: platformLabel(result.target_platform), queueTaskCreated: result.queue_task_created, queueTaskStatus: result.queue_task_status }
