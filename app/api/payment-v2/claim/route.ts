...
export async function POST(request: Request) {
  ...
  const result = await paymentFirstClaim({ enabled: process.env.PAYMENT_FIRST_CLAIM_V2_ENABLED, production: process.env.NODE_ENV === "production", configuredOrigin: process.env.PAYMENT_FIRST_CHECKOUT_V2_RETURN_ORIGIN,
    ...
    readOrigin: () => request.headers.get("origin"), async getAuthenticatedUser() { const auth = await supabaseServer(); const { data, error } = await auth.auth.getUser(); return error ? null : data.user?.id || null; }, createDatabase: claimDatabase });
