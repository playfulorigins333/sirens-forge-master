...
export function claimDatabase(): ClaimDatabase {
  ...
  return {
    ...
    loadPurchases: (hold, session, hash) => rows(db.from("payment_v2_purchases").select("id,hold_id,purchaser_credential_hash,tier,state,stripe_checkout_session_id,claimed_profile_id").eq("hold_id", hold).eq("stripe_checkout_session_id", session).eq("purchaser_credential_hash", bytea(hash))),
