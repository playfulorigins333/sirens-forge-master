...
export function createCheckoutHandler(deps:CheckoutDependencies){return async(req:Request)=>{
 ...
 try{
  ...
  let reservation:Reservation; try{reservation=await db.reserve(hash,plan)}catch(e:any){return response(String(e?.code||e?.message).includes("sold_out")?CHECKOUT_ERROR.SOLD_OUT:CHECKOUT_ERROR.TEMPORARILY_UNAVAILABLE,String(e?.code||e?.message).includes("sold_out")?409:503)}
  held={hash,plan,id:reservation.reservation_id,db}; const config=deps.configuration(plan,req); if(!config.priceId||!config.baseUrl)throw new Error("configuration");
  ...
  const expires=Math.floor(new Date(reservation.expires_at).getTime()/1000);if(!Number.isSafeInteger(expires)||expires<=Math.floor(Date.now()/1000))throw new Error("reservation");
...
function productionDependencies():CheckoutDependencies{return{
 ...
 async privileged(){const url=process.env.SUPABASE_URL||process.env.NEXT_PUBLIC_SUPABASE_URL,key=process.env.SUPABASE_SERVICE_ROLE_KEY;if(!url||!key)throw new Error("unavailable");const db=createClient(url,key);return{
  ...
  async reserve(hash,plan){const{data,error}=await db.rpc("acquire_guest_checkout_capacity_reservation",{p_purchaser_token_hash:`\\x${hash}`,p_tier:plan});if(error||!data?.[0]){const e:any=new Error(error?.message||"reservation");e.code=error?.message||"UNAVAILABLE";throw e}return data[0]},
 ...
 configuration(plan,req){const priceId=process.env[LAUNCH_PLAN_POLICY[plan].priceEnvironment]||"",configured=process.env.NEXT_PUBLIC_APP_URL||process.env.NEXT_PUBLIC_SITE_URL;return{priceId,baseUrl:(configured||new URL(req.url).origin).replace(/\/$/,"")}},
