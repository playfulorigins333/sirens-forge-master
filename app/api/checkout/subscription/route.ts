...
export function createCheckoutHandler(deps:CheckoutDependencies){return async(req:Request)=>{
 ...
 try{
  ...
  const session=await boundary("provider","session_creation",CHECKOUT_ERROR.CHECKOUT_PROVIDER_FAILURE,502,()=>deps.createSession(input,checkoutSessionIdempotencyKey(reservation.reservation_id)));providerExists=true;if(!session.id||!session.url||!/^https:\/\/checkout\.stripe\.com\//.test(session.url))throw new CheckoutFailure("provider","session_validation",CHECKOUT_ERROR.CHECKOUT_PROVIDER_FAILURE,502);
 ...
 }catch(error){const failure=error instanceof CheckoutFailure?error:new CheckoutFailure("database","database_initialization",CHECKOUT_ERROR.CHECKOUT_DATABASE_FAILURE,503);if(held&&!providerExists)try{await held.db.release(held.hash,held.plan,held.id)}catch{logFailure("cleanup","reservation_release")};logFailure(failure.category,failure.stage);return response(failure.code,failure.status)}
}}
function productionDependencies():CheckoutDependencies{return{
 ...
 preflight(plan,req){return checkoutCreationConfiguration({request:req,rateLimitSecret:process.env.CHECKOUT_RATE_LIMIT_SECRET,supabaseUrl:checkoutSupabaseUrl(process.env.SUPABASE_URL,process.env.NEXT_PUBLIC_SUPABASE_URL),serviceRoleKey:process.env.SUPABASE_SERVICE_ROLE_KEY,stripeSecret:process.env.STRIPE_SECRET_KEY,priceId:process.env[LAUNCH_PLAN_POLICY[plan].priceEnvironment],canonicalUrl:process.env.NEXT_PUBLIC_APP_URL})},
 generateToken:generatePurchaserToken,
 async privileged(){const url=checkoutSupabaseUrl(process.env.SUPABASE_URL,process.env.NEXT_PUBLIC_SUPABASE_URL),key=process.env.SUPABASE_SERVICE_ROLE_KEY;if(!url||!key)throw new Error("unavailable");const db=createClient(url,key);return{
