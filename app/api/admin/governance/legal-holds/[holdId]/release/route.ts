...
export async function POST(request: Request, context: { params: Promise<{ holdId: string }> }) {
  ...
  const input = validateReleaseLegalHoldInput(body)
  if (!input) return json({ ok: false, code: "LEGAL_HOLD_REQUEST_INVALID" }, 400)

  const result = await releaseLegalHold({ holdId, actorUserId: mfa.userId, freshTotpAt: mfa.freshTotpAt, idempotencyKey, input })
  if (!result.ok) {
    const denied = result.code === "GOVERNANCE_LEGAL_HOLD_ADMIN_REQUIRED" || result.code === "GOVERNANCE_LEGAL_HOLD_FRESH_AUTH_REQUIRED"
    const conflict = result.code === "GOVERNANCE_LEGAL_HOLD_IDEMPOTENCY_CONFLICT" || result.code === "GOVERNANCE_LEGAL_HOLD_NOT_ACTIVE" || result.code === "GOVERNANCE_LEGAL_HOLD_EXPIRED"
