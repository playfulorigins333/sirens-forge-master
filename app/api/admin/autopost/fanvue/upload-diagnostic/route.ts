...
const FANVUE_UPLOAD_DIAGNOSTIC_ACCOUNT_SELECT = [
  "user_id",
  "platform",
  "connection_status",
  "provider_account_id",
...
async function loadFanvueUploadDiagnosticAccount(userId: string): Promise<FanvueUploadDiagnosticAccount | null> {
  const supabaseAdmin = getSupabaseAdmin()
  const { data, error } = await supabaseAdmin
    .from("autopost_accounts")
    .select(FANVUE_UPLOAD_DIAGNOSTIC_ACCOUNT_SELECT)
    .eq("user_id", userId)
    .eq("platform", "fanvue")
    .maybeSingle()
...
export async function POST(req: Request) {
  ...
  const response = await handleFanvueUploadDiagnosticRoute({
    ...
    createLoadAccount: () => loadFanvueUploadDiagnosticAccount,
