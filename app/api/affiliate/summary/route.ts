...
export async function affiliateSummaryResponse(deps: SummaryDependencies) {
  ...
  try {
    ...
    const ledger = await rows(admin.from("affiliate_ledger").select("id,referred_user_id,commission_amount_cents,gross_amount_cents,commission_percent,status,created_at,payment_v2_purchase_id,attribution_status").eq("affiliate_user_id", profile.id))
    ...
    const payableLedger = ledger.filter((row: any) => row.attribution_status !== "VOID_SELF_REFERRAL" && row.status !== "void")
    ...
    const v2Referrals = ledger.filter((row: any) => row.payment_v2_purchase_id && row.attribution_status !== "VOID_SELF_REFERRAL").length
    return NextResponse.json({
      ...
      total_referrals: referrals.length + v2Referrals, referrals, commissions: [...commissions, ...ledger], total_earnings: paid, pending, paid,
