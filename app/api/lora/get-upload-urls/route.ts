...
import { reactivateTwinTrainingData, TwinLifecycleError } from "@/lib/twin-lifecycle";
...
export async function POST(req:Request){
 try{
  ...
  const admin=getSupabaseAdmin();const {data:lora,error:loraError}=await admin.from("user_loras").select("id,user_id,lifecycle_state,training_data_state").eq("id",loraId).eq("user_id",auth.user.id).maybeSingle();if(loraError)return NextResponse.json({error:"LIFECYCLE_UNAVAILABLE"},{status:503});if(!lora)return NextResponse.json({error:"NOT_FOUND"},{status:404});
  if(lora.lifecycle_state!=="active")return NextResponse.json({error:"TWIN_NOT_ACTIVE"},{status:409});
  if(lora.training_data_state==="purge_pending")return NextResponse.json({error:"TRAINING_DATA_PURGE_PENDING"},{status:409});
  if(lora.training_data_state==="purged")await reactivateTwinTrainingData(loraId,auth.user.id);
 ...
 }catch(error){if(error instanceof TwinLifecycleError)return NextResponse.json({error:error.code},{status:error.status});console.error("[get-upload-urls] failed",error);return NextResponse.json({error:"FAILED_TO_CREATE_UPLOAD"},{status:500});}
}
