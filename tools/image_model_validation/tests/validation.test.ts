...
async function tensorFixture(dtype: "F16" | "F32", data: Buffer, count: number, filename = "fixture.safetensors"): Promise<string> {
...
test("malformed SafeTensor and unvalidated float formats fail closed", async () => { const bad = path.join(await mkdtemp(path.join(os.tmpdir(), "sf-bad-")), "bad.safetensors"); await writeFile(bad, "not safetensors"); await assert.rejects(() => scanSafeTensor(bad), /MALFORMED_SAFETENSOR/); const f64 = await tensorFixture("F32", Buffer.alloc(8), 2); const bytes = await readFile(f64); bytes.write("F64", 8 + bytes.indexOf("F32")); await writeFile(f64, bytes); await assert.rejects(() => scanSafeTensor(f64), /invalid tensor record/); });
