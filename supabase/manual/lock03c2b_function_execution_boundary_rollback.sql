...
-- Requires explicit human approval and reopens direct browser/API execution exposure.
-- Never run automatically. Source-only preparation does not authorize Production use.
begin;
...
begin
  ...
  if drifted then
    raise exception using errcode = 'P0001', message = 'LOCK03C2B_ROLLBACK_DRIFT';
  end if;
end
$$;

grant execute on function public.add_tokens(uuid, integer, text) to public;
grant execute on function public.add_tokens(uuid, integer, text) to anon;
grant execute on function public.add_tokens(uuid, integer, text) to authenticated;
grant execute on function public.deduct_tokens(uuid, integer) to public;
grant execute on function public.deduct_tokens(uuid, integer) to anon;
grant execute on function public.deduct_tokens(uuid, integer) to authenticated;
grant execute on function public.deduct_tokens(uuid, integer, text) to public;
grant execute on function public.deduct_tokens(uuid, integer, text) to anon;
grant execute on function public.deduct_tokens(uuid, integer, text) to authenticated;
grant execute on function public.record_lora_terminal_status() to public;
grant execute on function public.record_lora_terminal_status() to anon;
grant execute on function public.record_lora_terminal_status() to authenticated;
grant execute on function public.creator_publishing_platform_account_clear_trusted_metadata() to public;
grant execute on function public.creator_publishing_platform_account_clear_trusted_metadata() to anon;
grant execute on function public.creator_publishing_platform_account_clear_trusted_metadata() to authenticated;
...
begin
  select exists (
    ...
    where p.oid is null or owner_role.rolname <> 'postgres' or not p.prosecdef
       ...
       or not exists (select 1 from aclexplode(coalesce(p.proacl, acldefault('f', p.proowner))) acl
                  where acl.grantee = 0 and acl.privilege_type = 'EXECUTE')
  ) into failed;
  ...
  failed := failed or (select count(*) <> 2 from pg_trigger t
    ...
    where n.nspname='public' and t.tgenabled <> 'D' and not t.tgisinternal and
      ...
      and pg_get_function_identity_arguments(p.oid)='');
  if failed then
    raise exception using errcode = 'P0001', message = 'LOCK03C2B_ROLLBACK_POSTCONDITION_FAILED';
  end if;
end
$$;
