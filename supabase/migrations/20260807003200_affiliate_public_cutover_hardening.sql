...
create or replace function public.create_affiliate_payout_batch(p_notes text default null) returns uuid language plpgsql security invoker set search_path=pg_catalog,pg_temp as $$ declare batch uuid; begin
 ...
 select batch,e.id,e.affiliate_user_id,e.commission_amount_cents,e.cur,e.charge,e.destination,'pfc03d:'||e.id,'pending' from eligible e join qualified q using(affiliate_user_id) where e.charge is not null and e.destination~'^acct_[A-Za-z0-9]+$' on conflict(ledger_id) do nothing;
 with legacy_qualified as(select affiliate_user_id from public.affiliate_ledger where status='available' and payment_v2_purchase_id is null and payment_v2_recurring_invoice_id is null group by affiliate_user_id having sum(commission_amount_cents)>=5000), legacy_items as(insert into public.affiliate_payout_items(batch_id,ledger_id,affiliate_user_id,amount_cents,execution_status) select batch,l.id,l.affiliate_user_id,l.commission_amount_cents,'legacy' from public.affiliate_ledger l join legacy_qualified q using(affiliate_user_id) where l.status='available' and l.payment_v2_purchase_id is null and l.payment_v2_recurring_invoice_id is null on conflict(ledger_id) do nothing returning ledger_id) update public.affiliate_ledger set status='paid',updated_at=now() where id in(select ledger_id from legacy_items);
 return batch; end $$;
