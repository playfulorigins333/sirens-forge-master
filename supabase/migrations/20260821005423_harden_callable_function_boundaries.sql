-- Source-only callable-function execution and search_path hardening.
...
begin
  ...
  if coalesce(v_bad,true) then raise exception using errcode='P0001',message='CALLABLE_FUNCTION_HARDENING_DRIFT'; end if;
...
alter function public.auto_approve_caption_templates() set search_path = pg_catalog, public, pg_temp;
revoke execute on function public.auto_approve_caption_templates() from public, anon, authenticated;
alter function public.auto_approve_cta_variants() set search_path = pg_catalog, public, pg_temp;
revoke execute on function public.auto_approve_cta_variants() from public, anon, authenticated;
alter function public.auto_approve_hashtag_sets() set search_path = pg_catalog, public, pg_temp;
revoke execute on function public.auto_approve_hashtag_sets() from public, anon, authenticated;
alter function public.claim_next_lora_job() set search_path = pg_catalog, pg_temp;
revoke execute on function public.claim_next_lora_job() from public, anon, authenticated;
alter function public.creator_publishing_audit_events_prevent_mutation() set search_path = pg_catalog, pg_temp;
revoke execute on function public.creator_publishing_audit_events_prevent_mutation() from public, anon, authenticated;
alter function public.creator_publishing_escalated_approved_has_review() set search_path = pg_catalog, pg_temp;
revoke execute on function public.creator_publishing_escalated_approved_has_review() from public, anon, authenticated;
alter function public.creator_publishing_prevent_creator_controlled_field_update() set search_path = pg_catalog, pg_temp;
revoke execute on function public.creator_publishing_prevent_creator_controlled_field_update() from public, anon, authenticated;
alter function public.creator_publishing_queue_jsonb_has_forbidden_credential_key(jsonb) set search_path = pg_catalog, pg_temp;
revoke execute on function public.creator_publishing_queue_jsonb_has_forbidden_credential_key(jsonb) from public, anon, authenticated;
alter function public.dataset_doctor_images_enforce_parent_match() set search_path = pg_catalog, pg_temp;
revoke execute on function public.dataset_doctor_images_enforce_parent_match() from public, anon, authenticated;
alter function public.dataset_doctor_images_refresh_counts() set search_path = pg_catalog, pg_temp;
revoke execute on function public.dataset_doctor_images_refresh_counts() from public, anon, authenticated;
alter function public.dataset_doctor_mark_approved(uuid, text, text) set search_path = pg_catalog, pg_temp;
revoke execute on function public.dataset_doctor_mark_approved(uuid, text, text) from public, anon, authenticated;
alter function public.dataset_doctor_mark_exported(uuid) set search_path = pg_catalog, pg_temp;
revoke execute on function public.dataset_doctor_mark_exported(uuid) from public, anon, authenticated;
alter function public.dataset_doctor_queue_lora_training(uuid, text, text, integer) set search_path = pg_catalog, pg_temp;
revoke execute on function public.dataset_doctor_queue_lora_training(uuid, text, text, integer) from public, anon, authenticated;
alter function public.dataset_doctor_selections_enforce_parent_match() set search_path = pg_catalog, pg_temp;
revoke execute on function public.dataset_doctor_selections_enforce_parent_match() from public, anon, authenticated;
alter function public.dataset_doctor_set_active_job(uuid, uuid) set search_path = pg_catalog, pg_temp;
revoke execute on function public.dataset_doctor_set_active_job(uuid, uuid) from public, anon, authenticated;
alter function public.increment_generation_count() set search_path = pg_catalog, public, pg_temp;
revoke execute on function public.increment_generation_count() from public, anon, authenticated;
alter function public.refresh_dataset_doctor_job_counts(uuid) set search_path = pg_catalog, pg_temp;
revoke execute on function public.refresh_dataset_doctor_job_counts(uuid) from public, anon, authenticated;
alter function public.set_updated_at() set search_path = pg_catalog, pg_temp;
revoke execute on function public.set_updated_at() from public, anon, authenticated;
alter function public.update_collection_item_count() set search_path = pg_catalog, public, pg_temp;
revoke execute on function public.update_collection_item_count() from public, anon, authenticated;
alter function public.update_collections_updated_at() set search_path = pg_catalog, pg_temp;
revoke execute on function public.update_collections_updated_at() from public, anon, authenticated;
...
begin
  ...
  into v_bad from (values
    ...
    left join pg_catalog.pg_proc p on p.pronamespace=n.oid and p.proname=target.name and pg_catalog.oidvectortypes(p.proargtypes)=target.args
    left join pg_catalog.pg_roles owner_role on owner_role.oid=p.proowner;
  if coalesce(v_bad,true) then raise exception using errcode='P0001',message='CALLABLE_FUNCTION_HARDENING_POSTCONDITION_FAILED'; end if;
