...
begin
  ...
  select exists(
    ...
    where m.content_package_id=pkg.id and (
      ...
      g.id is null or (g.user_id <> j.creator_id and g.user_id is distinct from v_profile_id) or g.status is distinct from 'completed' or
...
begin
 ...
 select * into v_existing from public.creator_publishing_schedule_idempotency where creator_id=p_creator_id and idempotency_key=v_key and publishing_plan_id=p_publishing_plan_id for update;
 ...
 create temp table scheduler_locked_creator_verifications on commit drop as select v.* from public.creator_publishing_creator_verifications v where v.creator_id=p_creator_id order by v.id for update;
 create temp table scheduler_locked_ai_consents on commit drop as select c.* from public.creator_publishing_ai_twin_consents c where c.creator_id=p_creator_id order by c.id for update;
...
begin
 ...
 if p_platform_job_id is not null and v_count=0 then perform public.creator_publishing_recalculate_plan_status(p_publishing_plan_id); end if;
...
begin
 ...
 perform pg_catalog.pg_advisory_xact_lock(pg_catalog.hashtextextended('creator_publishing_scheduler_facts:'||j.content_package_id::text,0));
 ...
 select * into c from public.creator_publishing_platform_capabilities where platform=j.target_platform for update;
