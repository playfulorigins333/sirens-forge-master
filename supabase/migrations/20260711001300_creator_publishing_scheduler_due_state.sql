...
begin
  ...
  select jsonb_build_object('creator_id',c.creator_id,'status',c.status,'revoked_at',c.revoked_at,'attestation_version',c.attestation_version,'attestation_text_sha256',c.attestation_text_sha256,'expected_attestation_version',p_expected_ai_twin_consent_version,'expected_attestation_text_sha256',p_expected_ai_twin_consent_text_sha256) into v_consent from public.creator_publishing_ai_twin_consents c where c.creator_id=j.creator_id;
...
begin
  ...
  if pkg.ai_flag in ('ai_enhanced','ai_generated') then
    select * into consent from public.creator_publishing_ai_twin_consents c where c.creator_id=j.creator_id;
  ...
  v_snapshot := public.creator_publishing_scheduler_fact_snapshot(j.id,p_expected_ai_twin_consent_version,p_expected_ai_twin_consent_text_sha256);
...
returns jsonb language plpgsql security definer set search_path=public,pg_temp as $$
declare
  v_now timestamptz:=now(); v_key text:=btrim(coalesce(p_idempotency_key,'')); v_plan public.creator_publishing_plans%rowtype; v_request_fingerprint text; v_snapshot_fingerprint text; v_existing record; v_results jsonb:='[]'::jsonb; v_audits jsonb:='[]'::jsonb; j record; v_rev int; v_state text; v_operator_due timestamptz; v_event_ids jsonb; v_superseded_event_ids jsonb:='[]'::jsonb; v_audit bigint; v_request_canonical jsonb; v_snapshot_canonical jsonb; v_gate jsonb; v_expected int; v_target_ids uuid[];
begin
 ...
 for j in select * from pg_temp.scheduler_locked_jobs order by id loop insert into pg_temp.scheduler_gate_snapshot values(j.id, public.creator_publishing_scheduler_gate(j.id,p_expected_ai_twin_consent_version,p_expected_ai_twin_consent_text_sha256)); end loop;
 ...
 for j in select j.*,g.gate from pg_temp.scheduler_locked_jobs j join pg_temp.scheduler_gate_snapshot g on g.job_id=j.id order by j.id loop
   v_gate:=j.gate; v_event_ids:='[]'::jsonb; v_superseded_event_ids:='[]'::jsonb; v_operator_due:=case when j.publishing_mode='assisted' then p_intended_publish_at - interval '60 minutes' else null end;
   ...
   with superseded as (update public.creator_publishing_scheduler_events set event_status='superseded',superseded_at=v_now,lock_token=null,locked_at=null where platform_job_id=j.id and event_status in ('pending','processing') returning id) select coalesce(jsonb_agg(id order by id),'[]'::jsonb) into v_superseded_event_ids from superseded;
...
begin
 ...
 for j in select * from pg_temp.cancel_locked_jobs order by id loop
   select coalesce(jsonb_agg(e.id order by e.id),'[]'::jsonb) into v_event_ids from pg_temp.cancel_locked_events e where e.platform_job_id=j.id;
   if jsonb_array_length(v_event_ids)>0 then
     update public.creator_publishing_scheduler_events set event_status='cancelled',cancelled_at=v_now,lock_token=null,locked_at=null,last_error_code='CANCELLED_BY_CREATOR' where platform_job_id=j.id and event_status in ('pending','processing');
     get diagnostics v_event_rows = row_count; v_events_changed:=v_events_changed+v_event_rows;
     insert into public.creator_publishing_audit_events(entity_type,entity_id,actor_id,actor_role,action,before_state,after_state,created_at) values('creator_publishing_platform_job',j.id,p_creator_id,'creator','creator_publishing_scheduler_events_cancelled',jsonb_build_object('scheduler_event_ids',v_event_ids),jsonb_build_object('scheduler_event_ids',v_event_ids,'reason','redacted'),v_now) returning id into v_event_audit;
     v_audit_created:=true;
   else
     v_event_audit:=null;
   end if;
   if j.cancelled_at is not null then v_results:=v_results||jsonb_build_array(jsonb_build_object('jobId',j.id,'outcome','already_cancelled','jobState',j.job_state,'scheduleRevision',coalesce(j.schedule_revision,0),'closedSchedulerEventIds',v_event_ids,'eventAuditEventId',case when v_event_audit is null then null else v_event_audit::text end)); continue; end if;
   if j.job_state = any(terminal_states) then v_results:=v_results||jsonb_build_array(jsonb_build_object('jobId',j.id,'outcome','already_terminal','jobState',j.job_state,'scheduleRevision',coalesce(j.schedule_revision,0),'closedSchedulerEventIds',v_event_ids,'eventAuditEventId',case when v_event_audit is null then null else v_event_audit::text end)); continue; end if;
   ...
   insert into public.creator_publishing_audit_events(entity_type,entity_id,actor_id,actor_role,action,before_state,after_state,created_at) values('creator_publishing_platform_job',j.id,p_creator_id,'creator','creator_publishing_job_cancelled',jsonb_build_object('job_state',j.job_state),jsonb_build_object('job_state','archived','scheduler_event_ids',v_event_ids),v_now) returning id into v_audit;
   v_audit_created:=true; v_count:=v_count+1; v_results:=v_results||jsonb_build_array(jsonb_build_object('jobId',j.id,'outcome','archived','jobState','archived','scheduleRevision',coalesce(j.schedule_revision,0),'auditEventId',v_audit::text,'closedSchedulerEventIds',v_event_ids,'eventAuditEventId',case when v_event_audit is null then null else v_event_audit::text end));
 end loop;
...
begin
 return query with due as (
   select e.id,e.platform_job_id,e.due_at,e.event_type,e.event_status as prior_event_status,e.processing_attempts as prior_processing_attempts,e.schedule_revision,case when e.event_type='operator_due' then 0 else 1 end as event_order
   ...
   returning e.id,e.lock_token,e.event_status,e.processing_attempts,e.schedule_revision,due.due_at,due.event_type,due.prior_event_status,due.prior_processing_attempts,due.event_order
 ), claim_audits as (
   ...
   select 'creator_publishing_scheduler_event',c.id,'system','creator_publishing_scheduler_event_claimed',jsonb_build_object('event_status',c.prior_event_status,'processing_attempts',c.prior_processing_attempts,'event_type',c.event_type,'due_at',c.due_at,'schedule_revision',c.schedule_revision),jsonb_build_object('event_status',c.event_status,'processing_attempts',c.processing_attempts,'event_type',c.event_type,'due_at',c.due_at,'schedule_revision',c.schedule_revision),now()
   from claimed c
   returning id
...
begin
 ...
 if p.id is null or p.status='cancelled' then update public.creator_publishing_scheduler_events set event_status='cancelled',cancelled_at=v_now,lock_token=null,locked_at=null,last_error_code='PLAN_CANCELLED' where id=e.id and event_status='processing' and lock_token is not distinct from p_lock_token; get diagnostics v_event_rows = row_count; if v_event_rows<>1 then raise exception 'PROTECTED_UPDATE_MISSED'; end if; insert into public.creator_publishing_audit_events(entity_type,entity_id,actor_role,action,before_state,after_state,created_at) values('creator_publishing_scheduler_event',e.id,'system','creator_publishing_scheduler_event_cancelled',jsonb_build_object('event_status','processing'),jsonb_build_object('event_status','cancelled','code','PLAN_CANCELLED','schedule_revision',e.schedule_revision),v_now); return jsonb_build_object('ok',true,'skipped',true,'code','PLAN_CANCELLED'); end if;
 if j.id is null then update public.creator_publishing_scheduler_events set event_status='superseded',superseded_at=v_now,lock_token=null,locked_at=null,last_error_code='JOB_NOT_FOUND' where id=e.id and event_status='processing' and lock_token is not distinct from p_lock_token; get diagnostics v_event_rows = row_count; if v_event_rows<>1 then raise exception 'PROTECTED_UPDATE_MISSED'; end if; insert into public.creator_publishing_audit_events(entity_type,entity_id,actor_role,action,before_state,after_state,created_at) values('creator_publishing_scheduler_event',e.id,'system','creator_publishing_scheduler_event_superseded',jsonb_build_object('event_status','processing'),jsonb_build_object('event_status','superseded','code','JOB_NOT_FOUND','schedule_revision',e.schedule_revision),v_now); return jsonb_build_object('ok',true,'skipped',true,'code','JOB_NOT_FOUND'); end if;
 if j.schedule_revision<>e.schedule_revision then update public.creator_publishing_scheduler_events set event_status='superseded',superseded_at=v_now,lock_token=null,locked_at=null,last_error_code='REVISION_SUPERSEDED' where id=e.id and event_status='processing' and lock_token is not distinct from p_lock_token; get diagnostics v_event_rows = row_count; if v_event_rows<>1 then raise exception 'PROTECTED_UPDATE_MISSED'; end if; insert into public.creator_publishing_audit_events(entity_type,entity_id,actor_role,action,before_state,after_state,created_at) values('creator_publishing_scheduler_event',e.id,'system','creator_publishing_scheduler_event_superseded',jsonb_build_object('event_status','processing'),jsonb_build_object('event_status','superseded','code','REVISION_SUPERSEDED','schedule_revision',e.schedule_revision),v_now); return jsonb_build_object('ok',true,'skipped',true,'code','REVISION_SUPERSEDED'); end if;
 if j.cancelled_at is not null or e.cancelled_at is not null or e.event_status in ('cancelled','superseded') then update public.creator_publishing_scheduler_events set event_status='cancelled',cancelled_at=coalesce(cancelled_at,v_now),lock_token=null,locked_at=null,last_error_code='CANCELLED' where id=e.id and event_status='processing' and lock_token is not distinct from p_lock_token; get diagnostics v_event_rows = row_count; if v_event_rows<>1 then raise exception 'PROTECTED_UPDATE_MISSED'; end if; insert into public.creator_publishing_audit_events(entity_type,entity_id,actor_role,action,before_state,after_state,created_at) values('creator_publishing_scheduler_event',e.id,'system','creator_publishing_scheduler_event_cancelled',jsonb_build_object('event_status','processing'),jsonb_build_object('event_status','cancelled','code','CANCELLED','schedule_revision',e.schedule_revision),v_now); return jsonb_build_object('ok',true,'skipped',true,'code','CANCELLED'); end if;
 if j.job_state = any(terminal_states) then update public.creator_publishing_scheduler_events set event_status='superseded',superseded_at=v_now,lock_token=null,locked_at=null,last_error_code='TERMINAL_JOB' where id=e.id and event_status='processing' and lock_token is not distinct from p_lock_token; get diagnostics v_event_rows = row_count; if v_event_rows<>1 then raise exception 'PROTECTED_UPDATE_MISSED'; end if; insert into public.creator_publishing_audit_events(entity_type,entity_id,actor_role,action,before_state,after_state,created_at) values('creator_publishing_scheduler_event',e.id,'system','creator_publishing_scheduler_event_superseded',jsonb_build_object('event_status','processing'),jsonb_build_object('event_status','superseded','code','TERMINAL_JOB','schedule_revision',e.schedule_revision),v_now); return jsonb_build_object('ok',true,'skipped',true,'code','TERMINAL_JOB'); end if;
 if e.event_type='operator_due' and j.publishing_mode='assisted' and j.job_state='due_now' then update public.creator_publishing_scheduler_events set event_status='superseded',superseded_at=v_now,lock_token=null,locked_at=null,last_error_code='OBSOLETE_OPERATOR_DUE' where id=e.id and event_status='processing' and lock_token is not distinct from p_lock_token; get diagnostics v_event_rows = row_count; if v_event_rows<>1 then raise exception 'PROTECTED_UPDATE_MISSED'; end if; insert into public.creator_publishing_audit_events(entity_type,entity_id,actor_role,action,before_state,after_state,created_at) values('creator_publishing_scheduler_event',e.id,'system','creator_publishing_scheduler_event_superseded',jsonb_build_object('event_status','processing'),jsonb_build_object('event_status','superseded','code','OBSOLETE_OPERATOR_DUE','schedule_revision',e.schedule_revision),v_now); return jsonb_build_object('ok',true,'skipped',true,'code','OBSOLETE_OPERATOR_DUE'); end if;
 ...
 if (v_gate->>'ok')::boolean is not true then update public.creator_publishing_platform_jobs set job_state=case when coalesce((v_gate->>'hard')::boolean,false) then 'blocked' else 'needs_fix' end, updated_at=v_now where id=j.id and not (job_state = any(terminal_states)); get diagnostics v_job_rows = row_count; update public.creator_publishing_scheduler_events set event_status='blocked',processed_at=v_now,lock_token=null,locked_at=null,last_error_code=v_gate->>'code' where id=e.id and event_status='processing' and lock_token is not distinct from p_lock_token; get diagnostics v_event_rows = row_count; if v_job_rows<>1 or v_event_rows<>1 then raise exception 'PROTECTED_UPDATE_MISSED'; end if; with superseded as (update public.creator_publishing_scheduler_events set event_status='superseded',superseded_at=v_now,lock_token=null,locked_at=null where platform_job_id=j.id and schedule_revision=e.schedule_revision and event_status in ('pending','processing') and id<>e.id returning id) select coalesce(array_agg(id),array[]::uuid[]) into v_superseded_event_ids from superseded; insert into public.creator_publishing_audit_events(entity_type,entity_id,actor_role,action,before_state,after_state,created_at) values('creator_publishing_platform_job',j.id,'system','creator_publishing_due_state_transition_blocked',jsonb_build_object('job_state',j.job_state),jsonb_build_object('event_id',e.id,'code',v_gate->>'code','superseded_event_ids',v_superseded_event_ids),v_now) returning id into v_audit; perform public.creator_publishing_recalculate_plan_status(j.publishing_plan_id); return jsonb_build_object('ok',false,'blocked',true,'code',v_gate->>'code','auditEventId',v_audit::text); end if;
 ...
 if e.event_type='publish_due' and j.publishing_mode='assisted' and v_state='due_now' then with superseded as (update public.creator_publishing_scheduler_events set event_status='superseded',superseded_at=v_now,lock_token=null,locked_at=null,last_error_code='OBSOLETE_OPERATOR_DUE' where platform_job_id=j.id and schedule_revision=e.schedule_revision and event_type='operator_due' and event_status in ('pending','processing') and id<>e.id returning id) select coalesce(array_agg(id),array[]::uuid[]) into v_superseded_event_ids from superseded; end if;
 insert into public.creator_publishing_audit_events(entity_type,entity_id,actor_role,action,before_state,after_state,created_at) values('creator_publishing_platform_job',j.id,'system','creator_publishing_due_state_transition_completed',jsonb_build_object('job_state',j.job_state),jsonb_build_object('job_state',v_state,'event_id',e.id,'superseded_event_ids',v_superseded_event_ids),v_now) returning id into v_audit;
