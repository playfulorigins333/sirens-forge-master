...
BEGIN
 ...
 IF p_status='succeeded' AND total=s.original_gross_amount THEN
  ...
  IF s.source_type='OG_INITIAL' THEN
   ...
   SELECT * INTO ledger FROM public.affiliate_ledger WHERE payment_v2_purchase_id=s.purchase_id FOR UPDATE;
   IF FOUND THEN SELECT execution_status INTO item_state FROM public.affiliate_payout_items WHERE ledger_id=ledger.id;
    IF ledger.status IN('pending','available')AND item_state IS DISTINCT FROM'dispatching'THEN UPDATE public.affiliate_ledger SET status='void',void_reason='PAYMENT_REFUNDED',voided_at=statement_timestamp(),updated_at=statement_timestamp()WHERE id=ledger.id;
    ELSIF item_state='dispatching'THEN PERFORM public.payment_v2_open_review(s.purchase_id,NULL,p_source_charge_id,'refund',p_provider_refund_id,'AFFILIATE_ALREADY_DISPATCHING',p_amount_cents,p_currency);
    ELSIF ledger.status='paid'THEN PERFORM public.payment_v2_open_review(s.purchase_id,NULL,p_source_charge_id,'refund',p_provider_refund_id,'AFFILIATE_ALREADY_PAID',p_amount_cents,p_currency); END IF;
   END IF;
  END IF;
 END IF;
 RETURN 'applied';
...
-- Batch selection and dispatch-time race closure: the existing payload is returned only after a fresh locked financial check.
create or replace function public.payment_v2_begin_payout_dispatch(p_item_id uuid)returns jsonb language plpgsql security definer set search_path=pg_catalog,pg_temp as $$declare x public.affiliate_payout_items%rowtype;l public.affiliate_ledger%rowtype;r public.payment_v2_affiliate_recurring_invoices%rowtype;p public.payment_v2_purchases%rowtype;h public.payment_v2_holds%rowtype;pr public.profiles%rowtype;destination text;purchase_id uuid;pre_status text;pre_charge text;begin
 select execution_status,source_charge_id into pre_status,pre_charge from public.affiliate_payout_items where id=p_item_id;if not found then return null;end if;
 if pre_status='pending' then perform pg_advisory_xact_lock(pg_catalog.hashtextextended('phase12:'||pre_charge,1200));end if;
 ...
 if public.payment_v2_source_charge_blocks_affiliate_payout(x.source_charge_id) then return null;end if;
