-- Phase 7 closeout hardening.
-- Existing applied Phase 7 migrations remain immutable.
-- Phase 8 still owns purge scheduling/execution; Phase 9 still owns notification delivery.

begin;

-- Voluntary deletion must not allow a recoverable recurring subscription to
-- keep renewing merely because its provider status is currently delinquent.
...
-- Durable lifecycle-only execution gate for background publishing. This mirrors
-- the Phase 7 paid/lifecycle semantics without relying on a browser session.
-- MFA remains a user-session boundary and is intentionally not evaluated here.
...
-- X provider dispatch has a final database-side Phase 7 lifecycle gate. A rule
-- approved before a later payment failure/deletion freeze cannot dispatch.
...
-- Fanvue jobs are not claimed while Phase 7 lifecycle access is frozen, so a
-- temporary billing/deletion block does not consume an attempt.
...
-- Re-check again at the irreversible provider-create marker to close the short
-- claim-to-dispatch race if lifecycle state changes after claim.
