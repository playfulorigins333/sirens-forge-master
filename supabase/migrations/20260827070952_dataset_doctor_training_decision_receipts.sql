...
create function public.dataset_training_quality_overridable(p_summary jsonb,p_selected_count integer,p_needs_more_images boolean,p_missing_coverage jsonb) returns boolean language plpgsql immutable set search_path='' as $$declare v text; item jsonb;begin
 ...
 if jsonb_array_length(coalesce(p_summary->'dataset_warnings','[]'::jsonb))=0 and jsonb_array_length(coalesce(p_summary->'secondary_issues','[]'::jsonb))=0 and coalesce(p_summary->>'primary_issue','')='' and jsonb_array_length(coalesce(p_summary->'dataset_warnings_structured','[]'::jsonb))=0 and coalesce(p_needs_more_images,false)=false and jsonb_array_length(coalesce(p_missing_coverage,'[]'::jsonb))=0 then return false;end if;
...
returns table(prompt_id uuid,decision_contract_version text,shown_at timestamptz,warning_snapshot jsonb) language plpgsql security definer set search_path='' as $$declare j public.dataset_doctor_jobs; ids jsonb; warnings jsonb; dataset_fp text; warning_fp text; existing public.dataset_doctor_training_decision_prompts; made public.dataset_doctor_training_decision_prompts;begin
 ...
 if not public.dataset_training_quality_overridable(j.summary,jsonb_array_length(ids),j.needs_more_images,to_jsonb(j.missing_coverage)) then raise exception 'DATASET_TRAINING_PROHIBITED';end if;
...
begin
 ...
 if dataset_job.summary->'dataset_ready'='true'::jsonb then if decision_authority is distinct from 'null'::jsonb then raise exception 'DATASET_TRAINING_DECISION_INVALID';end if;
  ...
  if not public.dataset_training_quality_overridable(dataset_job.summary,jsonb_array_length(stored_ids),dataset_job.needs_more_images,to_jsonb(dataset_job.missing_coverage)) then raise exception 'DATASET_TRAINING_PROHIBITED';end if;
