...
create function public.dataset_training_quality_overridable(p_summary jsonb,p_selected_count integer) returns boolean language plpgsql immutable set search_path='' as $$declare v text; item jsonb;begin
 if jsonb_typeof(p_summary)<>'object' or jsonb_typeof(p_summary->'dataset_ready')<>'boolean' or p_summary->'dataset_ready'<>'false'::jsonb or p_selected_count<3 then return false; end if;
 if p_summary ? 'non_overridable_conditions' and (jsonb_typeof(p_summary->'non_overridable_conditions')<>'array' or jsonb_array_length(p_summary->'non_overridable_conditions')>0) then return false; end if;
 if jsonb_typeof(coalesce(p_summary->'dataset_warnings','[]'::jsonb))<>'array' or jsonb_typeof(coalesce(p_summary->'secondary_issues','[]'::jsonb))<>'array' or jsonb_typeof(coalesce(p_summary->'dataset_warnings_structured','[]'::jsonb))<>'array' then return false; end if;
...
returns table(prompt_id uuid,decision_contract_version text,shown_at timestamptz,warning_snapshot jsonb) language plpgsql security definer set search_path='' as $$declare j public.dataset_doctor_jobs; ids jsonb; warnings jsonb; dataset_fp text; warning_fp text; existing public.dataset_doctor_training_decision_prompts; made public.dataset_doctor_training_decision_prompts;begin
 ...
 if jsonb_typeof(j.summary->'dataset_ready')<>'boolean' then raise exception 'DATASET_TRAINING_PROHIBITED';end if;
 if j.summary->'dataset_ready'='true'::jsonb then raise exception 'DATASET_TRAINING_DECISION_NOT_REQUIRED';end if;
 ...
 else if exists(select 1 from unnest(p_selected_image_ids)x left join public.dataset_doctor_images i on i.id=x and i.job_id=j.id and i.user_id=p_user_id and i.decision='accepted' where i.id is null) then raise exception 'DATASET_TRAINING_DECISION_INVALID';end if; end if;
 if not public.dataset_training_quality_overridable(j.summary,jsonb_array_length(ids)) then raise exception 'DATASET_TRAINING_PROHIBITED';end if;
...
returns table(receipt_id uuid,decision text,decision_contract_version text,decided_at timestamptz) language plpgsql security definer set search_path='' as $$declare p public.dataset_doctor_training_decision_prompts;j public.dataset_doctor_jobs;ids jsonb;warnings jsonb;r public.dataset_doctor_training_decision_receipts;begin
 ...
 ids:=public.dataset_training_final_selection(j.id);warnings:=public.dataset_training_warning_snapshot(j);
 if not public.dataset_training_quality_overridable(j.summary,jsonb_array_length(ids)) then raise exception 'DATASET_TRAINING_PROHIBITED';end if;
...
declare
 ...
 dataset_job public.dataset_doctor_jobs; requested_job_id uuid; stored_ids jsonb; decision_authority jsonb; receipt public.dataset_doctor_training_decision_receipts;
...
begin
 ...
 if jsonb_typeof(dataset_job.summary->'dataset_ready')<>'boolean' then raise exception 'DATASET_TRAINING_PROHIBITED';end if;
 decision_authority:=p_request_payload->'dataset_training_decision';
 if dataset_job.summary->'dataset_ready'='true'::jsonb then if decision_authority is distinct from 'null'::jsonb then raise exception 'DATASET_TRAINING_DECISION_INVALID';end if;
 else
  ...
  if not public.dataset_training_quality_overridable(dataset_job.summary,jsonb_array_length(stored_ids)) then raise exception 'DATASET_TRAINING_PROHIBITED';end if;
