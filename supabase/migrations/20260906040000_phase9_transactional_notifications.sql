...
create table public.transactional_notification_deliveries (
  ...
  terminal_reason text check (terminal_reason is null or terminal_reason in ('source_stale','ownership_mismatch','recipient_missing','recipient_invalid','attempts_exhausted','provider_outcome_uncertain')),
...
begin
  ...
  ), chosen as (select * from candidates where due_at is not null order by due_at limit p_limit), inserted as (
...
begin
 ...
 -- Expired leases represent an uncertain provider outcome: suppress rather than blindly resend.
 update public.transactional_notification_deliveries set state='failed_uncertain',terminal_reason='provider_outcome_uncertain',lease_token=null,lease_expires_at=null,updated_at=clock_timestamp()
 ...
 return query with picked as (
  select n.id from public.transactional_notification_deliveries n where n.state in ('pending','retry') and n.next_attempt_at<=clock_timestamp() order by n.due_at for update skip locked limit p_limit
 ), claimed as (update public.transactional_notification_deliveries n set state='claimed',attempts=n.attempts+1,lease_token=p_lease_token,lease_expires_at=clock_timestamp()+interval '10 minutes',updated_at=clock_timestamp() from picked where n.id=picked.id returning n.*)
...
revoke all on function public.materialize_phase9_notifications(integer),public.claim_phase9_notifications(uuid,integer),public.finalize_phase9_notification(uuid,uuid,text,text,text) from public,anon,authenticated;
grant execute on function public.materialize_phase9_notifications(integer),public.claim_phase9_notifications(uuid,integer),public.finalize_phase9_notification(uuid,uuid,text,text,text) to service_role;
