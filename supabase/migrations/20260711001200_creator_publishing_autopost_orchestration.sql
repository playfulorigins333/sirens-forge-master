...
create table if not exists public.creator_publishing_platform_capabilities (
  platform text primary key check (platform in ('onlyfans','fansly','fanvue','loyalfans','justforfans','x','reddit')),
  ...
  platform_supports_ppv boolean not null default false,
  platform_supports_visibility_controls boolean not null default false,
  platform_supports_native_scheduling boolean not null default false,
  platform_supports_drafts boolean not null default false,
  platform_requires_ai_disclosure boolean not null default true,
  platform_requires_creator_verification boolean not null default true,
  platform_requires_consent_records boolean not null default true,
  platform_blocks_fictional_personas boolean not null default false,
  connector_can_upload_media boolean not null default false,
  connector_can_publish_immediately boolean not null default false,
  connector_can_schedule_directly boolean not null default false,
  connector_can_fetch_publication_status boolean not null default false,
  connector_can_fetch_analytics boolean not null default false,
  human_operator_queue_supported boolean not null default false,
  human_publishing_required boolean not null default false,
  safe_label text not null,
  safe_description text not null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint creator_publishing_capabilities_disabled_consistent check ((availability_status in ('disabled','frozen','unassigned')) = (publishing_mode = 'disabled') or availability_status = 'available'),
...
insert into public.creator_publishing_platform_capabilities(platform,registry_version,display_name,publishing_mode,availability_status,platform_supports_ppv,platform_supports_visibility_controls,platform_supports_native_scheduling,platform_supports_drafts,connector_can_upload_media,connector_can_publish_immediately,connector_can_schedule_directly,human_operator_queue_supported,human_publishing_required,safe_label,safe_description)
values
('onlyfans','task14.20260711.001','OnlyFans','assisted','available',true,true,true,true,false,false,false,true,true,'Assisted publish required','Assisted publish required: Sirens Forge prepares and schedules the post package. Final publishing is completed manually by you or your authorized agency operator inside OnlyFans.'),
('fansly','task14.20260711.001','Fansly','disabled','unassigned',true,true,true,true,false,false,false,false,false,'Unavailable','Fansly routing is server-configurable but not yet assigned for active Autopost plan creation.'),
('fanvue','task14.20260711.001','Fanvue','disabled','frozen',true,true,true,true,false,false,false,false,false,'Unavailable','Fanvue is frozen for this Creator Publishing orchestration backbone and unavailable for new active plans.'),
('loyalfans','task14.20260711.001','LoyalFans','disabled','unassigned',false,false,false,false,false,false,false,false,false,'Unavailable','This destination is not yet assigned for active Autopost plan creation.'),
('justforfans','task14.20260711.001','JustForFans','disabled','unassigned',false,false,false,false,false,false,false,false,false,'Unavailable','This destination is not yet assigned for active Autopost plan creation.'),
('x','task14.20260711.001','X','disabled','unassigned',false,false,false,false,false,false,false,false,false,'Unavailable','This destination is not yet assigned for active Autopost plan creation.'),
('reddit','task14.20260711.001','Reddit','disabled','unassigned',false,false,false,false,false,false,false,false,false,'Unavailable','This destination is not yet assigned for active Autopost plan creation.')
...
comment on column public.creator_publishing_platform_jobs.job_state is 'Task 14 only creates draft. Other constrained values are reserved for future Tasks 15-17/Planner aggregation; no Task 14 mutation path transitions into them.';

create unique index if not exists creator_publishing_jobs_active_package_uidx on public.creator_publishing_platform_jobs(content_package_id) where job_state not in ('published_direct','confirmed_posted_manual','failed_manual_upload','direct_publish_failed','skipped','blocked','archived');
...
create or replace view public.creator_publishing_platform_capability_public as
...
returns boolean language sql stable set search_path = public, pg_temp as $$
  select p.updated_at = j.source_package_updated_at from public.creator_publishing_platform_jobs j join public.creator_publishing_content_packages p on p.id=j.content_package_id where j.id=p_job_id;
$$;
...
declare
  v_key text := btrim(coalesce(p_idempotency_key,'')); v_now timestamptz := now(); v_ids uuid[]; v_registry_version text; v_canonical jsonb; v_fingerprint text; v_existing public.creator_publishing_plans%rowtype; v_plan public.creator_publishing_plans%rowtype; v_plan_audit bigint; v_job_audits bigint[] := '{}'::bigint[]; v_jobs jsonb := '[]'::jsonb; r record; v_job public.creator_publishing_platform_jobs%rowtype; v_count int;
begin
  ...
  select registry_version into v_registry_version from public.creator_publishing_platform_capabilities group by registry_version order by registry_version desc limit 1;

  create temp table if not exists pg_temp.autopost_selected_packages as
  select p.id content_package_id,p.creator_id,p.platform_account_id,p.target_platform,p.updated_at source_package_updated_at,c.publishing_mode,c.registry_version,c.availability_status,c.human_publishing_required,c.connector_can_publish_immediately
  ...
  select p.id,p.creator_id,p.platform_account_id,p.target_platform,p.updated_at,c.publishing_mode,c.registry_version,c.availability_status,c.human_publishing_required,c.connector_can_publish_immediately
  from public.creator_publishing_content_packages p
  join public.creator_platform_accounts a on a.id=p.platform_account_id and a.creator_id=p.creator_id and a.platform=p.target_platform
  join public.creator_publishing_platform_capabilities c on c.platform=p.target_platform
  ...
  if exists(select 1 from public.creator_publishing_media_assets m right join pg_temp.autopost_selected_packages s on s.content_package_id=m.content_package_id group by s.content_package_id having count(m.id)=0 or bool_or(m.source <> 'ai_pipeline') or bool_or(not (m.ai_generation_metadata ? 'generation_id'))) then raise exception 'GENERATED_MEDIA_PROVENANCE_REQUIRED'; end if;
  if exists(select 1 from public.creator_publishing_platform_jobs j join pg_temp.autopost_selected_packages s on s.content_package_id=j.content_package_id where j.job_state not in ('published_direct','confirmed_posted_manual','failed_manual_upload','direct_publish_failed','skipped','blocked','archived')) then raise exception 'ACTIVE_PUBLICATION_JOB_CONFLICT'; end if;

  v_canonical := jsonb_build_object('creator_id',p_creator_id,'content_package_ids',(select jsonb_agg(content_package_id order by content_package_id) from pg_temp.autopost_selected_packages),'platform_account_ids',(select jsonb_agg(platform_account_id order by content_package_id) from pg_temp.autopost_selected_packages),'target_platforms',(select jsonb_agg(target_platform order by content_package_id) from pg_temp.autopost_selected_packages),'source_package_versions',(select jsonb_agg(source_package_updated_at order by content_package_id) from pg_temp.autopost_selected_packages),'publishing_modes',(select jsonb_agg(publishing_mode order by content_package_id) from pg_temp.autopost_selected_packages),'registry_version',v_registry_version);
  ...
  if found then
    ...
    select coalesce(jsonb_agg(to_jsonb(j) order by j.created_at,j.id),'[]'::jsonb) into v_jobs from public.creator_publishing_platform_jobs j where j.publishing_plan_id=v_existing.id;
    return jsonb_build_object('plan',to_jsonb(v_existing),'jobs',v_jobs,'audit_event_ids',jsonb_build_object('plan',v_existing.original_plan_audit_event_id,'jobs',v_existing.original_job_audit_event_ids),'registry_version',v_existing.registry_version,'idempotent',true,'request_fingerprint',v_existing.request_fingerprint);
  end if;
  ...
  insert into public.creator_publishing_audit_events(entity_type,entity_id,actor_id,actor_role,action,before_state,after_state,idempotency_key,created_at) values('creator_publishing_plan',v_plan.id,p_creator_id,'creator','creator_publishing_plan_created',null,jsonb_build_object('plan_id',v_plan.id,'creator_id',p_creator_id,'status','draft','request_canonical',v_canonical,'request_fingerprint',v_fingerprint,'registry_version',v_registry_version),v_key,v_now) returning id into v_plan_audit;
  for r in select * from pg_temp.autopost_selected_packages order by content_package_id loop
    insert into public.creator_publishing_platform_jobs(publishing_plan_id,creator_id,content_package_id,platform_account_id,target_platform,publishing_mode,job_state,source_package_updated_at,capability_registry_version,original_request_fingerprint,created_at,updated_at)
    values(v_plan.id,p_creator_id,r.content_package_id,r.platform_account_id,r.target_platform,r.publishing_mode,'draft',r.source_package_updated_at,r.registry_version,v_fingerprint,v_now,v_now) returning * into v_job;
    insert into public.creator_publishing_audit_events(entity_type,entity_id,actor_id,actor_role,action,before_state,after_state,idempotency_key,created_at) values('creator_publishing_platform_job',v_job.id,p_creator_id,'creator','creator_publishing_platform_job_created',null,jsonb_build_object('job_id',v_job.id,'publishing_plan_id',v_plan.id,'content_package_id',r.content_package_id,'platform_account_id',r.platform_account_id,'target_platform',r.target_platform,'publishing_mode',r.publishing_mode,'job_state','draft','source_package_updated_at',r.source_package_updated_at,'registry_version',r.registry_version,'request_fingerprint',v_fingerprint),v_key,v_now) returning id into v_plan_audit;
    v_job_audits := array_append(v_job_audits,v_plan_audit); v_jobs := v_jobs || to_jsonb(v_job);
  end loop;
  select id into v_plan_audit from public.creator_publishing_audit_events where entity_type='creator_publishing_plan' and entity_id=v_plan.id and action='creator_publishing_plan_created' limit 1;
  update public.creator_publishing_plans set original_plan_audit_event_id=v_plan_audit, original_job_audit_event_ids=v_job_audits where id=v_plan.id returning * into v_plan;
  return jsonb_build_object('plan',to_jsonb(v_plan),'jobs',v_jobs,'audit_event_ids',jsonb_build_object('plan',v_plan.original_plan_audit_event_id,'jobs',v_plan.original_job_audit_event_ids),'registry_version',v_registry_version,'idempotent',false,'request_fingerprint',v_fingerprint);
end;
$$;
