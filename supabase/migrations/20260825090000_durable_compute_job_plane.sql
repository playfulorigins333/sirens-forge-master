...
create table public.compute_jobs (
 ...
 check((state in ('claimed','running','cancel_requested') and lease_token is not null and lease_expires_at is not null) or (state in ('queued','recovering','succeeded','failed','cancelled') and lease_token is null and lease_expires_at is null))
);
...
create table public.compute_cost_ledger (
 ...
 check((kind='reservation' and amount_micros>0 and correction_key is null and reason_code is null) or (kind='actual' and amount_micros>=0 and correction_key is null and reason_code is null) or (kind='release' and amount_micros<0 and correction_key is null and reason_code is null) or (kind='correction' and amount_micros<>0 and length(correction_key) between 1 and 128 and reason_code ~ '^[A-Z][A-Z0-9_]{0,63}$'))
);
...
create index compute_jobs_active_owner_idx on public.compute_jobs(owner_id,workload) where workload in ('trainer','image','video') and state in ('claimed','running','recovering','cancel_requested');
...
create unique index compute_jobs_one_active_owner_workload_idx on public.compute_jobs(owner_id,workload) where workload in ('trainer','image','video') and state in ('claimed','running','recovering','cancel_requested');
...
create function public.release_compute_reservation(p_attempt_id uuid) returns boolean language plpgsql security definer set search_path=pg_catalog,public as $$
declare a public.compute_job_attempts;
begin
 select * into a from public.compute_job_attempts where id=p_attempt_id for update; if not found then raise exception 'ATTEMPT_NOT_FOUND'; end if;
 if a.reserved_cost_micros is null or a.actual_cost_micros is not null or exists(select 1 from public.compute_cost_ledger where attempt_id=a.id and kind='release') then return false; end if;
 insert into public.compute_cost_ledger(job_id,attempt_id,kind,amount_micros) values(a.job_id,a.id,'release',-a.reserved_cost_micros); return true;
end$$;
...
create function public.begin_compute_provider_dispatch(p_job_id uuid,p_attempt_id uuid,p_lease_token uuid) returns timestamptz language plpgsql security definer set search_path=pg_catalog,public as $$
declare j public.compute_jobs; a public.compute_job_attempts;
begin
 ...
 if a.reserved_cost_micros is null or a.spend_policy_id is null or exists(select 1 from public.compute_cost_ledger where attempt_id=a.id and kind='release') then raise exception 'SPEND_AUTHORIZATION_REQUIRED'; end if;
 update public.compute_job_attempts set provider_dispatch_intent_at=coalesce(provider_dispatch_intent_at,now()) where id=a.id; return (select provider_dispatch_intent_at from public.compute_job_attempts where id=a.id);
end$$;
...
begin
 ...
 select * into j from public.compute_jobs where id=p_job_id for update; select * into a from public.compute_job_attempts where id=p_attempt_id and job_id=p_job_id for update;
 if not found or j.lease_token is distinct from p_lease_token or a.lease_token<>p_lease_token or a.finished_at is not null or j.lease_expires_at<=clock_timestamp() or j.state not in ('claimed','running','cancel_requested') then raise exception 'LEASE_MISMATCH'; end if;
 if a.provider_dispatch_intent_at is null then raise exception 'DISPATCH_INTENT_REQUIRED'; end if;
...
begin
 ...
 if a.provider_dispatch_intent_at is not null or a.provider_dispatched_at is not null or a.provider_operation_ref is not null then raise exception 'POST_DISPATCH_RETRY_FORBIDDEN'; end if;
 perform public.release_compute_reservation(a.id); update public.compute_job_attempts set finished_at=now(),outcome_class='retryable_failure',safe_error_code=safe_code where id=a.id;
...
begin
 for j in select * from public.compute_jobs where state in ('claimed','running','cancel_requested') and lease_expires_at<now() order by id for update skip locked loop
  ...
  if a.provider_dispatch_intent_at is not null or a.provider_dispatched_at is not null or a.provider_operation_ref is not null then
   ...
   perform public.release_compute_reservation(a.id); update public.compute_jobs set state='cancelled',terminal_at=now(),lease_token=null,lease_expires_at=null,updated_at=now() where id=j.id;
   ...
   perform public.release_compute_reservation(a.id); update public.compute_jobs set state='queued',retry_count=retry_count+1,lease_token=null,lease_expires_at=null,available_at=now(),updated_at=now() where id=j.id;
   update public.compute_job_attempts set finished_at=now(),outcome_class='lease_expired' where id=a.id;
  else
   perform public.release_compute_reservation(a.id); update public.compute_jobs set state='failed',retry_count=retry_count+1,lease_token=null,lease_expires_at=null,terminal_at=now(),safe_error_code='RETRY_LIMIT_REACHED',updated_at=now() where id=j.id;
   update public.compute_job_attempts set finished_at=now(),outcome_class='lease_expired' where id=a.id;
  end if; n:=n+1;
 end loop; return n;
end$$;
...
create function public.compute_recovery_signal(p_job_id uuid,p_attempt_id uuid,p_recovery_lease_token uuid)
returns table(job_state public.compute_job_state,cancellation_requested boolean) language plpgsql security definer set search_path=pg_catalog,public as $$
declare j public.compute_jobs; a public.compute_job_attempts;
begin
 select * into j from public.compute_jobs where id=p_job_id;
 select * into a from public.compute_job_attempts where id=p_attempt_id and job_id=p_job_id;
 if not found or j.state<>'recovering' or a.ordinal<>j.attempt_count or a.finished_at is not null or a.recovery_lease_token is distinct from p_recovery_lease_token or a.recovery_lease_expires_at<=clock_timestamp() then raise exception 'RECOVERY_LEASE_MISMATCH'; end if;
 return query select j.state,j.cancellation_requested_at is not null;
end$$;
...
begin
 ...
 select * into pol from public.compute_spend_policies where enabled and effective_from<=now() and (effective_until is null or effective_until>now()) order by version desc limit 1 for update;
...
declare existing public.compute_cost_ledger; policy_id uuid; inserted_id uuid; net_cost bigint;
begin
