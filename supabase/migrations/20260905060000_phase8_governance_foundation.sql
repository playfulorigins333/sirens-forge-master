...
--
-- Supabase CLI was unavailable in the execution environment, so the migration
-- file was created manually using the repository's established timestamp format.
...
create table public.retention_policy_versions (
  ...
  constraint retention_policy_key_check
    check (policy_key ~ '^[a-z0-9][a-z0-9_]{2,79}$'),
  constraint retention_policy_version_check check (policy_version > 0),
  constraint retention_policy_subject_type_check
    check (subject_type ~ '^[a-z0-9][a-z0-9_]{2,79}$'),
  constraint retention_policy_duration_check
    check (retention_duration >= interval '0 seconds'),
  constraint retention_policy_purge_mode_check
    check (purge_mode in ('automatic','manual_or_automatic','manual_only')),
  constraint retention_policy_document_version_check
    check (char_length(policy_document_version) between 3 and 120),
  constraint retention_policy_effective_check check (effective_at <= created_at + interval '5 seconds')
);
...
as $$
  ...
  order by p.policy_version desc, p.effective_at desc
  limit 1
$$;
...
create table public.governance_audit_events (
  ...
  constraint governance_audit_actor_type_check
    check (actor_type in ('creator','founder_admin','system','service')),
  constraint governance_audit_actor_identity_check
    check (actor_type in ('system','service') or actor_user_id is not null),
  constraint governance_audit_action_check
    check (action ~ '^[a-z0-9][a-z0-9_.:-]{2,119}$'),
  constraint governance_audit_target_type_check
    check (target_type ~ '^[a-z0-9][a-z0-9_]{2,79}$'),
  constraint governance_audit_target_id_check
    check (char_length(target_id) between 1 and 200 and target_id !~ '[[:cntrl:]]'),
  constraint governance_audit_reason_category_check
    check (reason_category is null or (char_length(reason_category) between 1 and 80 and reason_category !~ '[[:cntrl:]]')),
  constraint governance_audit_reason_check
    check (reason is null or (char_length(reason) between 1 and 1000 and reason !~ '[[:cntrl:]]')),
  constraint governance_audit_result_check
    check (char_length(result) between 1 and 80 and result !~ '[[:cntrl:]]'),
  constraint governance_audit_policy_version_check
    check (policy_version is null or char_length(policy_version) between 1 and 120),
  constraint governance_audit_form_version_check
    check (form_version is null or char_length(form_version) between 1 and 120),
  constraint governance_audit_request_id_check
    check (request_id is null or (char_length(request_id) between 1 and 200 and request_id !~ '[[:cntrl:]]')),
  constraint governance_audit_facts_check
    check (jsonb_typeof(facts)='object' and not public.governance_jsonb_has_forbidden_private_key(facts)),
  constraint governance_audit_reference_hashes_check
    check (jsonb_typeof(reference_hashes)='object' and not public.governance_jsonb_has_forbidden_private_key(reference_hashes)),
  constraint governance_audit_previous_hash_check
...
create index governance_audit_target_idx
  on public.governance_audit_events(target_type,target_id,sequence_no desc);
create index governance_audit_actor_idx
  on public.governance_audit_events(actor_user_id,sequence_no desc)
  where actor_user_id is not null;
create index governance_audit_correlation_idx
  on public.governance_audit_events(correlation_id,sequence_no desc);
...
grant select on table public.governance_audit_events to service_role;
...
begin
  ...
  if jsonb_typeof(coalesce(p_facts,'{}'::jsonb))<>'object' or public.governance_jsonb_has_forbidden_private_key(coalesce(p_facts,'{}'::jsonb)) then raise exception 'GOVERNANCE_AUDIT_PRIVATE_CONTENT_FORBIDDEN'; end if;
  if jsonb_typeof(coalesce(p_reference_hashes,'{}'::jsonb))<>'object' or public.governance_jsonb_has_forbidden_private_key(coalesce(p_reference_hashes,'{}'::jsonb)) then raise exception 'GOVERNANCE_AUDIT_PRIVATE_CONTENT_FORBIDDEN'; end if;
  ...
  v_payload := jsonb_build_object(
    ...
    'reference_hashes',coalesce(p_reference_hashes,'{}'::jsonb),
...
create table public.governance_action_receipts (
  ...
  constraint governance_receipt_type_check check (receipt_type in (
    'ai_likeness_identity_consent',
    'onlyfans_publishing_1to1_declaration',
    'dataset_doctor_train_anyway',
    'account_deletion',
    'creator_export_choice',
    'material_policy_acceptance',
    'platform_connect',
    'platform_disconnect',
    'admin_private_content_access'
  )),
  ...
  constraint governance_receipt_facts_check check (jsonb_typeof(facts)='object' and not public.governance_jsonb_has_forbidden_private_key(facts)),
...
create index governance_action_receipts_subject_idx
  on public.governance_action_receipts(subject_user_id,created_at desc);
create index governance_action_receipts_target_idx
  on public.governance_action_receipts(target_type,target_id,created_at desc);
...
begin
  ...
  if jsonb_typeof(coalesce(p_facts,'{}'::jsonb))<>'object' or public.governance_jsonb_has_forbidden_private_key(coalesce(p_facts,'{}'::jsonb)) then raise exception 'GOVERNANCE_RECEIPT_PRIVATE_CONTENT_FORBIDDEN'; end if;
...
create table public.governance_legal_holds (
  ...
  constraint governance_legal_hold_window_check check (review_due_at > opened_at and expires_at >= review_due_at),
  constraint governance_legal_hold_fresh_auth_method_check check (fresh_auth_method='totp'),
...
declare
  ...
  v_case_hash text;
begin
  if p_actor_user_id is null or not exists(select 1 from auth.users where id=p_actor_user_id) then raise exception 'GOVERNANCE_LEGAL_HOLD_INVALID'; end if;
  ...
  if p_review_due_at is null or p_expires_at is null or p_review_due_at<=v_now or p_expires_at<p_review_due_at then raise exception 'GOVERNANCE_LEGAL_HOLD_INVALID'; end if;
  if p_fresh_auth_method<>'totp' or p_fresh_auth_at is null or p_fresh_auth_at < v_now-interval '10 minutes' or p_fresh_auth_at > v_now+interval '5 seconds' then raise exception 'GOVERNANCE_LEGAL_HOLD_FRESH_AUTH_REQUIRED'; end if;
  ...
  for v_target in select value from jsonb_array_elements(p_targets) loop
    ...
    if coalesce(v_target_type,'') !~ '^[a-z0-9][a-z0-9_]{2,79}$'
       ...
       or not exists(select 1 from auth.users where id=v_subject_user_id) then
      raise exception 'GOVERNANCE_LEGAL_HOLD_TARGET_INVALID';
    end if;
...
declare
  ...
  v_audit_id uuid;
begin
  if p_hold_id is null or p_actor_user_id is null or not exists(select 1 from auth.users where id=p_actor_user_id) then raise exception 'GOVERNANCE_LEGAL_HOLD_INVALID'; end if;
...
as $$
  select exists(
    ...
    where t.target_type=p_target_type and t.target_id=p_target_id
      and h.status='active' and h.expires_at>statement_timestamp()
  )
$$;
revoke all on function public.governance_target_has_active_legal_hold(text,text) from public, anon, authenticated;
grant execute on function public.governance_target_has_active_legal_hold(text,text) to service_role;
