...
create or replace function public.creator_publishing_operator_current_safety_gate(p_job public.creator_publishing_platform_jobs,p_task public.creator_publishing_queue_tasks,p_actor_id uuid,p_expected_ai_twin_consent_version text,p_expected_attestation_text_sha256 text)
returns text language plpgsql stable set search_path=public,pg_temp as $$
declare capability_rec public.creator_publishing_platform_capabilities%rowtype; package_rec public.creator_publishing_content_packages%rowtype; account_rec public.creator_platform_accounts%rowtype; consent_rec public.creator_publishing_ai_twin_consents%rowtype; evidence_rec public.creator_publishing_compliance_reviews%rowtype; v_queue_count integer;
begin
  ...
  if p_job.publishing_mode<>'assisted' then return 'OPERATOR_TARGET_NOT_SUPPORTED'; end if;
  select * into capability_rec from public.creator_publishing_platform_capabilities where platform='onlyfans';
  if not found or capability_rec.availability_status<>'available' then return 'PLATFORM_UNAVAILABLE'; end if;
  if capability_rec.publishing_mode<>'assisted' then return 'PLATFORM_MODE_UNSUPPORTED'; end if;
  if p_job.job_state not in ('draft','scheduled_internally','awaiting_operator','due_now') then return 'OPERATOR_TASK_INELIGIBLE'; end if;
  if p_job.cancelled_at is not null then return 'OPERATOR_TASK_INELIGIBLE'; end if;
  ...
  select * into account_rec from public.creator_platform_accounts where id=p_job.platform_account_id and creator_id=p_job.creator_id and platform=p_job.target_platform;
  if not found then return 'DESTINATION_ACCOUNT_NOT_FOUND'; end if;
  if account_rec.verification_status='revoked' then return 'DESTINATION_ACCOUNT_REVOKED'; end if;
  if account_rec.verification_status<>'verified' then return 'DESTINATION_ACCOUNT_NOT_VERIFIED'; end if;
  select * into consent_rec from public.creator_publishing_ai_twin_consents where creator_id=p_job.creator_id;
  if not found or consent_rec.status<>'granted' or consent_rec.revoked_at is not null then return 'AI_TWIN_CONSENT_MISSING'; end if;
  if coalesce(p_expected_ai_twin_consent_version,'')='' or consent_rec.attestation_version<>p_expected_ai_twin_consent_version then return 'AI_TWIN_CONSENT_MISSING'; end if;
  if coalesce(p_expected_attestation_text_sha256,'') !~ '^[a-f0-9]{64}$' or consent_rec.attestation_text_sha256<>p_expected_attestation_text_sha256 then return 'AI_TWIN_CONSENT_MISSING'; end if;
  select * into package_rec from public.creator_publishing_content_packages where id=p_job.content_package_id and creator_id=p_job.creator_id and platform_account_id=p_job.platform_account_id and target_platform=p_job.target_platform;
  if not found then return 'CONTENT_PACKAGE_MISMATCH'; end if;
  if package_rec.creator_approval_status<>'approved' then return 'CREATOR_APPROVAL_MISSING'; end if;
  if package_rec.compliance_status not in ('passed','escalated_approved') then return 'COMPLIANCE_EVIDENCE_INVALID'; end if;
  if package_rec.compliance_status='passed' then
    select * into evidence_rec
    from public.creator_publishing_compliance_reviews r
    where r.content_package_id=p_job.content_package_id
      and r.compliance_policy_version=package_rec.compliance_policy_version
      and r.review_source='automated'
      and r.outcome='pass'
    ...
    where r.content_package_id=p_job.content_package_id
      and r.compliance_policy_version=package_rec.compliance_policy_version
      and r.review_source='human'
      and r.outcome='escalate'
      and length(btrim(coalesce(r.escalated_approval_reason,''))) > 0
    order by r.created_at desc, r.id desc
    limit 1;
  end if;
  if not found then return 'COMPLIANCE_EVIDENCE_INVALID'; end if;
  if exists(
    select 1
    from public.creator_publishing_compliance_reviews r
    where r.content_package_id=p_job.content_package_id
      and r.outcome in ('block','manual_review')
      and (r.created_at > evidence_rec.created_at or (r.created_at = evidence_rec.created_at and r.id > evidence_rec.id))
  ) then return 'COMPLIANCE_EVIDENCE_INVALID'; end if;
  if package_rec.second_person_present and not exists(select 1 from public.creator_publishing_co_performer_records c where c.content_package_id=p_job.content_package_id) then return 'CO_PERFORMER_RELEASE_MISSING'; end if;
  if exists(select 1 from public.creator_publishing_co_performer_records c where c.content_package_id=p_job.content_package_id and (c.platform_release_confirmed is not true or length(btrim(coalesce(c.release_document_reference,'')))=0)) then return 'CO_PERFORMER_RELEASE_MISSING'; end if;
  if p_job.source_package_updated_at is distinct from package_rec.updated_at then return 'SOURCE_FINGERPRINT_STALE'; end if;
  ...
  if not public.creator_publishing_operator_is_authorized(p_job.creator_id,p_actor_id,'onlyfans') then return 'OPERATOR_NOT_AUTHORIZED'; end if;
  return null;
end; $$;

create or replace function public.creator_publishing_operator_queue_is_clean(p_task public.creator_publishing_queue_tasks) returns boolean language sql stable set search_path=public,pg_temp as $$
  select p_task.posted_by is null
    and p_task.posted_at is null
    and p_task.posted_confirmation is false
    and p_task.final_post_url is null
    and p_task.final_post_url_skip_reason is null
    and p_task.proof_screenshot_storage_key is null
    and p_task.skip_or_fail_reason is null;
$$;
...
begin
 ...
 if task.status='claimed' and task.claim_expires_at <= v_now then
   ...
   values('creator_publishing_queue_task',prior_task.id,p_actor_id,'operator','operator_expired_claim_recovered',jsonb_build_object('status',prior_task.status,'claimed_by',prior_task.claimed_by,'claimed_at',prior_task.claimed_at,'claim_expires_at',prior_task.claim_expires_at,'claim_attempt_count',prior_task.claim_attempt_count,'progress_state',prior_task.operator_progress_state,'progress_revision',prior_task.operator_progress_revision,'progress_updated_by',prior_task.operator_progress_updated_by,'progress_updated_at',prior_task.operator_progress_updated_at,'assigned_operator_id',prior_task.assigned_operator_id),jsonb_build_object('queue_task_id',task.id,'platform_job_id',job.id,'status',restore),p_idempotency_key,v_now);
 end if;
 ...
 insert into public.creator_publishing_audit_events(entity_type,entity_id,actor_id,actor_role,action,before_state,after_state,idempotency_key,created_at) values('creator_publishing_queue_task',task.id,p_actor_id,'operator','operator_task_claimed',jsonb_build_object('status',prior_task.status,'claimed_by',prior_task.claimed_by,'claimed_at',prior_task.claimed_at,'claim_expires_at',prior_task.claim_expires_at,'progress_state',prior_task.operator_progress_state,'progress_revision',prior_task.operator_progress_revision,'assigned_operator_id',prior_task.assigned_operator_id),result - 'claim_token',p_idempotency_key,v_now);
...
begin
  perform public.creator_publishing_operator_validate_idempotency_key(p_idempotency_key);
  if p_actor_id is null or p_queue_task_id is null or p_platform_job_id is null or p_claim_token is null then raise exception 'OPERATOR_REQUEST_INVALID'; end if;
  fp:=public.creator_publishing_operator_request_fingerprint(jsonb_build_object('actor',p_actor_id,'task',p_queue_task_id,'job',p_platform_job_id,'token',p_claim_token));
  replay:=public.creator_publishing_operator_replay_or_conflict(p_actor_id,'release',p_idempotency_key,fp);
  ...
  select * into task from public.creator_publishing_queue_tasks where id=p_queue_task_id for update;
  ...
  if not public.creator_publishing_operator_is_authorized(job.creator_id,p_actor_id,'onlyfans') then raise exception 'OPERATOR_NOT_AUTHORIZED'; end if;
  if not public.creator_publishing_operator_queue_is_clean(task) then raise exception 'OPERATOR_TASK_INELIGIBLE'; end if;
  if task.status<>'claimed' or task.claimed_by<>p_actor_id or task.claim_token<>p_claim_token or task.claim_expires_at<=v_now then raise exception 'OPERATOR_CLAIM_TOKEN_MISMATCH'; end if;
  prior_task:=task;
  restore:=public.creator_publishing_operator_restore_queue_status(job,v_now);
  if restore is null then raise exception 'OPERATOR_TASK_INELIGIBLE'; end if;
  update public.creator_publishing_queue_tasks set status=restore, claimed_by=null, claimed_at=null, claim_token=null, claim_expires_at=null where id=task.id returning * into task;
  result:=jsonb_build_object('ok',true,'action','release','queue_task_id',task.id,'platform_job_id',job.id,'status',task.status);
  insert into public.creator_publishing_audit_events(entity_type,entity_id,actor_id,actor_role,action,before_state,after_state,idempotency_key,created_at)
  values(
    'creator_publishing_queue_task',task.id,p_actor_id,'operator','operator_task_released',
    jsonb_build_object('status',prior_task.status,'claimed_by',prior_task.claimed_by,'claimed_at',prior_task.claimed_at,'claim_expires_at',prior_task.claim_expires_at,'claim_attempt_count',prior_task.claim_attempt_count,'progress_state',prior_task.operator_progress_state,'progress_revision',prior_task.operator_progress_revision,'progress_updated_by',prior_task.operator_progress_updated_by,'progress_updated_at',prior_task.operator_progress_updated_at,'assigned_operator_id',prior_task.assigned_operator_id),
    result,
    p_idempotency_key,
    v_now
  );
  insert into public.creator_publishing_operator_action_idempotency(actor_id,creator_id,queue_task_id,platform_job_id,action_type,idempotency_key,request_fingerprint,stored_result,created_at)
  values(p_actor_id,job.creator_id,task.id,job.id,'release',p_idempotency_key,fp,result,v_now);
  return result;
end; $$;
...
begin
  ...
  replay:=public.creator_publishing_operator_replay_or_conflict(p_actor_id,'progress_update',p_idempotency_key,fp); if replay is not null then return replay; end if;
  select * into job from public.creator_publishing_platform_jobs where id=p_platform_job_id for update; if not found then raise exception 'OPERATOR_JOB_NOT_FOUND'; end if;
  select * into task from public.creator_publishing_queue_tasks where id=p_queue_task_id for update; if not found then raise exception 'OPERATOR_TASK_NOT_FOUND'; end if;
  if task.content_package_id<>job.content_package_id or task.creator_id<>job.creator_id or task.platform_account_id<>job.platform_account_id or task.target_platform<>job.target_platform then raise exception 'OPERATOR_TASK_JOB_MISMATCH'; end if;
  if task.target_platform<>'onlyfans' or job.publishing_mode<>'assisted' then raise exception 'OPERATOR_TARGET_NOT_SUPPORTED'; end if;
  ...
  if task.operator_progress_state<>p_expected_progress_state or task.operator_progress_revision<>p_expected_progress_revision then raise exception 'OPERATOR_PROGRESS_STALE'; end if;
  if not ((p_expected_progress_state='not_started' and p_next_progress_state='preparing') or (p_expected_progress_state='preparing' and p_next_progress_state='prepared') or (p_expected_progress_state='prepared' and p_next_progress_state='handoff_ready')) then raise exception 'OPERATOR_PROGRESS_INVALID_TRANSITION'; end if;
  update public.creator_publishing_queue_tasks set operator_progress_state=p_next_progress_state, operator_progress_updated_by=p_actor_id, operator_progress_updated_at=v_now, operator_progress_revision=operator_progress_revision+1 where id=task.id returning * into task;
  action:=case p_next_progress_state when 'preparing' then 'operator_preparation_started' when 'prepared' then 'operator_package_prepared' else 'operator_handoff_ready' end;
  result:=jsonb_build_object('ok',true,'action','progress_update','queue_task_id',task.id,'platform_job_id',job.id,'progress_state',task.operator_progress_state,'progress_revision',task.operator_progress_revision);
  insert into public.creator_publishing_audit_events(entity_type,entity_id,actor_id,actor_role,action,before_state,after_state,idempotency_key,created_at) values('creator_publishing_queue_task',task.id,p_actor_id,'operator',action,jsonb_build_object('progress_state',p_expected_progress_state,'progress_revision',p_expected_progress_revision),result,p_idempotency_key,v_now);
  insert into public.creator_publishing_operator_action_idempotency(actor_id,creator_id,queue_task_id,platform_job_id,action_type,idempotency_key,request_fingerprint,stored_result,created_at) values(p_actor_id,job.creator_id,task.id,job.id,'progress_update',p_idempotency_key,fp,result,v_now);
  return result;
end; $$;
...
begin
 ...
 insert into public.creator_publishing_audit_events(entity_type,entity_id,actor_id,actor_role,action,before_state,after_state,idempotency_key,created_at)
 values('creator_publishing_queue_task',prior_task.id,p_actor_id,'operator','operator_expired_claim_recovered',jsonb_build_object('status',prior_task.status,'claimed_by',prior_task.claimed_by,'claimed_at',prior_task.claimed_at,'claim_expires_at',prior_task.claim_expires_at,'claim_attempt_count',prior_task.claim_attempt_count,'progress_state',prior_task.operator_progress_state,'progress_revision',prior_task.operator_progress_revision,'progress_updated_by',prior_task.operator_progress_updated_by,'progress_updated_at',prior_task.operator_progress_updated_at,'assigned_operator_id',prior_task.assigned_operator_id),result,p_idempotency_key,v_now);
...
revoke all on function public.creator_publishing_operator_validate_idempotency_key(text) from public, anon, authenticated;
revoke all on function public.creator_publishing_operator_is_authorized(uuid,uuid,text) from public, anon, authenticated;
revoke all on function public.creator_publishing_operator_current_safety_gate(public.creator_publishing_platform_jobs,public.creator_publishing_queue_tasks,uuid,text,text) from public, anon, authenticated;
revoke all on function public.creator_publishing_operator_restore_queue_status(public.creator_publishing_platform_jobs,timestamptz) from public, anon, authenticated;
revoke all on function public.creator_publishing_operator_queue_is_clean(public.creator_publishing_queue_tasks) from public, anon, authenticated;
revoke all on function public.creator_publishing_operator_request_fingerprint(jsonb) from public, anon, authenticated;
revoke all on function public.creator_publishing_operator_replay_or_conflict(uuid,text,text,text) from public, anon, authenticated;
