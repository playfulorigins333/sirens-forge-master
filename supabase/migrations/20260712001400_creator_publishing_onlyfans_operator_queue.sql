...
declare capability_rec public.creator_publishing_platform_capabilities%rowtype; package_rec public.creator_publishing_content_packages%rowtype; account_rec public.creator_platform_accounts%rowtype; consent_rec public.creator_publishing_ai_twin_consents%rowtype; v_queue_count integer;
begin
  ...
  if not exists(select 1 from public.creator_publishing_compliance_reviews r where r.content_package_id=p_job.content_package_id and r.outcome in ('pass','escalate') and r.compliance_policy_version=package_rec.compliance_policy_version) then return 'COMPLIANCE_EVIDENCE_INVALID'; end if;
  if exists(select 1 from public.creator_publishing_compliance_reviews r where r.content_package_id=p_job.content_package_id and r.outcome in ('block','escalate') and r.created_at > coalesce(package_rec.creator_approved_at,package_rec.updated_at)) then return 'COMPLIANCE_EVIDENCE_INVALID'; end if;
