...
returns table(switch_outcome text,reservation_id uuid,expires_at timestamptz,stripe_session_id text,reservation_tier text)
language plpgsql security definer set search_path = public, pg_temp as $$
...
begin
 ...
 if not found or old.purchaser_token_hash<>p_purchaser_token_hash or old.tier=p_tier or old.status not in ('associated','expired') or old.stripe_session_id is distinct from p_previous_session_id or p_previous_session_id is null then raise exception 'switch_conflict'; end if;
 select x.* into fresh from public.checkout_capacity_reservations x where x.purchaser_token_hash=p_purchaser_token_hash and x.tier=p_tier and x.status in ('active','associated') for update;
 if found then return query select 'switched'::text,fresh.id,fresh.expires_at,fresh.stripe_session_id,fresh.tier; return; end if;
 -- Provider expiration has already been proven. Reconcile the dead hold outside the
 -- exception-handled target allocation block so target failures cannot revive it.
 update public.checkout_capacity_reservations set status='expired',updated_at=now() where id=old.id and status='associated';
 begin
  select count(*) into hourly from public.checkout_guest_rate_limit_attempts a where a.network_hash=p_network_hash and a.created_at>now()-interval '60 minutes';
  if hourly>=5 then return query select 'closed_rate_limited'::text,null::uuid,null::timestamptz,null::text,null::text; return; end if;
  select count(*) into daily from public.checkout_guest_rate_limit_attempts a where a.network_hash=p_network_hash and a.created_at>now()-interval '24 hours';
  if daily>=10 then return query select 'closed_rate_limited'::text,null::uuid,null::timestamptz,null::text,null::text; return; end if;
  select x.* into t from public.subscription_tiers x where x.name=p_tier for update;
  if not found or not coalesce(t.is_active,false) or t.max_slots is null then return query select 'closed_plan_unavailable'::text,null::uuid,null::timestamptz,null::text,null::text; return; end if;
  select count(*) into paid from public.user_subscriptions s where s.tier_name=p_tier and s.status in ('active','trialing') and not (p_tier='og_throne' and coalesce(s.metadata->>'counts_toward_seats','true')='false');
  ...
  if paid+held>=t.max_slots then return query select 'closed_sold_out'::text,null::uuid,null::timestamptz,null::text,null::text; return; end if;
  ...
  return query select 'switched'::text,fresh.id,fresh.expires_at,fresh.stripe_session_id,fresh.tier; return;
 exception when others then
  return query select 'closed_database_failure'::text,null::uuid,null::timestamptz,null::text,null::text; return;
 end;
end $$;
