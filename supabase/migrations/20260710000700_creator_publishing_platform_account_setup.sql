  ...
  if p_account_id is null then
    ...
    if found then
      if v_duplicate.profile_url is not distinct from v_profile_url
        ...
        if exists (select 1 from public.creator_publishing_audit_events where entity_type = 'creator_platform_account' and entity_id = v_duplicate.id and idempotency_key = v_idempotency_key) then
          return v_duplicate;
        end if;
      end if;
    ...
    if exists (select 1 from public.creator_publishing_audit_events where entity_type = 'creator_platform_account' and entity_id = p_account_id and idempotency_key = v_idempotency_key) then
      return v_existing;
    end if;
    select * into v_duplicate from public.creator_platform_accounts
