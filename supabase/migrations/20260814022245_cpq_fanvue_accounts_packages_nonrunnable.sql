-- Enable canonical OAuth-linked Fanvue destinations for CPQ package composition only.
-- Fanvue execution guards remain unchanged; this migration performs no data backfill.
...
begin
  ...
  if v_account.platform not in ('onlyfans','fansly','fanvue') then raise exception 'UNSUPPORTED_PLATFORM'; end if;
  if v_account.verification_status = 'revoked' then raise exception 'PLATFORM_ACCOUNT_REVOKED'; end if;
  if v_account.platform = 'fanvue' then
    if v_account.oauth_account_id is null or not exists (
      select 1 from public.autopost_accounts oauth
      where oauth.id = v_account.oauth_account_id
        and oauth.user_id = p_creator_id
        and oauth.platform = 'fanvue'
        and oauth.provider_account_id is not null
        and length(btrim(oauth.provider_account_id)) > 0
        and oauth.connection_status = 'CONNECTED'
    ) then raise exception 'FANVUE_OAUTH_DESTINATION_NOT_CONNECTED'; end if;
  elsif v_account.verification_status is distinct from 'verified' then
