...
export type FanvueProviderContentType = "text" | "media"
export type FanvueProviderStatusClass = "not_attempted" | "2xx" | "4xx" | "5xx" | "unknown"
export type FanvueProviderReadinessStatusClass = FanvueProviderStatusClass | "timeout" | "processing" | "not_ready" | "other"
export type FanvueProviderReadinessFinalState = "ready" | "processing" | "error" | "timeout" | "unknown"
export type FanvueProviderRefreshStatusClass = FanvueProviderStatusClass | "blocked"

export type FanvueProviderAccount = {
  user_id?: string | null
...
export type FanvueProviderApprovedMedia = {
  filename: string
...
export type FanvueProviderApprovedContent = {
  platform: "fanvue"
  content_type: FanvueProviderContentType
  text?: string | null
  media?: FanvueProviderApprovedMedia | null
}

export type FanvueProviderPostInput = {
  userId: string
  account: FanvueProviderAccount | null
  content: FanvueProviderApprovedContent
  apiBaseUrl: string
  apiVersion: string
  ...
  refreshAccessToken?: (account: FanvueProviderAccount) => Promise<FanvueTokenRefreshResult>
...
export type FanvueProviderPostResult = {
  ...
  content_type: FanvueProviderContentType | null
  text_present: boolean
  media_asset_present: boolean
  token_refresh_attempted: boolean
  token_refresh_status_class: FanvueProviderRefreshStatusClass
  upload_attempted: boolean
  upload_session_status_class: FanvueProviderStatusClass
  signed_url_status_class: FanvueProviderStatusClass
  byte_upload_status_class: FanvueProviderStatusClass
  finalize_status_class: FanvueProviderStatusClass
  readiness_checked: boolean
  readiness_ready: boolean
  readiness_status_class: FanvueProviderReadinessStatusClass | null
  readiness_attempts_used: number | null
  readiness_final_state: FanvueProviderReadinessFinalState | null
  create_attempted: boolean
  create_status_class: FanvueProviderStatusClass
...
function tokenFresh(account: FanvueProviderAccount, now: Date) {
  if (!account.token_expires_at) return false
  const expires = Date.parse(account.token_expires_at)
  return Number.isFinite(expires) && expires > now.getTime() + 60_000
}

export function fanvueProviderBaseResult(overrides: Partial<FanvueProviderPostResult> = {}): FanvueProviderPostResult {
  return {
    ok: false,
    safe_code: "FANVUE_EXECUTION_NOT_ATTEMPTED",
    platform: "fanvue",
...
function inferContentType(media: FanvueProviderApprovedMedia) {
...
function inferByteSize(media: FanvueProviderApprovedMedia) {
...
function readinessConfig(mediaType: FanvueProviderApprovedMedia["mediaType"]) {
...
function readinessFailureStatusClass(failure: FanvueApiFailure): FanvueProviderReadinessStatusClass {
...
function readinessFailureFinalState(failure: FanvueApiFailure): FanvueProviderReadinessFinalState {
  if (failure.error_code === "FANVUE_MEDIA_READY_TIMEOUT") return "timeout"
...
async function resolveCreatorUuid(input: Pick<FanvueProviderPostInput, "apiBaseUrl" | "apiVersion" | "fetchIdentity">, accessToken: string) {
  ...
  try {
    ...
    return { ok: false as const, safe_code: "FANVUE_EXECUTION_CREATOR_IDENTITY_NETWORK_FAILED" }
  }
  if (!response.ok) return { ok: false as const, safe_code: "FANVUE_EXECUTION_CREATOR_IDENTITY_PROVIDER_FAILED" }
...
function validateContent(content: FanvueProviderApprovedContent) {
  if (content.platform !== "fanvue") return { ok: false as const, safe_code: "FANVUE_EXECUTION_PLATFORM_INVALID" }
  if (content.content_type !== "text" && content.content_type !== "media") return { ok: false as const, safe_code: "FANVUE_EXECUTION_CONTENT_TYPE_UNSUPPORTED" }
  const text = clean(content.text)
  if (content.content_type === "text" && !text) return { ok: false as const, safe_code: "FANVUE_EXECUTION_TEXT_REQUIRED" }
  if (text && Array.from(text).length > TEXT_MAX) return { ok: false as const, safe_code: "FANVUE_EXECUTION_TEXT_TOO_LONG" }
  if (content.content_type === "media" && !content.media) return { ok: false as const, safe_code: "FANVUE_EXECUTION_MEDIA_ASSET_REQUIRED" }
  if (content.media) {
    const filename = clean(content.media.filename)
    if (!filename) return { ok: false as const, safe_code: "FANVUE_EXECUTION_MEDIA_FILENAME_REQUIRED" }
    if (content.media.mediaType !== "image" && content.media.mediaType !== "video") return { ok: false as const, safe_code: "FANVUE_EXECUTION_MEDIA_TYPE_UNSUPPORTED" }
  }
  return { ok: true as const, text }
}

export async function executeFanvueProviderPost(input: FanvueProviderPostInput): Promise<FanvueProviderPostResult> {
  ...
  if (!validation.ok) return fanvueProviderBaseResult({ ...contentFlags, safe_code: validation.safe_code })
  ...
  if (!tokenFresh(account, input.now?.() ?? new Date())) {
    ...
    if (!nonEmptyString(account.encrypted_refresh_token)) return fanvueProviderBaseResult({ ...contentFlags, live_attempted: true, token_refresh_attempted: true, token_refresh_status_class: "blocked", safe_code: "FANVUE_REFRESH_TOKEN_MISSING" })
    ...
    if ("blocked" in refreshResult) return fanvueProviderBaseResult({ ...contentFlags, live_attempted: true, token_refresh_attempted: true, token_refresh_status_class: "blocked", safe_code: refreshResult.error_code, safe_error_message: refreshResult.safe_error_message })
    ...
    if (input.reloadAccountAfterRefresh) {
      ...
      if (!account || account.user_id !== input.userId || account.platform !== "fanvue" || account.connection_status !== "CONNECTED" || !nonEmptyString(account.encrypted_access_token) || !tokenFresh(account, input.now?.() ?? new Date())) {
        return fanvueProviderBaseResult({ ...contentFlags, live_attempted: true, token_refresh_attempted: true, token_refresh_status_class: refreshStatusClass, supabase_mutated: supabaseMutated, safe_code: "FANVUE_TOKEN_REFRESH_STALE" })
      }
    }
  }
  ...
  try {
    ...
    return fanvueProviderBaseResult({ ...contentFlags, live_attempted: true, token_refresh_attempted: refreshAttempted, token_refresh_status_class: refreshStatusClass, supabase_mutated: supabaseMutated, safe_code: "FANVUE_EXECUTION_TOKEN_DECRYPT_FAILED" })
  }
  ...
  if (!creator.ok) return fanvueProviderBaseResult({ ...contentFlags, live_attempted: true, token_refresh_attempted: refreshAttempted, token_refresh_status_class: refreshStatusClass, supabase_mutated: supabaseMutated, safe_code: creator.safe_code })
  ...
  if (!session.ok) { const failure = session as FanvueApiFailure; return fanvueProviderBaseResult({ ...uploadFlags, upload_session_status_class: statusClass(failure.status), safe_code: failure.error_code, safe_error_message: failure.safe_error_message }) }
  ...
  if (!signed.ok) { const failure = signed as FanvueApiFailure; return fanvueProviderBaseResult({ ...uploadFlags, upload_session_status_class: "2xx", signed_url_status_class: statusClass(failure.status), safe_code: failure.error_code, safe_error_message: failure.safe_error_message }) }
  ...
  if (!byteUpload.ok) { const failure = byteUpload as FanvueApiFailure; return fanvueProviderBaseResult({ ...uploadFlags, upload_session_status_class: "2xx", signed_url_status_class: "2xx", byte_upload_status_class: statusClass(failure.status), safe_code: failure.error_code, safe_error_message: failure.safe_error_message }) }
  ...
  if (!finalized.ok) { const failure = finalized as FanvueApiFailure; return fanvueProviderBaseResult({ ...uploadFlags, upload_session_status_class: "2xx", signed_url_status_class: "2xx", byte_upload_status_class: "2xx", finalize_status_class: statusClass(failure.status), safe_code: failure.error_code, safe_error_message: failure.safe_error_message }) }
...
export function redactFanvueProviderPostResult(result: FanvueProviderPostResult): Omit<FanvueProviderPostResult, "provider_post_uuid" | "safe_error_message"> {
