...
export const FANVUE_INTERNAL_VIDEO_PROOF_UPLOAD_OPERATION = "fanvue_internal_video_proof_upload_asset_r2_supabase_only" as const
export const FANVUE_INTERNAL_VIDEO_PROOF_UPLOAD_CONFIRMATION = "UPLOAD_ONE_REAL_SERVER_OWNED_MP4_TO_R2_FOR_FANVUE_INTERNAL_VIDEO_PROOF_ONLY" as const
export const FANVUE_INTERNAL_VIDEO_PROOF_UPLOAD_ROUTE = "/api/admin/autopost/fanvue/internal-video-proof-upload" as const
export const FANVUE_INTERNAL_VIDEO_PROOF_UPLOAD_ASSET_PROFILE = "fanvue_internal_video_proof_upload_real_mp4_v1" as const
export const FANVUE_INTERNAL_VIDEO_PROOF_UPLOAD_MAX_BYTES = 100 * 1024 * 1024
...
export function buildFanvueInternalVideoProofUploadR2Key(userId: string, filename: string, now = Date.now()) { const safeName = filename.toLowerCase().replace(/[^a-z0-9._-]+/g, "-").replace(/^-+|-+$/g, "") || "proof.mp4"; return `fanvue/internal-video-proof-uploads/${userId}/${now}-${safeName}` }
...
export function buildFanvueInternalVideoProofUploadMetadata(source: Omit<FanvueInternalVideoProofSource, "metadata">) { return { engine: "admin_internal_upload", kind: "video", mode: source.mode, placeholder: false, test: false, unsafe: false, asset_profile: source.assetProfile, source: source.source, filename: source.filename, content_type: source.contentType, byte_size: source.body.length, fanvue_upload_attempted: false, fanvue_post_attempted: false, dispatch_attempted: false, schedule_attempted: false } }
...
export function buildFanvueInternalVideoProofContentPayload(generationId: string, caption: string = FANVUE_INTERNAL_VIDEO_PROOF_SEED_CAPTION) { return { platform: "fanvue", content_type: "media_video", text: caption, source_asset_ids: [generationId] } }
...
function isMatchingGeneration(row: FanvueInternalVideoProofSeedGenerationRow, userId: string, r2Key: string, assetProfile: string = FANVUE_INTERNAL_VIDEO_PROOF_SEED_ASSET_PROFILE) { const metadata = asRecord(row.metadata); return row.user_id === userId && row.status === "completed" && (metadata?.asset_profile === assetProfile || row.r2_key === r2Key) }
...
async function findExistingGenerations(dependencies: FanvueInternalVideoProofSeedDependencies, userId: string, r2Key: string, assetProfile: string = FANVUE_INTERNAL_VIDEO_PROOF_SEED_ASSET_PROFILE) { const query = dependencies.supabaseAdmin.from("generations").select("id,user_id,status,job_type,mode,metadata,r2_bucket,r2_key").eq("user_id", userId).eq("status", "completed").or(`metadata->>asset_profile.eq.${assetProfile},r2_key.eq.${r2Key}`); const { data, error } = await query; if (error) throw error; return (Array.isArray(data) ? data : []).filter((row) => isMatchingGeneration(row, userId, r2Key, assetProfile)) }
...
export async function createOrReuseFanvueInternalVideoProofSeedAsset(input: { userId: string; source?: FanvueInternalVideoProofSource }, dependencies: FanvueInternalVideoProofSeedDependencies): Promise<FanvueInternalVideoProofSeedResult> {
  ...
  const source: FanvueInternalVideoProofSource = input.source ?? { body: FANVUE_INTERNAL_VIDEO_PROOF_SEED_MP4, filename: FANVUE_INTERNAL_VIDEO_PROOF_SEED_FILENAME, contentType: FANVUE_INTERNAL_VIDEO_PROOF_SEED_CONTENT_TYPE, assetProfile: FANVUE_INTERNAL_VIDEO_PROOF_SEED_ASSET_PROFILE, source: "server_bundled_safe_tiny_video_mp4", prompt: "Fanvue internal video proof seed: harmless tiny MP4", mode: FANVUE_INTERNAL_VIDEO_PROOF_SEED_MODE, r2Key: buildFanvueInternalVideoProofSeedR2Key(userId), jobPayloadSource: "fanvue_internal_video_proof_seed", caption: FANVUE_INTERNAL_VIDEO_PROOF_SEED_CAPTION, metadata: buildFanvueInternalVideoProofSeedMetadata() }
  ...
  try {
    const existing = await findExistingGenerations(dependencies, userId, r2Key, source.assetProfile)
  ...
  if (!generation) {
    try { await (dependencies.r2PutObject ?? defaultR2PutObject)({ bucket, key: r2Key, body: source.body, contentType: source.contentType }); r2Uploaded = true } catch { return baseResult({ safe_code: "FANVUE_INTERNAL_VIDEO_PROOF_SEED_UPLOAD_FAILED" }) }
    const payload = { user_id: userId, prompt: source.prompt, image_url: null, job_type: "video", body_type: "none", mode: source.mode, status: "completed", completed_at: now, metadata: source.metadata, r2_bucket: bucket, r2_key: r2Key, updated_at: now }
    try { const { data, error } = await dependencies.supabaseAdmin.from("generations").insert(payload).select("id").single(); if (error) throw error; generation = { id: clean(data?.id), user_id: userId, status: "completed", job_type: "video", mode: source.mode, metadata: payload.metadata, r2_bucket: bucket, r2_key: r2Key }; generationInserted = true } catch { return baseResult({ safe_code: "FANVUE_INTERNAL_VIDEO_PROOF_SEED_INSERT_FAILED", r2_object_present: true, r2_uploaded: r2Uploaded }) }
  }
...
function uploadDenied(safeCode: FanvueInternalVideoProofSeedSafeCode) { return baseResult({ safe_code: safeCode }) }
function formValue(value: FormDataEntryValue | null) { return typeof value === "string" ? value.trim() : "" }
async function validateFanvueInternalVideoProofUploadForm(form: FormData, userId: string): Promise<{ ok: true; source: FanvueInternalVideoProofSource } | { ok: false; safe_code: FanvueInternalVideoProofSeedSafeCode }> {
  for (const key of form.keys()) if (FORBIDDEN_BODY_FIELDS.has(key) && key !== "file") return { ok: false, safe_code: "CALLER_SUPPLIED_FORBIDDEN_FIELD" }
  if (formValue(form.get("operation")) !== FANVUE_INTERNAL_VIDEO_PROOF_UPLOAD_OPERATION) return { ok: false, safe_code: "INVALID_OPERATION" }
  if (formValue(form.get("confirm")) !== FANVUE_INTERNAL_VIDEO_PROOF_UPLOAD_CONFIRMATION) return { ok: false, safe_code: "INVALID_CONFIRMATION" }
  const files = form.getAll("file").filter((entry): entry is File => typeof entry !== "string")
  if (files.length === 0) return { ok: false, safe_code: "FILE_REQUIRED" }
  if (files.length > 1) return { ok: false, safe_code: "MULTIPLE_FILES_REJECTED" }
  const file = files[0]
  if (file.type !== FANVUE_INTERNAL_VIDEO_PROOF_SEED_CONTENT_TYPE) return { ok: false, safe_code: "INVALID_CONTENT_TYPE" }
  if (!file.name.toLowerCase().endsWith(".mp4")) return { ok: false, safe_code: "INVALID_FILE_EXTENSION" }
  if (file.size <= 0) return { ok: false, safe_code: "FILE_EMPTY" }
  if (file.size > FANVUE_INTERNAL_VIDEO_PROOF_UPLOAD_MAX_BYTES) return { ok: false, safe_code: "FILE_TOO_LARGE" }
  const body = Buffer.from(await file.arrayBuffer())
  if (body.length <= 0) return { ok: false, safe_code: "FILE_EMPTY" }
  if (body.length > FANVUE_INTERNAL_VIDEO_PROOF_UPLOAD_MAX_BYTES) return { ok: false, safe_code: "FILE_TOO_LARGE" }
  const sourceWithoutMetadata = { body, filename: file.name, contentType: FANVUE_INTERNAL_VIDEO_PROOF_SEED_CONTENT_TYPE, assetProfile: FANVUE_INTERNAL_VIDEO_PROOF_UPLOAD_ASSET_PROFILE, source: "server_admin_uploaded_real_video_mp4", prompt: "Fanvue internal video proof upload: server-owned real MP4", mode: "fanvue_internal_video_proof_upload", r2Key: buildFanvueInternalVideoProofUploadR2Key(userId, file.name), jobPayloadSource: "fanvue_internal_video_proof_upload", caption: "Fanvue internal controlled video proof: server-owned MP4 test video." }
  return { ok: true, source: { ...sourceWithoutMetadata, metadata: buildFanvueInternalVideoProofUploadMetadata(sourceWithoutMetadata) } }
}
export type FanvueInternalVideoProofUploadRouteDependencies = Omit<FanvueInternalVideoProofSeedRouteDependencies, "createSeedAsset"> & { createProofAsset: (input: { userId: string; source: FanvueInternalVideoProofSource }) => Promise<FanvueInternalVideoProofSeedResult> }
export async function handleFanvueInternalVideoProofUploadRoute(dependencies: FanvueInternalVideoProofUploadRouteDependencies): Promise<{ status: number; body: FanvueInternalVideoProofSeedResult }> { if (dependencies.request.method.toUpperCase() !== "POST") return { status: 405, body: baseResult({ safe_code: "METHOD_NOT_ALLOWED" }) }; const auth = await authorizeFanvueUploadDiagnosticRequest({ request: dependencies.request, expectedSecret: dependencies.expectedSecret, adminUserIds: dependencies.adminUserIds, getAuthenticatedUserId: dependencies.getAuthenticatedUserId }); if (auth.ok === false) return { status: auth.status, body: baseResult({ safe_code: auth.error_code }) }; let form: FormData; try { form = await dependencies.request.formData() } catch { return { status: 400, body: uploadDenied("INVALID_BODY") } } const validation = await validateFanvueInternalVideoProofUploadForm(form, auth.adminUserId); if (validation.ok === false) return { status: 400, body: uploadDenied(validation.safe_code) }; const result = await dependencies.createProofAsset({ userId: auth.adminUserId, source: validation.source }); return { status: 200, body: result } }
