...
export async function createOrReuseFanvueInternalVideoProofSeedAsset(input: { userId: string }, dependencies: FanvueInternalVideoProofSeedDependencies): Promise<FanvueInternalVideoProofSeedResult> {
  ...
  if (!generation) {
    ...
    const payload = { user_id: userId, prompt: "Fanvue internal video proof seed: harmless tiny MP4", image_url: null, video_url: null, output_url: null, job_type: "video", body_type: "none", mode: FANVUE_INTERNAL_VIDEO_PROOF_SEED_MODE, status: "completed", completed_at: now, metadata: buildFanvueInternalVideoProofSeedMetadata(), r2_bucket: bucket, r2_key: r2Key, updated_at: now }
