...
export async function runPhase8dCanceledAccountEnforcement(): Promise<Phase8dRunResult> {
  ...
  for (const claim of rows(claims.data)) {
    ...
    const validation = await admin.rpc("phase8d_validate_canceled_account_purge", {
    ...
    if (validationRow?.allowed !== true) {
      if (validationRow?.retention_state === "superseded") result.accountsSuperseded += 1;
    ...
    const finalValidation = await admin.rpc("phase8d_validate_canceled_account_purge", {
      p_retention_id: retentionId,
      p_auth_user_id: authUserId,
      p_claim_token: claimToken,
    });
    ...
    if (finalValidationRow?.allowed !== true) {
      if (finalValidationRow?.retention_state === "superseded") result.accountsSuperseded += 1;
      else result.accountsHeld += 1;
      continue;
    }
