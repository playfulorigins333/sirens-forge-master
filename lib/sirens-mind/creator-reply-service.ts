...
export async function createSubscriber(userId:string,input:Record<string,unknown>) {
  const w=await resolveCreatorReplyWorkspace(userId), db=getSupabaseAdmin(), sid=randomUUID(), cid=randomUUID(), threadId=randomUUID(), keyVersion=creatorReplyDataKeyVersion()
  ...
  const {error}=await db.rpc("creator_reply_create_subscriber",{p_workspace_id:w,p_user_id:userId,p_subscriber_id:sid,p_conversation_id:cid,p_thread_id:threadId,p_display_name:display_name,p_platform:platform,p_platform_handle:platform_handle,p_notes_ciphertext:notes_ciphertext,p_notes_key_version:notes?keyVersion:null,p_checkpoint_ciphertext:checkpoint_ciphertext,p_checkpoint_key_version:keyVersion})
...
export async function conversations(userId:string,subscriberId:string){const {w,db}=await ownedSubscriber(userId,subscriberId);const {data,error}=await db.from("sirens_mind_creator_reply_conversations").select("id,status,last_used_at,updated_at,checkpoint_ciphertext,checkpoint_key_version").eq("workspace_id",w).eq("subscriber_id",subscriberId).order("updated_at",{ascending:false});if(error)throw error;return (data||[]).map(c=>{assertCreatorReplyKeyVersion(c.checkpoint_key_version);const p=parseCreatorReplyCheckpoint(JSON.parse(decryptCreatorReplyData(c.checkpoint_ciphertext,conversationCheckpointAad(w,c.id))));if(!p)throw new Error("CHECKPOINT_INVALID");return{id:c.id,status:c.status,last_used_at:c.last_used_at,updated_at:c.updated_at,label:p.label||`Conversation • ${new Date(c.updated_at).toLocaleDateString()}`}})}
export async function newConversation(userId:string,subscriberId:string){const {w,db}=await ownedSubscriber(userId,subscriberId),id=randomUUID(),threadId=randomUUID(),v=creatorReplyDataKeyVersion();const {error}=await db.rpc("creator_reply_new_conversation",{p_workspace_id:w,p_user_id:userId,p_subscriber_id:subscriberId,p_conversation_id:id,p_thread_id:threadId,p_checkpoint_ciphertext:checkpoint(w,id,emptyCreatorReplyCheckpoint()),p_checkpoint_key_version:v});if(error)throw error;return{id,status:"active",label:""}}
export async function resumeConversation(userId:string,id:string){const w=await resolveCreatorReplyWorkspace(userId),db=getSupabaseAdmin();const {data}=await db.from("sirens_mind_creator_reply_conversations").select("subscriber_id,checkpoint_ciphertext,checkpoint_key_version").eq("workspace_id",w).eq("id",id).maybeSingle();if(!data)throw new Error("NOT_FOUND");assertCreatorReplyKeyVersion(data.checkpoint_key_version);const p=parseCreatorReplyCheckpoint(JSON.parse(decryptCreatorReplyData(data.checkpoint_ciphertext,conversationCheckpointAad(w,id))));if(!p)throw new Error("CHECKPOINT_INVALID");const {data:subscriberId,error}=await db.rpc("creator_reply_resume_conversation",{p_workspace_id:w,p_user_id:userId,p_conversation_id:id});if(error||!subscriberId)throw error||new Error("NOT_FOUND");return{id,subscriber_id:data.subscriber_id,status:"active",label:p.label||"Conversation"}}
...
export async function renameConversation(userId:string,id:string,labelValue:unknown){const label=clean(labelValue,160),w=await resolveCreatorReplyWorkspace(userId),db=getSupabaseAdmin();const {data,error}=await db.from("sirens_mind_creator_reply_conversations").select("checkpoint_ciphertext,checkpoint_key_version,checkpoint_revision,status").eq("workspace_id",w).eq("id",id).in("status",["active","paused"]).maybeSingle();if(error)throw error;if(!data)throw new Error("NOT_FOUND");assertCreatorReplyKeyVersion(data.checkpoint_key_version);const parsed=parseCreatorReplyCheckpoint(JSON.parse(decryptCreatorReplyData(data.checkpoint_ciphertext,conversationCheckpointAad(w,id))));if(!parsed)throw new Error("CHECKPOINT_INVALID");const {data:updated,error:updateError}=await db.from("sirens_mind_creator_reply_conversations").update({checkpoint_ciphertext:checkpoint(w,id,{...parsed,label}),checkpoint_key_version:creatorReplyDataKeyVersion(),checkpoint_revision:Number(data.checkpoint_revision)+1,updated_at:new Date().toISOString()}).eq("workspace_id",w).eq("id",id).eq("checkpoint_revision",data.checkpoint_revision).in("status",["active","paused"]).select("id").maybeSingle();if(updateError)throw updateError;if(!updated)throw new Error("CONFLICT");return{id,label}}
...
export async function loadCreatorReplyAuthority(userId:string,subscriberId:string,conversationId:string){
 ...
 const {data:conversation,error}=await db.from("sirens_mind_creator_reply_conversations").select("id,thread_id,checkpoint_ciphertext,checkpoint_key_version,checkpoint_revision,status").eq("workspace_id",w).eq("subscriber_id",subscriberId).eq("id",conversationId).eq("status","active").maybeSingle()
