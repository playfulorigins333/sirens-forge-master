...
export async function listOwnCases(before:string|null,limit:number){const {data,error}=await (await supabaseServer()).rpc("list_own_support_cases",{p_before:before,p_limit:limit});return error?null:data??[]}
export async function listAdminCases(actorUserId:string,status:string|null,before:string|null,limit:number){const {data,error}=await getSupabaseAdmin().rpc("list_admin_support_cases",{p_actor_user_id:actorUserId,p_status:status,p_before:before,p_limit:limit});return error?null:data??[]}
