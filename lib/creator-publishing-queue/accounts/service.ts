...
export async function loadCreatorPlatformAccounts(deps: Pick<AccountDeps,"getAuthenticatedUserId"|"getAdminClient"> = defaultDeps) { const creatorId = await deps.getAuthenticatedUserId(); if (!creatorId) redirect("/login"); const { data, error } = await deps.getAdminClient().from("creator_platform_accounts").select("id,platform,platform_username,profile_url,is_virtual_entity,verification_status,verification_attested_at,verification_reviewed_at,oauth_account_id,autopost_accounts!creator_platform_accounts_oauth_owner_fk(id,connection_status),created_at,updated_at").eq("creator_id", creatorId).in("platform", ["onlyfans", "fansly", "fanvue"]).order("platform", { ascending: true }).order("platform_username", { ascending: true }); if (error) throw new Error("Creator platform accounts could not be loaded."); return (data ?? []).map(mapPlatformAccountRow) }
