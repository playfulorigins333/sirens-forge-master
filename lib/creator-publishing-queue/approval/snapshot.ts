...
export function buildCreatorPublishingApprovalSnapshot(pkg: CreatorPublishingPackageForApproval, mediaAssets: readonly CreatorPublishingMediaAssetForApproval[], reviewSummary: Record<string, unknown> | null = null): { snapshot: CreatorPublishingApprovalSnapshot; hash: string } {
  ...
  const finalCaption = [pkg.forced_disclosure_text?.trim(), pkg.caption_body?.trim()].filter(Boolean).join(pkg.forced_disclosure_text && pkg.caption_body ? "\n\n" : "")
  const snapshot: CreatorPublishingApprovalSnapshot = {
    ...
    media_assets: [...mediaAssets].sort((a,b) => a.id.localeCompare(b.id)).map((asset) => ({ id: asset.id, storage_key: asset.storage_key, mime_type: asset.mime_type, sha256: asset.sha256, source: asset.source, ai_generation_metadata: asset.ai_generation_metadata ?? {} })),
