...
export function validateFanvueExecutionFacts(f:any){
 ...
 if(f.oauth.id!==f.job.oauth_account_id||f.oauth.user_id!==f.job.creator_id||f.oauth.platform!=="fanvue"||f.oauth.connection_status!=="CONNECTED"||!nonblank(f.oauth.provider_account_id))return"FANVUE_CPQ_OAUTH_INVALID"
