...
type Query = { select: (c?: string) => Query; eq: (c: string, v: unknown) => Query; single: () => DbResult; maybeSingle?: () => DbResult; insert: (p: unknown) => Query; update: (p: unknown) => Query; limit?: (n: number) => Query; order?: (c: string, o?: unknown) => Query; neq?: (c: string, v: unknown) => Query }
...
export type CreatorPublishingReviewRowForEligibility = Readonly<{ outcome: string; review_source?: string | null; created_at?: string | null }>

export function validateCreatorPublishingPackageForTrustedReview(pkg: CreatorPublishingPackageForReview | null | undefined, input: CreatorPublishingTrustedReviewInput, latestAutomatedReview?: CreatorPublishingReviewRowForEligibility | null, laterBlockingReview?: CreatorPublishingReviewRowForEligibility | null) {
  ...
  if (!latestAutomatedReview || latestAutomatedReview.review_source !== "automated" || latestAutomatedReview.outcome !== "manual_review") throw new CreatorPublishingReviewError("REVIEW_AUTOMATED_REVIEW_REQUIRED", "Latest automated compliance review must be manual_review.")
  if (laterBlockingReview) throw new CreatorPublishingReviewError("REVIEW_BLOCKED_NOT_ESCALATABLE", "A later blocking review prevents escalation.")
  return pkg
}
...
export async function performTrustedCreatorPublishingReview(input: CreatorPublishingTrustedReviewInput, deps: { supabaseAdmin?: CreatorPublishingReviewDb; reviewerAuthorization?: CreatorPublishingReviewerAuthorization; now?: () => string } = {}): Promise<CreatorPublishingTrustedReviewResult> {
  ...
  const reviewerAuth = assertTrustedCreatorPublishingReviewer(deps.reviewerAuthorization)
  if (reviewerAuth.reviewer_id !== input.reviewer_id) throw new CreatorPublishingReviewError("REVIEW_IDENTITY_MISMATCH", "Reviewer identity does not match the authenticated reviewer.")
  const rawPkg = await must<CreatorPublishingPackageForReview>(db.from("creator_publishing_content_packages").select("id,creator_id,target_platform,compliance_status,compliance_policy_version,forced_disclosure_text,creator_approval_status,creator_approved_at,creator_approved_by,updated_at").eq("id", input.content_package_id).single())
  const latestAutomatedReview = await must<CreatorPublishingReviewRowForEligibility>(db.from("creator_publishing_compliance_reviews").select("outcome,review_source,created_at").eq("content_package_id", input.content_package_id).eq("review_source", "automated").order?.("created_at", { ascending: false }).limit?.(1).maybeSingle?.() ?? Promise.resolve({ data: null, error: null }))
  const laterBlockingReview = latestAutomatedReview?.created_at ? await must<CreatorPublishingReviewRowForEligibility>(db.from("creator_publishing_compliance_reviews").select("outcome,review_source,created_at").eq("content_package_id", input.content_package_id).eq("outcome", "block").order?.("created_at", { ascending: false }).limit?.(1).maybeSingle?.() ?? Promise.resolve({ data: null, error: null })) : null
  const pkg = validateCreatorPublishingPackageForTrustedReview(rawPkg, input, latestAutomatedReview, laterBlockingReview && latestAutomatedReview?.created_at && laterBlockingReview.created_at && laterBlockingReview.created_at > latestAutomatedReview.created_at ? laterBlockingReview : null)
  assertTrustedCreatorPublishingReviewer(reviewerAuth, pkg)
  if (db.rpc) {
    const rpcResult = await must<CreatorPublishingTrustedReviewResult>(db.rpc("creator_publishing_apply_manual_review_decision", {
      ...
      p_rule_hits: input.confirmed_rule_hits ?? [], p_review_metadata: { reviewer_evidence: input.reviewer_evidence ?? null, rule_hit_references: input.rule_hit_references ?? [] }, p_idempotency_key: input.idempotency_key ?? null,
    }))
    return rpcResult
  }
