...
export function parseTrustedFactsRpcResponse(data: unknown, expectedCreatorId: string): TrustedFactsRpcResponse {
  ...
  const lock = obj(f.human_review_lock ?? { locked: false, reason: null, latest_review_id: null, latest_review_outcome: null, latest_review_created_at: null })
  const facts = { schema_version: TRUSTED_FACTS_SCHEMA_VERSION, package: pkg, platform_account: account, creator_verification, ai_twin_consent, media_manifest, generation_manifest, co_performer_summary: { record_count: nonNegativeInt(co.record_count), all_platform_release_confirmed: bool(co, "all_platform_release_confirmed") }, active_queue_task: bool(f, "active_queue_task"), human_review_lock: { locked: bool(lock, "locked"), reason: str(lock, "reason", true), latest_review_id: str(lock, "latest_review_id", true), latest_review_outcome: str(lock, "latest_review_outcome", true), latest_review_created_at: ts(lock.latest_review_created_at, true) } } satisfies TrustedComplianceFacts
  if (facts.active_queue_task) throw new Error("locked")
  if (facts.human_review_lock.locked) throw new Error("human review locked")
