...
export type ComposerEligibleAccount = { id:string; platform:"onlyfans"|"fansly"; platform_username:string; verification_status:"creator_attested" }
export type ComposerEditablePackage = { id:string; platform_account_id:string; target_platform:"onlyfans"|"fansly"; title:string; caption_body:string; second_person_present:boolean; price_notes:string|null; visibility_notes:string|null; updated_at:string; locked:boolean; lockedReason:string|null }
export async function loadCreatorPublishingComposerAccounts(): Promise<ComposerEligibleAccount[]> { const creatorId = await currentCreatorId(); const admin=getSupabaseAdmin(); const {data,error}=await admin.from("creator_platform_accounts").select("id,platform,platform_username,verification_status").eq("creator_id", creatorId).in("platform", ["onlyfans","fansly"]).eq("verification_status", "creator_attested").order("platform", {ascending:true}).order("platform_username", {ascending:true}); if(error) throw new Error("Eligible platform accounts could not be loaded."); return (data??[]) as any }
export async function loadCreatorPublishingEditablePackage(contentPackageId: string): Promise<ComposerEditablePackage> { const creatorId=await currentCreatorId(); const admin=getSupabaseAdmin(); const {data:pkg,error}=await admin.from("creator_publishing_content_packages").select("id,creator_id,platform_account_id,target_platform,title,caption_body,second_person_present,price_notes,visibility_notes,creator_approval_status,updated_at").eq("id", contentPackageId).eq("creator_id", creatorId).neq("target_platform", "fanvue").maybeSingle(); if(error) throw new Error("Publishing package could not be loaded."); if(!pkg) notFound(); const {data:task,error:taskError}=await admin.from("creator_publishing_queue_tasks").select("id,status").eq("content_package_id", contentPackageId).neq("status", "archived").maybeSingle(); if(taskError) throw new Error("Publishing package lock state could not be loaded."); const locked=(pkg as any).creator_approval_status === "approved" || Boolean(task); return { id:(pkg as any).id, platform_account_id:(pkg as any).platform_account_id, target_platform:(pkg as any).target_platform, title:(pkg as any).title, caption_body:(pkg as any).caption_body ?? "", second_person_present:Boolean((pkg as any).second_person_present), price_notes:(pkg as any).price_notes ?? null, visibility_notes:(pkg as any).visibility_notes ?? null, updated_at:(pkg as any).updated_at, locked, lockedReason: (pkg as any).creator_approval_status === "approved" ? "Approved packages cannot be edited." : task ? "This package has an active publishing task and cannot be edited." : null } }
