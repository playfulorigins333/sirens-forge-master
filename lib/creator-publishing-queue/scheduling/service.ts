...
import { scheduleFanvueDirectPlanCore } from "./fanvueDirect"
...
export async function scheduleCreatorPublishingPlan(input:unknown):Promise<SafeMutationResult>{
  ...
  if(typeof platformJobId==="string"){
    try{
      const {data}=await getSupabaseAdmin().from("creator_publishing_platform_jobs").select("target_platform").eq("id",platformJobId).maybeSingle()
      if(data?.target_platform==="fanvue") return scheduleFanvueDirectPlanCore(input,deps)
    }catch{}
  }
