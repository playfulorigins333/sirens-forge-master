...
export async function collectOnlyFansHistoryRows(admin: any, job: any): Promise<OnlyFansHistoryRows> {
  const schedulerEvents = await many(
    admin
      ...
      .select("id,platform_job_id,status,created_at,updated_at,processed_at,blocked_at,superseded_at")
  ...
  const idempotencyRows = await many(
    admin
      .from("creator_publishing_operator_action_idempotency")
      .select("actor_id,queue_task_id,platform_job_id,action_type,idempotency_key,internal_request_snapshot,created_at")
      .eq("platform_job_id", job.id)
      .eq("action_type", "manual_completion")
      .order("created_at", { ascending: false }),
  )

  const links = { auditEvents: jobAuditEvents, evidenceIntents, idempotencyRows }
...
export async function loadOperatorOnlyFansTerminalHistory() {
  ...
  try {
    ...
    const creators = [actor, ...auths.map((authorization: any) => authorization.creator_id)]
    const jobs = await many(
      admin
        ...
        .eq("target_platform", "onlyfans")
        .eq("publishing_mode", "assisted")
        .in("job_state", ["confirmed_posted_manual", "failed_manual_upload", "skipped", "blocked", "platform_rejected", "archived"])
