...
async function loadDiscoveryPage(admin: TrustedAdminClient, table: "creator_publishing_content_packages" | "creator_platform_accounts", platformColumn: "target_platform" | "platform", from: number, to: number) {
  ...
  return admin.from("creator_platform_accounts").select("id,creator_id").in("platform", [...CREATOR_IDENTITY_DISCOVERY_PLATFORMS]).not("creator_id", "is", null).order("creator_id", { ascending: true }).order("id", { ascending: true }).range(from, to)
}
