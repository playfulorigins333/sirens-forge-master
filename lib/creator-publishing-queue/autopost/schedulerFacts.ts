...
function schedulerInputError(code:string){return Object.assign(new Error(code),{code})}
export function normalizeExpectedRevisionMap(value:unknown,targetJobIds:string[],required:boolean){ if(value==null){ if(required) throw schedulerInputError("EXPECTED_REVISIONS_REQUIRED"); return {} as Record<string,number> } if(typeof value!=="object"||Array.isArray(value)) throw schedulerInputError("EXPECTED_REVISIONS_REQUIRED"); const out:Record<string,number>={}; const keys=Object.keys(value as Record<string,unknown>); const targets=new Set(targetJobIds); for(const k of keys){ if(!targets.has(k)) throw schedulerInputError("UNEXPECTED_REVISION_JOB"); const n=(value as Record<string,unknown>)[k]; if(!Number.isInteger(n)||Number(n)<0||Number(n)>1_000_000) throw schedulerInputError("INVALID_EXPECTED_REVISION"); out[k]=Number(n) } if(required&&targetJobIds.some(id=>out[id]===undefined)) throw schedulerInputError("EXPECTED_REVISIONS_REQUIRED"); return out }
