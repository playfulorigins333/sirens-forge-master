...
export function isValidIanaTimeZone(tz:unknown):tz is string{ if(typeof tz!=="string"||tz.trim()!==tz||tz.length<3||tz.length>128||/^UTC[+-]|^[+-]\d{2}:?\d{2}$|^Etc\/GMT[+-]/.test(tz)) return false; try{ const zones=(Intl as any).supportedValuesOf?.("timeZone") as string[]|undefined; if(zones?.includes(tz)) return true; return Boolean(new Intl.DateTimeFormat("en-US",{timeZone:tz}).resolvedOptions().timeZone) }catch{return false}}
...
export function validateScheduleInstant(instant:unknown, timeZone:unknown){
  ...
  if(off!=="Z"){
    const mins=(Number(off.slice(1,3))*60+Number(off.slice(4,6)))*(off[0]==="-"?-1:1)
    if(!Number.isFinite(mins)||Math.abs(mins)>14*60) throw Object.assign(new Error("INVALID_RFC3339_OFFSET"),{code:"INVALID_RFC3339_OFFSET"})
  }
