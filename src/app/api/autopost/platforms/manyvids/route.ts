// app/api/autopost/platforms/manyvids/route.ts
import { NextResponse } from "next/server";

/**
 * ============================================================
 * MANYVIDS AUTOPOST ADAPTER
 *
 * Contract:
 * - Called internally by /api/autopost/run
 * - MUST return JSON
 * - MUST return { ok: true, platform_post_id }
 *   OR { ok: false, error_code, error_message }
 *
 * This adapter is LAUNCH-SAFE:
 * - No silent success
 * - No silent failure
...
export async function POST(req: Request) {
  ...
  if (payload.platform !== "manyvids") {
    return json(400, {
      ok: false,
      error_code: "PLATFORM_MISMATCH",
      error_message: "Platform must be 'manyvids'",
    });
  }

  // ──────────────────────────────────────────────
  // MANYVIDS-SPECIFIC VALIDATION (SAFE DEFAULTS)
  // ──────────────────────────────────────────────
  if (typeof payload.explicitness !== "number") {
    return json(400, {
      ok: false,
      error_code: "INVALID_EXPLICITNESS",
      error_message: "explicitness must be a number",
    });
  }
