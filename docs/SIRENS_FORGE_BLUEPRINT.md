...
This architectural overview is reconciled through 2026-08-20. The last externally verified frontend Production baseline before this source revision is PR #265/main SHA `7dfb59209f6f3d2df5b9de2d8d665d7988674a0c`, with Vercel Production deployment `dpl_28Jkau4orQtNLCFZMj2iFRhomoQT` `READY` at that exact SHA. The last independently verified API Production baseline before this change is merged API PR #5/main SHA `b357ff918a30ba4e771b798f591a1611cf8a4d97`. No custom-domain alias or public-response verification was performed in this cleanup. Operational claims explicitly identified as **operator-verified** are supplied current-state evidence. Repository files, tests, migrations, and routes establish implementation evidence only; they do not independently prove Production configuration or an external action. See [`CURRENT_STATE.md`](./CURRENT_STATE.md) and the canonical [`LAUNCH_ROADMAP_STATUS.md`](./LAUNCH_ROADMAP_STATUS.md). Static repository documentation is not self-updating: every merge advances `main`, so this pre-change baseline does not prove current post-merge GitHub/Vercel state. Exact-current proof requires external re-verification, and absence of a newer SHA here does not mean Production remains on the recorded baseline.
...
The separate Railway/FastAPI API is proxy/gateway infrastructure for generation and Dataset Doctor. Its privileged business ingress is server-to-server secret protected; that authorization contract is now inventoried in API PR #4. This does not change the locked Option A architecture: the frontend builds the full Comfy workflow JSON and the API remains proxy/validation/execution/storage gateway infrastructure rather than owning workflow composition. API PR #5 is merged/Production and fails identity-bearing generation closed until worker-side/shared-filesystem identity-LoRA materialization is proven. The frontend verifies owned completed identity metadata, derives the deterministic worker filename, and builds the complete workflow; it does not download or materialize the remote worker artifact, so that cross-host runtime boundary remains unresolved.
...
- **DONE:** launch-wide observability/alerting/recovery has a zero-spend operating contract and CI regression coverage plus a historical read-only Railway/FastAPI audit at `2c84f8620dc626a449740b6e946fef1388605cee`. The last independently verified API Production baseline before this change is `b357ff918a30ba4e771b798f591a1611cf8a4d97`; no custom API health endpoint, universal automated paging platform, or 24/7 staffing is claimed.
