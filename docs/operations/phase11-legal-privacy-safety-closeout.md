...
- The admin queue/detail/chronology/transitions require verified user + fresh TOTP/AAL2 + database capability. Queue metadata contains no reporter narrative, contact email, content URL, or requested action; report details remain available only through the case-specific audited detail read. Case chronology is likewise case-specific and audited. Operators see only valid next states. Closure requires a safe outcome; the full 3–1000-character outcome is stored in its dedicated append-only chronology field and survives reopen, while reopen clears the mutable current closure projection. Queue and newest-first history views use bounded cursor pagination with one-row overfetch and never fetch unbounded case history. There is no deletion, ban, signed URL, private-media browser, Auth mutation, or external-report action.
