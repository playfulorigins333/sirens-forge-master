...
Immediately before transport, a service-role-only RPC atomically revalidates source ownership, exact due-marker identity, and lifecycle state, then either marks that provider delivery has started or durably suppresses newly stale work. An expired lease without that marker is safely requeued; an expired lease after provider attempt start is deliberately terminalized as uncertain. This prevents both late stale delivery and loss of unattempted rows later in an interrupted batch without blindly repeating an uncertain send. Resend receives the stable outbox ID as its idempotency key; only a SHA-256 digest of a successful provider message ID is retained. Explicit SDK `statusCode`/`name` classification retries 408, 425, 429, 5xx, and known transient errors; definite 4xx rejection is safely suppressed as `provider_permanent`; thrown network outcomes remain uncertain.

Phase 9 also bridges the previously producer-less deletion-completion due marker: a trigger sets it exactly once from authoritative `purge_completed_at` on transition, and migration-time repair uses the same durable timestamp for already-completed rows. It changes no deletion deadline, purge authority, or delivery state.
