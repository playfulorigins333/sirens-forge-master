...
- **API authorization and operations audit — cross-repo DONE:** The Next.js frontend inventory covers all 88 current `app/api/**/route.ts` files and 102 exported HTTP route-methods. API PR #5 is operator-confirmed merged/Production at `b357ff918a30ba4e771b798f591a1611cf8a4d97`; its identity-bearing generation boundary fails closed unless worker/shared-filesystem materialization proof is present. The separate Railway/FastAPI repo was previously audited read-only at `2c84f8620dc626a449740b6e946fef1388605cee`: `app/main.py`, exactly 10 business endpoints, public framework docs/schema routes, centralized fail-closed privileged ingress, startup/configuration, generation/Dataset Doctor failures, and logging posture were inspected. No custom API health endpoint and no proven secret leak exist. Railway Production is `SUCCESS` on that exact SHA. The API repository was not modified, and deployment success does not prove generation compute availability.
