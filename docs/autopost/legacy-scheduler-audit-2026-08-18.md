...
The operator-run activation targets only `https://www.sirensforge.vip/api/creator-publishing-queue/fanvue/run`, the route that advances CPQ scheduler state and invokes the CPQ Fanvue worker. It does not call `/api/autopost/run`, read or mutate legacy AutoPost state, or create another publishing state machine. Before scheduling, it fails if a differently named cron already targets the CPQ Fanvue route; rerunning it replaces only its own named job. `vercel.json` remains unchanged, so it does not duplicate the trigger there.

## Activation boundary

The activation SQL under `supabase/manual` is intentionally not a migration. Committing it does not apply it, create Vault secrets, change an environment, or activate Production. The forward migration installs only the required `pg_net` extension. An authorized operator must first re-audit `cron.job`, configure the single `fanvue_cpq_cron_secret` Vault secret, and obtain separate authorization before applying the activation SQL and verifying the resulting job and public route response. The companion deactivation SQL removes only the named CPQ trigger and leaves legacy structures untouched.
