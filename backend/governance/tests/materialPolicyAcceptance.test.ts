...
const normalizedPrivacySource = privacySource.replace(/\s+/g, " ")
assert.match(normalizedTermsSource, /Data Export, Account Deletion, and Legal Holds/)
assert.match(normalizedTermsSource, /current material policy bundle/)
assert.match(normalizedPrivacySource, /30-day Recently Deleted windows/)
assert.match(normalizedPrivacySource, /60-day voluntary account-deletion recovery period/)
assert.match(normalizedPrivacySource, /60-day post-cancellation retention period/)
assert.match(normalizedPrivacySource, /60-day retention period after the second missed recurring subscription payment/)
assert.match(normalizedPrivacySource, /Draft working data uses a 90-day retention rule/)
assert.match(normalizedPrivacySource, /security and governance audit evidence is retained for 12 months/)
assert.match(normalizedPrivacySource, /Governance and Audit Evidence/)
