...
test("desktop reading layout and conversational naming contracts remain explicit", () => {
  ...
  assert.match(input, /textarea\.style\.height = "auto"[\s\S]*Math\.min\([\s\S]*TEXTAREA_MAX_HEIGHT[\s\S]*Math\.max\(TEXTAREA_MIN_HEIGHT, textarea\.scrollHeight\)/)
  assert.match(input, /textarea\.style\.overflowY =[\s\S]*textarea\.scrollHeight > TEXTAREA_MAX_HEIGHT \? "auto" : "hidden"/)
  assert.match(input, /useLayoutEffect\(\(\) => \{[\s\S]*resizeTextarea\(\)[\s\S]*\}, \[resizeTextarea, value\]\)/)
  assert.match(input, /new ResizeObserver\([\s\S]*width === previousWidth[\s\S]*resizeTextarea\(\)/)
  assert.match(input, /if \(!textarea\.value\) textarea\.scrollTop = 0/)
  assert.match(input, /e\.key === "Enter" && !e\.shiftKey && !e\.nativeEvent\.isComposing/)
  assert.match(input, /max-h-48 min-h-\[34px\][^\n]*overflow-y-hidden/)
