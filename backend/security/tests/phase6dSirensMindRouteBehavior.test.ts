...
let providerCalls = 0, providerRequest: any
const logs: string[] = []
...
mock.module(new URL("../../../lib/subscription-checker.ts", import.meta.url).href, { namedExports: { ensureActiveSubscription: async () => ({ ok: true, user: { id: USER } }) } })
...
try {
  ...
  assert.ok(!providerRequest.messages.some((message: any) => message.content.includes("secret")))
