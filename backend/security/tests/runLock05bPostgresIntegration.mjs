...
const tableAcl=()=>psql("select coalesce(grantee.rolname,'PUBLIC'),grantor.rolname,x.privilege_type,x.is_grantable from pg_class c cross join lateral aclexplode(c.relacl) x left join pg_roles grantee on grantee.oid=x.grantee join pg_roles grantor on grantor.oid=x.grantor where c.oid='public.profiles'::regclass order by 1,2,3,4;").stdout.trim();
const attrAcl=()=>psql("select a.attname,coalesce(grantee.rolname,'PUBLIC'),grantor.rolname,x.privilege_type,x.is_grantable from pg_attribute a cross join lateral aclexplode(a.attacl) x left join pg_roles grantee on grantee.oid=x.grantee join pg_roles grantor on grantor.oid=x.grantor where a.attrelid='public.profiles'::regclass order by 1,2,3,4,5;").stdout.trim();
...
assert.equal(data(),beforeData); assert.equal(policy(),beforePolicy); assert.equal(objects(),beforeObjects); assert.match(tableAcl(),/^service_role\|postgres\|SELECT\|f$/m); assert.equal(psql("select count(*),string_agg(a.attname,',' order by a.attname) from pg_attribute a cross join lateral aclexplode(a.attacl) x join pg_roles grantee on grantee.oid=x.grantee join pg_roles grantor on grantor.oid=x.grantor where a.attrelid='public.profiles'::regclass and grantee.rolname='service_role' and grantor.rolname='postgres' and x.privilege_type='UPDATE';").stdout.trim(),'3|stripe_connect_account_id,stripe_connect_onboarded,stripe_customer_id'); assert.equal(psql("select pg_get_userbyid(relowner),relrowsecurity,relforcerowsecurity from pg_class where oid='public.profiles'::regclass;").stdout.trim(),'postgres|t|f'); assert.equal(psql("select has_table_privilege('anon','public.unrelated_control','SELECT');").stdout.trim(),'t');
psql(rollback);
