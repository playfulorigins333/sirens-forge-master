...
assert.equal(data(),beforeData); assert.equal(policy(),beforePolicy); assert.equal(objects(),beforeObjects); assert.match(tableAcl(),/service_role=r/); assert.equal((attrAcl().match(/service_role=w\/postgres/g) ?? []).length, 3); assert.equal(psql("select pg_get_userbyid(relowner)||'|'||relrowsecurity||'|'||relforcerowsecurity from pg_class where oid='public.profiles'::regclass;").stdout.trim(),'postgres|t|f'); assert.equal(psql("select has_table_privilege('anon','public.unrelated_control','SELECT');").stdout.trim(),'t');
psql(rollback);
