...
test("female-subscriber male-Brat-Tamer completion survives chunked protocol parsing without evidence copying", async () => {
  const keyNotes: CreatorReplyAuthoritySource[] = [
    { id: "profile.key_notes", kind: "key_notes", text: "Subscriber is female. Pronouns: she/her." },
  ]
  const visible = "Listen to Sir, good girl. Are you trying to distract me? Keep testing me if you dare, and I'll choose a playful consequence."
  const result = await parseAndValidate(visible, { version: 4, claims: [
    { claim: "good girl", source_id: "profile.key_notes" },
  ] }, keyNotes)
  assert.equal(result.buffered, visible)
  assert.equal(result.validation.ok, true)
  assert.doesNotMatch(visible, /Mommy|Goddess|Findom|tribute|payment|good boy/i)
})
