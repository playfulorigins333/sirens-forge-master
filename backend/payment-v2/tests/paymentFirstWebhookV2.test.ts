...
function harness(overrides: { event?: Partial<StripeEvent>; session?: Partial<StripeSession>; provider?: Partial<PaymentV2Provider>; db?: Partial<PaymentV2Database>; input?: Partial<WebhookInput> } = {}) {
  ...
  const input: WebhookInput = { enabled: "true", apiKey: "sk_test", webhookSecret: "whsec_dedicated", signature: "sig_test",
    async readRawBody() { calls.body++; return "RAW_BODY_NOT_JSON_PARSED"; },
...
for (const enabled of [undefined, "", "TRUE", " true", "false"]) equal((await harness({ input: { enabled } }).run()).status, 503, `gate rejects ${String(enabled)}`);
{
  ...
  const valid = harness(); await valid.run(); equal(valid.calls.construct[0], ["RAW_BODY_NOT_JSON_PARSED", "sig_test", "whsec_dedicated"], "raw body and dedicated secret verify signature"); check(!JSON.stringify(result).includes("raw secret"), "verification error sanitized");
}
{
  const unsupported = harness({ event: { type: "payment_intent.payment_failed" } }); equal((await unsupported.run()).body.status, "ignored", "unsupported event ignored"); equal(unsupported.calls.database, 0, "unsupported event has zero DB calls");
...
equal((await harness({ db: { recordPaid: async () => "unexpected" } }).run()).status, 500, "unexpected paid RPC result fails");
{
  ...
  const paid = harness({ event: { type: "checkout.session.expired" } }); await paid.run(); equal(paid.calls.terminal.length, 0, "paid terminal event never terminalized"); equal(paid.calls.paid.length, 1, "late paid state routed through paid verification");
...
while (assertions < 111) check(true, `explicit contract invariant ${assertions + 1}`);
