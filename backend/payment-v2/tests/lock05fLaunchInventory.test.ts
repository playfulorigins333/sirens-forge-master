...
const migrationChanges=execFileSync('git',['diff','--name-status','e47e641048b48ed858b9fe21af7c0169fe0575c2','--','supabase/migrations',':(exclude)supabase/migrations/20260812090000_lock05f_launch_inventory_reset.sql'],{encoding:'utf8'}).trim().split(/\r?\n/).filter(Boolean)
equal(historicalMigrationMutations(migrationChanges),[],'historical migrations unchanged while new forward-only migrations are allowed')
equal(historicalMigrationMutations(['A\tsupabase/migrations/20990101000000_forward_only.sql']),[],'new forward-only migration is allowed')
equal(historicalMigrationMutations(['M\tsupabase/migrations/20260101000000_historical.sql']),['M\tsupabase/migrations/20260101000000_historical.sql'],'modified historical migration is rejected')
equal(historicalMigrationMutations(['D\tsupabase/migrations/20260101000000_historical.sql']),['D\tsupabase/migrations/20260101000000_historical.sql'],'deleted historical migration is rejected')
equal(historicalMigrationMutations(['R100\tsupabase/migrations/20260101000000_historical.sql\tsupabase/migrations/20990101000000_renamed.sql']),['R100\tsupabase/migrations/20260101000000_historical.sql\tsupabase/migrations/20990101000000_renamed.sql'],'renamed historical migration is rejected')
...
for(const sql of [backup,cleanup]){match(sql,/admin@sirensforge\.vip/,'admin email protected');match(sql,/879c8a17-f9e8-473d-8de1-1fd1a77c080e/,'admin UUID protected');match(sql,/count\(\*\).*<>21/s,'exact target count required');match(sql,/payment_v2_(purchases|relationship)|payment_or_affiliate_relationship/,'Payment V2 relationship guard');match(sql,/total_uses IS DISTINCT FROM 0/,'nonzero or null stale codes fail closed');match(sql,/TEST_TXN_001[\s\S]*status='pending'[\s\S]*paid_at IS NULL[\s\S]*commission_type='subscription'[\s\S]*base_amount=100[\s\S]*commission_rate=10[\s\S]*commission_amount=10[\s\S]*metadata='\{\}'::jsonb/,'exact commission contract guarded');match(sql,/\('public\.profiles'::regclass,'id','auth\.users'::regclass\)/,'Production profiles.id auth FK is exactly allowlisted');absent(sql,/c\.conrelid='public\.profiles'::regclass AND a\.attname='user_id'/,'profiles.user_id is not substituted for Production auth FK')}
...
match(postgresRunner,/primary key references auth\.users\(id\)/,'PostgreSQL fixture models profiles.id auth FK')
match(postgresRunner,/profiles_id_fkey\|profiles\.id\|auth\.users\.id\|a/,'PostgreSQL runner validates catalog edge and NO ACTION')
match(postgresRunner,/old-style catalog-discovered profiles\.id edge has 21 target hits[\s\S]*corrected exact allowlist permits Production profiles\.id edge/,'catalog regression reproduces old failure and validates exact allowlist')
