...
function harness(options: { holdState?: string; purchaseState?: string | null; allocation?: boolean; entitlement?: boolean; entitlementStatus?: string; profileCount?: number; rpcResult?: string } = {}) {
  ...
  const purchase = (): Purchase => ({ id: ids.purchase, hold_id: ids.hold, purchaser_credential_hash: digest, tier: "og_throne", state: purchaseState!, stripe_checkout_session_id: sid, stripe_subscription_id: null, stripe_customer_id: "cus_test", claimed_profile_id: purchaseState === "CLAIMED" ? ids.profile : null });
