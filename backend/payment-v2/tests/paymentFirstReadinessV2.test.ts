...
includes(pricingSource, '<SeatCounterText tier={seats.earlyBird} fullCapacityLabel="spots remaining" />');
includes(pricingSource, 'fullCapacityLabel = "founder spots remaining"');
includes(pricingSource, '<span> seats left</span>');
excludes(pricingSource, 'tier.total.toLocaleString()} founder spots remaining');
