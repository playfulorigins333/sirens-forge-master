...
function harness(overrides: Partial<CheckoutDependencies> = {}) {
  const calls = { tier: [] as unknown[], acquire: [] as unknown[], referral: [] as unknown[], create: [] as unknown[], associate: [] as unknown[], retrieve: [] as unknown[] };
  const deps: CheckoutDependencies = {
    ...
    async acquireHold(hash, tier, expiresAt) { calls.acquire.push([Buffer.from(hash), tier, expiresAt]); return { holdId, state: "HELD" }; },
    ...
    async resolveReferral(code) { calls.referral.push(code); return null; },
    async retrievePriceUnitAmount() { return 10000; },
  ...
  const run = (body: unknown = { tierName: "og_throne" }, more: Record<string, unknown> = {}) => paymentFirstCheckout({ enabled: "true", body, production: true, baseUrl: "https://sirensforge.com", ...more }, deps);
...

{
  ...
  equal(h.calls.acquire.length, 0, "disabled makes zero database acquisitions"); equal(h.calls.create.length, 0, "disabled makes zero Stripe calls"); equal(h.calls.referral.length, 0, "disabled does not resolve referrals");
}
...
{
  const h = harness({ acquireHold: async () => ({ holdId, state: "SESSION_ASSOCIATED" }) }); const result = await h.run(); equal(result.status, 200, "associated exact retry succeeds");
  ...
  ]) { const rejected = await harness({ acquireHold: async () => ({ holdId, state: "SESSION_ASSOCIATED" }), retrieveSession: async () => session }).run(); equal(rejected.status, 500, "unsafe associated Session fails closed"); equal(rejected.body.url, undefined, "unsafe Session URL withheld"); }
}

{
  const verified = harness({ resolveReferral: async code => { equal(code, "SAFE_CODE", "referral normalized server-side"); return { destination: "acct_Verified123", commissionPercent: 25 }; } });
  await verified.run({ tierName: "og_throne", referralCode: " safe_code " }); const params = verified.calls.create[0][0] as any;
  equal(params.payment_intent_data.transfer_data.destination, "acct_Verified123", "destination server-resolved"); equal(params.payment_intent_data.application_fee_amount, 7500, "fee server-resolved");
  const unsafe = await harness({ resolveReferral: async () => ({ destination: "unverified", commissionPercent: 25 }) }).run({ tierName: "og_throne", referralCode: "CODE" }); equal(unsafe.status, 500, "unverified destination fails closed");
}
