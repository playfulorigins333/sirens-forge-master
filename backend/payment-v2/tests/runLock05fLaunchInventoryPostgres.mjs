...
create table profiles(id uuid primary key references auth.users(id),user_id uuid not null unique,email text,role text not null default 'user',is_og_vip boolean not null default false,is_tester boolean not null default false,is_beta_tester boolean not null default false,seat_number integer,og_seat_number integer,tier text,subscription_status text,password_hash text,stripe_customer_id text,stripe_connect_account_id text,stripe_connect_onboarded boolean not null default false);
...
run(`insert into auth.users(id,email)values('10000000-0000-4000-8000-000000000002','profile-buyer@example.invalid'),('20000000-0000-4000-8000-000000000002','buyer@example.invalid');insert into profiles(id,user_id)values('10000000-0000-4000-8000-000000000002','20000000-0000-4000-8000-000000000002')`)
...
equal(`select conname||'|'||conrelid::regclass||'.'||a.attname||'|'||confrelid::regclass||'.id|'||confdeltype::text from pg_constraint c join lateral unnest(c.conkey) k(attnum) on true join pg_attribute a on a.attrelid=c.conrelid and a.attnum=k.attnum where c.contype='f' and c.conrelid='public.profiles'::regclass and a.attname='id' and c.confrelid='auth.users'::regclass`,'profiles_id_fkey|profiles.id|auth.users.id|a','Production profiles.id auth FK uses NO ACTION')
equal(`select count(*) from pg_constraint c join lateral unnest(c.conkey) k(attnum) on true join pg_attribute a on a.attrelid=c.conrelid and a.attnum=k.attnum join profiles p on c.conrelid='public.profiles'::regclass and a.attname='id' join profiles t on t.id=p.id and t.is_og_vip and t.tier='og_throne' and t.subscription_status='active' and t.stripe_customer_id is null and t.stripe_connect_account_id is null and t.stripe_connect_onboarded=false where c.contype='f' and c.confrelid='auth.users'::regclass`,'21','old-style catalog-discovered profiles.id edge has 21 target hits')
equal(`select count(*) from pg_constraint c join lateral unnest(c.conkey) k(attnum) on true join pg_attribute a on a.attrelid=c.conrelid and a.attnum=k.attnum where c.contype='f' and c.conrelid='public.profiles'::regclass and a.attname='id' and c.confrelid='auth.users'::regclass and not ((c.conrelid,a.attname,c.confrelid) in (('public.profiles'::regclass,'id','auth.users'::regclass),('public.user_subscriptions'::regclass,'user_id','public.profiles'::regclass),('public.referral_codes'::regclass,'user_id','auth.users'::regclass),('public.referral_tracking'::regclass,'referred_user_id','auth.users'::regclass),('public.commission_earnings'::regclass,'referred_user_id','auth.users'::regclass)))`,'0','corrected exact allowlist permits Production profiles.id edge')
