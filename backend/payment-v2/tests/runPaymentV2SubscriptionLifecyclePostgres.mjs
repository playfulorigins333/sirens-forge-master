...
ok(`drop schema public cascade;create schema public;grant all on schema public to postgres;grant usage on schema public to public;
...
const sig="public.payment_v2_apply_early_bird_subscription_lifecycle(text,text,text,timestamp with time zone,timestamp with time zone,boolean,timestamp with time zone,timestamp with time zone,timestamp with time zone)"
...
const call=(status='active',customer='cus_early')=>`select payment_v2_apply_early_bird_subscription_lifecycle('sub_early','${customer}','${status}',timestamp '2026-08-01Z',timestamp '2026-09-01Z',true,null,null,null)`
...
equal(`select payment_v2_apply_early_bird_subscription_lifecycle('sub_missing','cus_missing','active',null,null,false,null,null,null)`,'purchase_pending','missing purchase pending','service_role')
...
equal(`select payment_v2_apply_early_bird_subscription_lifecycle('sub_unclaimed','cus_unclaimed','canceled',null,null,false,now(),null,null)`,'unclaimed','unclaimed no-op','service_role')
...
equal(`select count(*) from user_subscriptions where tier_name='og_throne'`,0,'OG untouched')
