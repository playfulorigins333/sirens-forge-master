...
do $$ declare j uuid; a uuid; l uuid; a2 uuid; begin
 select job_id into j from public.submit_compute_job('10000000-0000-4000-8000-000000000003','image','normal-retry',repeat('0',64),'{}','standard'); select job_id,attempt_id,lease_token into j,a,l from public.claim_compute_job('image','retry-1'); assert public.retry_compute_pre_dispatch(j,a,l,'TRANSIENT_FAILURE',0)='queued';
 select attempt_id,lease_token into a2,l from public.claim_compute_job('image','retry-2'); assert a2<>a; assert (select ordinal=2 from public.compute_job_attempts where id=a2); assert public.retry_compute_pre_dispatch(j,a2,l,'TRANSIENT_FAILURE',0)='failed';
end$$;
...
do $$ declare j uuid; a uuid; l uuid; rt uuid; begin
 ...
 perform public.compute_worker_transition(j,a,l,'start'); perform public.mark_compute_provider_dispatch(j,a,l,'opaque-op'); perform public.mark_compute_provider_dispatch(j,a,l,'opaque-op');
...
do $$ declare j uuid; a uuid; l uuid; rt uuid; begin
 select job_id into j from public.submit_compute_job('10000000-0000-4000-8000-000000000002','trainer','reconcile-requeue',repeat('e',64),'{}','standard'); select job_id,attempt_id,lease_token into j,a,l from public.claim_compute_job('trainer','reconcile-worker'); perform public.mark_compute_provider_dispatch(j,a,l,'trainer-op'); update public.compute_jobs set lease_expires_at=now()-interval '1 second' where id=j; perform public.recover_stale_compute_jobs(); select recovery_token into rt from public.compute_job_attempts where id=a; assert public.reconcile_compute_recovery(j,a,rt,'requeue',true)='queued';
end$$;
...
do $$ declare j uuid; a uuid; l uuid; rt uuid; begin
 ...
 select job_id into j from public.submit_compute_job('10000000-0000-4000-8000-000000000003','video','cancel-stale',repeat('1',64),'{}','standard'); select job_id,attempt_id,lease_token into j,a,l from public.claim_compute_job('video','cancel-worker'); perform public.cancel_compute_job('10000000-0000-4000-8000-000000000003',j); update public.compute_jobs set lease_expires_at=now()-interval '1 second' where id=j; perform public.recover_stale_compute_jobs(); assert (select state='cancelled' from public.compute_jobs where id=j);
 select job_id into j from public.submit_compute_job('10000000-0000-4000-8000-000000000003','video','cancel-dispatched',repeat('2',64),'{}','standard'); select job_id,attempt_id,lease_token into j,a,l from public.claim_compute_job('video','cancel-dispatched-worker'); perform public.mark_compute_provider_dispatch(j,a,l,'cancel-op'); perform public.cancel_compute_job('10000000-0000-4000-8000-000000000003',j); update public.compute_jobs set lease_expires_at=now()-interval '1 second' where id=j; perform public.recover_stale_compute_jobs(); assert (select state='recovering' and cancellation_requested_at is not null from public.compute_jobs where id=j); select recovery_token into rt from public.compute_job_attempts where id=a; assert public.reconcile_compute_recovery(j,a,rt,'cancelled')='cancelled';
end$$;
...
do $$ declare j uuid; a uuid; l uuid; begin
 select job_id into j from public.submit_compute_job('10000000-0000-4000-8000-000000000002','video','missing-policy',repeat('a',64),'{}','standard'); select job_id,attempt_id,lease_token into j,a,l from public.claim_compute_job('video','missing-policy-worker'); assert not public.authorize_compute_dispatch(j,a,l,100); assert (select state='queued' and lease_token is null and available_at>now() from public.compute_jobs where id=j); assert (select finished_at is not null from public.compute_job_attempts where id=a);
end$$;
...
insert into public.compute_spend_policies(version,effective_from,daily_limit_micros,monthly_limit_micros,video_job_limit_micros,enabled) values(1,now(),1000,1000,600,true);
do $$ declare j uuid; a uuid; l uuid; begin
 select job_id into j from public.submit_compute_job('10000000-0000-4000-8000-000000000002','image','cost',repeat('b',64),'{}','standard'); select job_id,attempt_id,lease_token into j,a,l from public.claim_compute_job('image','cost-worker'); assert public.authorize_compute_dispatch(j,a,l,1000); assert public.authorize_compute_dispatch(j,a,l,1000);
 begin perform public.authorize_compute_dispatch(j,a,l,999); raise exception 'reservation conflict accepted'; exception when others then assert sqlerrm like '%RESERVATION_CONFLICT%'; end;
 assert (select count(*)=8 from public.compute_spend_threshold_events); perform public.compute_worker_transition(j,a,l,'start'); perform public.record_compute_actual_cost(j,a,l,700,123,'{}'); perform public.record_compute_actual_cost(j,a,l,700,123,'{}');
 begin perform public.record_compute_actual_cost(j,a,l,701,123,'{}'); raise exception 'actual conflict accepted'; exception when others then assert sqlerrm like '%ACTUAL_COST_CONFLICT%'; end;
 assert (select sum(amount_micros)=700 from public.compute_cost_ledger where attempt_id=a); assert (select count(*)=1 from public.compute_cost_ledger where attempt_id=a and kind='release'); update public.compute_spend_policies set daily_limit_micros=1,monthly_limit_micros=1 where version=1; assert public.compute_worker_transition(j,a,l,'success')='succeeded';
end$$;
