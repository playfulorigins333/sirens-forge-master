...
run(["-c",`create table public.pass4c_predefinition_sentinel(signature text primary key,definition text not null); insert into public.pass4c_predefinition_sentinel values
 ('finalize_private_generation',pg_get_functiondef('public.finalize_private_generation(uuid,uuid,jsonb,jsonb)'::regprocedure)),
 ...
 create table public.pass4c_preacl_sentinel(acl aclitem[]); insert into public.pass4c_preacl_sentinel select proacl from pg_proc where oid='public.finalize_private_generation(uuid,uuid,jsonb,jsonb)'::regprocedure;
...
run(["-c",`delete from public.generation_assets where generation_id in(select id from public.video_projects); delete from public.generations where id in(select id from public.video_projects); delete from public.video_project_segments; delete from public.private_storage_objects where source_reference ? 'video_project_id'; delete from public.video_projects; delete from public.compute_cost_ledger where job_id in(select id from public.compute_jobs where workload in('video','stitch')); delete from public.compute_job_attempts where job_id in(select id from public.compute_jobs where workload in('video','stitch')); delete from public.compute_jobs where workload in('video','stitch');`]);
...
run(["-c",`do $$ begin
 ...
 assert pg_get_functiondef('public.finalize_private_generation(uuid,uuid,jsonb,jsonb)'::regprocedure)=(select definition from public.pass4c_predefinition_sentinel where signature='finalize_private_generation');
 assert (select proacl from pg_proc where oid='public.finalize_private_generation(uuid,uuid,jsonb,jsonb)'::regprocedure)=(select acl from public.pass4c_preacl_sentinel);
 ...
 assert to_regprocedure('public.project_trainer_compute_state()') is not null; assert exists(select 1 from pg_trigger where tgname='project_trainer_compute_state' and not tgisinternal); assert has_table_privilege('anon','public.compute_unrelated_grant_sentinel','select'); assert has_table_privilege('authenticated','public.compute_unrelated_grant_sentinel','insert');
