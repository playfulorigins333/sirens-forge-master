...
select task17a_test.recovery_prepare_fixture('recovery_drift_operator_offset_not_60_minutes',931209,true,'after_operator_due','creator','recdrift39-claim',0,false) as recdrift39_f \gset
select to_jsonb(q) as recdrift39_q0 from public.creator_publishing_queue_tasks q where id=(:'recdrift39_f'::jsonb->>'task')::uuid \gset
select to_jsonb(j) as recdrift39_j0 from public.creator_publishing_platform_jobs j where id=(:'recdrift39_f'::jsonb->>'job')::uuid \gset
begin;
update public.creator_publishing_platform_jobs set operator_due_at=intended_publish_at - interval '30 minutes' where id=(:'recdrift39_f'::jsonb->>'job')::uuid;
select task17a_test.assert_recovery_rejected('recovery_drift_operator_offset_not_60_minutes','OPERATOR_TASK_INELIGIBLE',(:'recdrift39_f'::jsonb->>'creator')::uuid,(:'recdrift39_f'::jsonb->>'task')::uuid,(:'recdrift39_f'::jsonb->>'job')::uuid,'recdrift39');
rollback;
select task17a_test.assert((select to_jsonb(q)=(:'recdrift39_q0')::jsonb from public.creator_publishing_queue_tasks q where id=(:'recdrift39_f'::jsonb->>'task')::uuid), 'recovery_drift_operator_offset_not_60_minutes queue restored after rollback');
select task17a_test.assert((select to_jsonb(j)=(:'recdrift39_j0')::jsonb from public.creator_publishing_platform_jobs j where id=(:'recdrift39_f'::jsonb->>'job')::uuid), 'recovery_drift_operator_offset_not_60_minutes job restored after rollback');
...
insert into task17a_recovery_rejections(label,key,task_id,job_id) values('recovery_drift_operator_offset_not_60_minutes','recdrift39',(:'recdrift39_f'::jsonb->>'task')::uuid,(:'recdrift39_f'::jsonb->>'job')::uuid) on conflict (label) do update set key=excluded.key, task_id=excluded.task_id, job_id=excluded.job_id;

\echo TASK17A_SCENARIO_START: recovery_drift_job_state_inconsistent_with_schedule
select task17a_test.recovery_prepare_fixture('recovery_drift_job_state_inconsistent_with_schedule',931210,true,'after_operator_due','creator','recdrift40-claim',0,false) as recdrift40_f \gset
select to_jsonb(q) as recdrift40_q0 from public.creator_publishing_queue_tasks q where id=(:'recdrift40_f'::jsonb->>'task')::uuid \gset
select to_jsonb(j) as recdrift40_j0 from public.creator_publishing_platform_jobs j where id=(:'recdrift40_f'::jsonb->>'job')::uuid \gset
begin;
update public.creator_publishing_platform_jobs set job_state='draft' where id=(:'recdrift40_f'::jsonb->>'job')::uuid;
select task17a_test.assert_recovery_rejected('recovery_drift_job_state_inconsistent_with_schedule','OPERATOR_TASK_INELIGIBLE',(:'recdrift40_f'::jsonb->>'creator')::uuid,(:'recdrift40_f'::jsonb->>'task')::uuid,(:'recdrift40_f'::jsonb->>'job')::uuid,'recdrift40');
rollback;
select task17a_test.assert((select to_jsonb(q)=(:'recdrift40_q0')::jsonb from public.creator_publishing_queue_tasks q where id=(:'recdrift40_f'::jsonb->>'task')::uuid), 'recovery_drift_job_state_inconsistent_with_schedule queue restored after rollback');
select task17a_test.assert((select to_jsonb(j)=(:'recdrift40_j0')::jsonb from public.creator_publishing_platform_jobs j where id=(:'recdrift40_f'::jsonb->>'job')::uuid), 'recovery_drift_job_state_inconsistent_with_schedule job restored after rollback');
...
insert into task17a_recovery_rejections(label,key,task_id,job_id) values('recovery_drift_job_state_inconsistent_with_schedule','recdrift40',(:'recdrift40_f'::jsonb->>'task')::uuid,(:'recdrift40_f'::jsonb->>'job')::uuid) on conflict (label) do update set key=excluded.key, task_id=excluded.task_id, job_id=excluded.job_id;

\echo TASK17A_SCENARIO_START: recovery_drift_unscheduled_job_with_schedule_fields
select task17a_test.recovery_prepare_fixture('recovery_drift_unscheduled_job_with_schedule_fields',931211,false,null,'creator','recdrift41-claim',0,false) as recdrift41_f \gset
select to_jsonb(q) as recdrift41_q0 from public.creator_publishing_queue_tasks q where id=(:'recdrift41_f'::jsonb->>'task')::uuid \gset
select to_jsonb(j) as recdrift41_j0 from public.creator_publishing_platform_jobs j where id=(:'recdrift41_f'::jsonb->>'job')::uuid \gset
begin;
alter table public.creator_publishing_platform_jobs drop constraint creator_publishing_jobs_unscheduled_fields_null;
update public.creator_publishing_platform_jobs set intended_publish_at=clock_timestamp()+interval '2 hours', operator_due_at=clock_timestamp()+interval '1 hour', schedule_timezone='UTC', scheduled_at=clock_timestamp(), scheduled_by=creator_id where id=(:'recdrift41_f'::jsonb->>'job')::uuid;
select task17a_test.assert_recovery_rejected('recovery_drift_unscheduled_job_with_schedule_fields','OPERATOR_TASK_INELIGIBLE',(:'recdrift41_f'::jsonb->>'creator')::uuid,(:'recdrift41_f'::jsonb->>'task')::uuid,(:'recdrift41_f'::jsonb->>'job')::uuid,'recdrift41');
rollback;
...
select task17a_test.assert((select to_jsonb(q)=(:'recdrift41_q0')::jsonb from public.creator_publishing_queue_tasks q where id=(:'recdrift41_f'::jsonb->>'task')::uuid), 'recovery_drift_unscheduled_job_with_schedule_fields queue restored after rollback');
select task17a_test.assert((select to_jsonb(j)=(:'recdrift41_j0')::jsonb from public.creator_publishing_platform_jobs j where id=(:'recdrift41_f'::jsonb->>'job')::uuid), 'recovery_drift_unscheduled_job_with_schedule_fields job restored after rollback');
...
insert into task17a_recovery_rejections(label,key,task_id,job_id) values('recovery_drift_unscheduled_job_with_schedule_fields','recdrift41',(:'recdrift41_f'::jsonb->>'task')::uuid,(:'recdrift41_f'::jsonb->>'job')::uuid) on conflict (label) do update set key=excluded.key, task_id=excluded.task_id, job_id=excluded.job_id;
