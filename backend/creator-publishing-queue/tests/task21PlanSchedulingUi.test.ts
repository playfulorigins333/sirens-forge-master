...
assert.match(ui,/UNCERTAIN_CODES/); assert.match(ui,/SCHEDULING_TRUSTED_RESPONSE_INVALID/); assert.match(ui,/SCHEDULING_SERVICE_UNAVAILABLE/)
assert.match(ui,/setScheduleExpectation\(\{publishingPlanId:plan\.publishingPlanId,platformJobId:plan\.platformJobId,intendedPublishAtUtc:summary\.intendedPublishAtUtc,scheduleTimezone:summary\.scheduleTimezone,idempotencyKey:key\}\)/)
assert.match(ui,/setCancelExpectation\(\{publishingPlanId:planId,normalizedReason:reason,idempotencyKey:cancelKey\}\)/)
assert.match(ui,/b\?\.error && !UNCERTAIN_CODES\.has\(b\.error\)/)
