...
assert.notEqual(AI_TWIN_CONSENT_VERSION,"creator-ai-twin-consent-v1")
assert.notEqual(getAiTwinConsentTextSha256(),"0c36baeb6477f36caa583cc46dd204cad4b5b57f0bd9c34779b0a14672b5de12")
...
const photoFacts = parseTrustedFactsRpcResponse(facts({ generation:{ lora_used:null, safe_classification_metadata:{ photorealistic:true, lifelike:true } } }), uuid).facts
let photoDerived = deriveTrustedComplianceInput(photoFacts)
assert.equal(photoDerived.aiDetail.lora_generated,false)
assert.equal(photoDerived.aiDetail.ai_twin,false)
assert.equal(photoDerived.aiDetail.photorealistic,true)
assert.equal(photoDerived.aiDetail.lifelike,true)
assert.equal(photoDerived.input.ai_twin_consent_status,"granted")
photoDerived = deriveTrustedComplianceInput(parseTrustedFactsRpcResponse(facts({ ai_twin_consent:{status:"missing", attestation_version:null, attestation_text_sha256:null, granted_at:null, updated_at:null}, generation:{ lora_used:null, safe_classification_metadata:{ photorealistic:true } } }), uuid).facts)
assert.equal(photoDerived.input.ai_twin_consent_status,"missing")
const unclassifiedDerived = deriveTrustedComplianceInput(parseTrustedFactsRpcResponse(facts({ generation:{ lora_used:null, safe_classification_metadata:{} } }), uuid).facts)
assert.equal(unclassifiedDerived.aiDetail.photorealistic,true)
assert.equal(unclassifiedDerived.aiDetail.lifelike,true)
assert.equal(unclassifiedDerived.input.ai_twin_consent_status,"granted")
const nonPhotoDerived = deriveTrustedComplianceInput(parseTrustedFactsRpcResponse(facts({ generation:{ lora_used:null, safe_classification_metadata:{ non_photorealistic:true } } }), uuid).facts)
assert.equal(nonPhotoDerived.input.ai_twin_consent_status,"not_applicable")
assert.match(migration,/jsonb_array_elements\(v_facts->'generation_manifest'\)[\s\S]*lora_used/)
assert.match(migration,/safe_classification_metadata'->>'photorealistic'/)
assert.match(migration,/safe_classification_metadata'->>'lifelike'/)
assert.match(migration,/safe_classification_metadata'->>'non_photorealistic'/)
assert.match(migration,/then case when v_facts->'ai_twin_consent'->>'status'='granted'/)
assert.doesNotMatch(migration,/when coalesce\(\(p_ai_detail->>'ai_twin'\)::boolean,false\) is not true then 'not_applicable'/)
console.log("trusted compliance submission consent-required parity tests passed")
