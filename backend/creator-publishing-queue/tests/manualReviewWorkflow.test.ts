...
validateCreatorPublishingPackageForTrustedReview(pkg(), input())
for (const status of ['pending','passed','escalated_approved'] as const) assertCode(() => validateCreatorPublishingPackageForTrustedReview(pkg({ compliance_status: status }), input()), 'REVIEW_INVALID_CURRENT_STATUS')
assertCode(() => validateCreatorPublishingPackageForTrustedReview(pkg({ compliance_status: 'blocked' }), input()), 'REVIEW_BLOCKED_NOT_ESCALATABLE')
assertCode(() => validateCreatorPublishingPackageForTrustedReview(pkg({ target_platform: 'fanvue' }), input()), 'REVIEW_FANVUE_NOT_SUPPORTED')
assertCode(() => validateCreatorPublishingPackageForTrustedReview(pkg({ compliance_policy_version: 'old' }), input()), 'REVIEW_STALE_POLICY_VERSION')
assertCode(() => validateCreatorPublishingPackageForTrustedReview(pkg({ compliance_policy_version: 'unassigned' }), input({ expected_policy_version: 'unassigned' })), 'REVIEW_POLICY_VERSION_UNASSIGNED')
assertCode(() => validateCreatorPublishingPackageForTrustedReview(null, input()), 'REVIEW_PACKAGE_NOT_FOUND')
assertCode(() => validateCreatorPublishingPackageForTrustedReview(pkg(), input({ expected_current_status: 'pending' })), 'REVIEW_INVALID_CURRENT_STATUS')

function mockDb(row: CreatorPublishingPackageForReview, opts: { duplicate?: boolean, conflict?: boolean } = {}) {
  const calls: any[] = []
  const db = { from(table: string) { const q: any = { table, payload: undefined, filters: [] as any[], select(c?: string) { calls.push(['select', table, c]); return q }, eq(c: string, v: unknown) { q.filters.push([c,v]); calls.push(['eq', table, c, v]); return q }, order() { return q }, limit() { return q }, maybeSingle() { calls.push(['maybeSingle', table]); return Promise.resolve({ data: opts.duplicate ? { id: 7 } : null, error: null }) }, single() { calls.push(['single', table]); if (table === 'creator_publishing_content_packages' && !q.payload) return Promise.resolve({ data: row, error: null }); if (table === 'creator_publishing_content_packages' && q.payload) return Promise.resolve({ data: opts.conflict ? null : { id: row.id }, error: null }); if (table === 'creator_publishing_compliance_reviews') return Promise.resolve({ data: { id: 'review-1' }, error: null }); if (table === 'creator_publishing_audit_events') return Promise.resolve({ data: { id: 9 }, error: null }); return Promise.resolve({ data: null, error: null }) }, insert(p: unknown) { q.payload = p; calls.push(['insert', table, p]); return q }, update(p: unknown) { q.payload = p; calls.push(['update', table, p]); return q } }; return q } }
  return { db, calls }
}
...
assert.match(migration, /revoke all[\s\S]*from anon, authenticated/, 'ordinary users cannot execute RPC')
...
assert.match(migration, /outcome = 'manual_review' and review_source = 'automated'/, 'automated manual_review rows are tracked but not escalation approvals')
