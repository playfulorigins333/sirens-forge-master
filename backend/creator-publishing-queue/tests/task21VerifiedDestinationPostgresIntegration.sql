...
-- Signatures, privileges, ordering, locking, and untouched scheduler RPC definitions.
do $$
declare
...
begin
  ...
  perform pg_temp.assert_true(not has_function_privilege('public','public.creator_publishing_save_content_package(uuid,text,uuid,uuid,text,text,boolean,text,text,timestamptz,text)','execute'),'public package execute');
  perform pg_temp.assert_true(not has_function_privilege('anon','public.creator_publishing_save_content_package(uuid,text,uuid,uuid,text,text,boolean,text,text,timestamptz,text)','execute'),'anon package execute');
  perform pg_temp.assert_true(not has_function_privilege('authenticated','public.creator_publishing_save_content_package(uuid,text,uuid,uuid,text,text,boolean,text,text,timestamptz,text)','execute'),'authenticated package execute');
  perform pg_temp.assert_true(has_function_privilege('service_role','public.creator_publishing_save_content_package(uuid,text,uuid,uuid,text,text,boolean,text,text,timestamptz,text)','execute'),'service package execute');
  perform pg_temp.assert_true(not has_function_privilege('public','public.creator_publishing_create_autopost_plan(uuid,uuid[],text)','execute'),'public plan execute');
  perform pg_temp.assert_true(not has_function_privilege('anon','public.creator_publishing_create_autopost_plan(uuid,uuid[],text)','execute'),'anon plan execute');
  perform pg_temp.assert_true(not has_function_privilege('authenticated','public.creator_publishing_create_autopost_plan(uuid,uuid[],text)','execute'),'authenticated plan execute');
  perform pg_temp.assert_true(has_function_privilege('service_role','public.creator_publishing_create_autopost_plan(uuid,uuid[],text)','execute'),'service plan execute');
...
-- Package create rejects every nonverified state without rows or audits.
select pg_temp.expect_error('unattested package','DESTINATION_ACCOUNT_NOT_VERIFIED',$q$
 select public.creator_publishing_save_content_package('11111111-1111-4111-8111-111111111111','create',null,'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa1','Unattested should fail','caption',false,null,null,null,'pkg_unattested_01')$q$);
select pg_temp.expect_error('attested package','DESTINATION_ACCOUNT_NOT_VERIFIED',$q$
 select public.creator_publishing_save_content_package('11111111-1111-4111-8111-111111111111','create',null,'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa2','Attested should fail','caption',false,null,null,null,'pkg_attested_001')$q$);
select pg_temp.expect_error('revoked package','PLATFORM_ACCOUNT_REVOKED',$q$
 select public.creator_publishing_save_content_package('11111111-1111-4111-8111-111111111111','create',null,'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa3','Revoked should fail','caption',false,null,null,null,'pkg_revoked_0001')$q$);
do $$ begin
...
create temp table task21_package_create as
select public.creator_publishing_save_content_package('11111111-1111-4111-8111-111111111111','create',null,'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa4','Verified created package','verified caption',false,'price','visibility',null,'pkg_verified_001') result;
do $$ begin
...
create temp table task21_package_replay as
select public.creator_publishing_save_content_package('11111111-1111-4111-8111-111111111111','create',null,'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa4','Verified created package','verified caption',false,'price','visibility',null,'pkg_verified_001') result;
do $$ begin
...
select pg_temp.expect_error('changed package replay','IDEMPOTENCY_CONFLICT',$q$
 select public.creator_publishing_save_content_package('11111111-1111-4111-8111-111111111111','create',null,'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa4','Changed replay','verified caption',false,'price','visibility',null,'pkg_verified_001')$q$);
...
-- Nonverified and mixed plan requests fail atomically before plan, job, event, or audit creation.
select pg_temp.expect_error('attested plan','DESTINATION_ACCOUNT_NOT_VERIFIED',$q$
 select public.creator_publishing_create_autopost_plan('11111111-1111-4111-8111-111111111111',array['cccccccc-cccc-4ccc-8ccc-ccccccccccc1'::uuid],'plan_attested_001')$q$);
select pg_temp.expect_error('mixed plan','DESTINATION_ACCOUNT_NOT_VERIFIED',$q$
 select public.creator_publishing_create_autopost_plan('11111111-1111-4111-8111-111111111111',array['cccccccc-cccc-4ccc-8ccc-ccccccccccc1'::uuid,'cccccccc-cccc-4ccc-8ccc-ccccccccccc2'::uuid],'plan_mixed_00001')$q$);
do $$ begin
...
create temp table task21_plan_create as
select public.creator_publishing_create_autopost_plan('11111111-1111-4111-8111-111111111111',array['cccccccc-cccc-4ccc-8ccc-ccccccccccc4'::uuid],'plan_verified_001') result;
...
update public.creator_platform_accounts set verification_status='creator_attested',verification_reviewed_by=null,verification_reviewed_at=null,verification_evidence_reference=null,verification_reason=null,verification_legacy_revoked=false,verification_attested_at=coalesce(verification_attested_at,clock_timestamp()) where id='aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaa6';
create temp table task21_plan_replay as
select public.creator_publishing_create_autopost_plan('11111111-1111-4111-8111-111111111111',array['cccccccc-cccc-4ccc-8ccc-ccccccccccc4'::uuid],'plan_verified_001') result;
do $$ begin
...
select pg_temp.expect_error('changed plan replay','IDEMPOTENCY_CONFLICT',$q$
 select public.creator_publishing_create_autopost_plan('11111111-1111-4111-8111-111111111111',array['cccccccc-cccc-4ccc-8ccc-ccccccccccc3'::uuid],'plan_verified_001')$q$);

-- Untouched scheduling and processing functions still independently require exact verified state.
do $$
declare
