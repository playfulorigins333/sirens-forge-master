...
const logPath = 'task15-postgres-diagnostics.log'
writeFileSync(logPath, `Task 15 PostgreSQL diagnostics\nstarted_at=${new Date().toISOString()}\n`)
const databaseUrl = process.env.TASK15_DATABASE_URL || process.env.DATABASE_URL || 'postgres://postgres:postgres@127.0.0.1:5432/postgres'
function runSql(label, sql){
  appendFileSync(logPath, `\n## ${label}\n`)
  const file = join(mkdtempSync(join(tmpdir(), 'task15-sql-')), `${label.replace(/[^a-z0-9_-]/gi,'_')}.sql`)
...
try{
  ...
  runFile('task15 assertions', 'backend/creator-publishing-queue/tests/task15PostgresIntegration.sql')
