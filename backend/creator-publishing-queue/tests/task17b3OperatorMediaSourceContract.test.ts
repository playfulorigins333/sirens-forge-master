...
for (const race of ["finalJob:{...base().job, cancelled_at:now}", "job_state:\"posted\"", "target_platform:\"fansly\"", "publishing_mode:\"automatic\""]) assert.ok(access.includes(race), `behavioral final-read job mutation exists for ${race}`)
...
const finalHelper = core.slice(core.indexOf("async function resolveFinalClaimOnly"), core.indexOf("export function calculateOnlyFansOperatorMediaExpiresIn"))
const finalResolveIndex = finalHelper.indexOf("resolveOperatorJobTask")
const finalActiveIndex = finalHelper.indexOf("isActiveOnlyFansAssistedPreparation(row)")
const finalClaimIndex = finalHelper.indexOf("claimFromRow")
assert.ok(finalResolveIndex >= 0 && finalActiveIndex > finalResolveIndex && finalClaimIndex > finalActiveIndex, "resolveFinalClaimOnly checks active work after final resolve and before claimFromRow")
