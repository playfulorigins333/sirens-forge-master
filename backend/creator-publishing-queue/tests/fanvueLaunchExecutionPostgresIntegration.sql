...
insert into public.creator_publishing_platform_jobs(id,publishing_plan_id,creator_id,content_package_id,platform_account_id,target_platform,publishing_mode,job_state,source_package_updated_at,source_package_fingerprint,capability_registry_version,original_request_fingerprint,oauth_account_id,publication_type,intended_publish_at,schedule_timezone,schedule_revision,scheduled_at,scheduled_by,server_idempotency_key)
select '55555555-5555-4555-8555-555555555555','44444444-4444-4444-8444-444444444444',p.creator_id,p.id,p.platform_account_id,'fanvue','direct','direct_publish_queued',p.updated_at,repeat('b',64),'task14.20260711.001',repeat('c',64),a.id,'text',clock_timestamp()-interval '1 minute','UTC',1,clock_timestamp(),'11111111-1111-4111-8111-111111111111','server-derived-logical-request'
...
insert into public.creator_publishing_scheduler_events(creator_id,publishing_plan_id,platform_job_id,event_type,status,due_at,schedule_revision,processed_at) select creator_id,publishing_plan_id,id,'publish_due','processed',intended_publish_at,schedule_revision,clock_timestamp() from public.creator_publishing_platform_jobs where id='55555555-5555-4555-8555-555555555555';
...
select pg_temp.assert_true((select count(*)=0 from public.creator_publishing_claim_scheduled_fanvue_jobs(1,15)),'concurrent claim excluded');
select pg_temp.assert_true((select count(*)=1 from public.creator_publishing_fanvue_attempts where id=(select attempt_id from claim)),'attempt durable at claim');
select pg_temp.assert_true(public.creator_publishing_mark_fanvue_create_dispatched((select attempt_id from claim),(select lease_token from claim)),'dispatch marker durable');
update public.creator_publishing_platform_jobs set leased_at=clock_timestamp()-interval '16 minutes' where id='55555555-5555-4555-8555-555555555555';
select count(*) from public.creator_publishing_claim_scheduled_fanvue_jobs(1,15);
select pg_temp.assert_true((select job_state='uncertain' and next_attempt_at is null from public.creator_publishing_platform_jobs where id='55555555-5555-4555-8555-555555555555'),'crash after marker uncertain');
select pg_temp.assert_true((select count(*)=0 from public.creator_publishing_claim_scheduled_fanvue_jobs(1,15)),'second create impossible');
