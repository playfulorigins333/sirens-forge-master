...
test("creator presentation follows supplied registry facts and history resolves activation server-side", () => {
...
test("CPQ Fanvue runner preserves authenticated GET-only bounded execution ordering", () => {
  ...
  assert.ok(route.indexOf("runCreatorPublishingScheduler") < route.indexOf("runProductionFanvueCpqWorker()"), "scheduler must run before the provider worker")
...
test("provider success requires durable proof and uncertain outcomes remain fail closed", () => {
  ...
  assert.match(core, /outcome === "uncertain"/)
  assert.doesNotMatch(core, /outcome === "uncertain"[\s\S]{0,160}scheduleRetry/)
})
...
test("OnlyFans final verification dependency remains explicit", () => {
