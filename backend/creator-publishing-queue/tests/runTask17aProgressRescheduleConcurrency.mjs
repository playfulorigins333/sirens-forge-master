...
async function runRace(seed, raceName, expectedOrder) {
  const fixture = parseLastJson(psqlSync(`${raceName} fixture`, `select task17a_test.reset_fixture(${seed}, 'scheduled_internally', 'scheduled_internally', true)::text`))
  ...
  const jobBaseline = parseLastJson(psqlSync(`${raceName} job baseline`, `select jsonb_build_object('revision', schedule_revision, 'intended', intended_publish_at, 'operator_due', operator_due_at)::text from public.creator_publishing_platform_jobs where id='${fixture.job}'`))
  ...
  const rescheduleOk = reschedule.code === 0 && rescheduleResult?.ok === true && rescheduleResult?.action_type === 'reschedule' && rescheduleResult?.success_count === 1 && rescheduleResult?.failure_count === 0 && rescheduleResult?.jobs?.[0]?.job_id === fixture.job && rescheduleCleanup?.performed === true && rescheduleCleanup?.queue_task_id === fixture.task && rescheduleCleanup?.previous_status === 'claimed' && rescheduleCleanup?.resulting_status === 'scheduled_internally' && rescheduleCleanup?.reason === 'rescheduled_before_operator_due' && !JSON.stringify(rescheduleResult).includes('claim_token')
  ...
  return { raceName, expectedOrder, observedOrder, finalState, progressResult, rescheduleCleanup, staleFailed, staleState, deadlock: false, timeout: false }
}
