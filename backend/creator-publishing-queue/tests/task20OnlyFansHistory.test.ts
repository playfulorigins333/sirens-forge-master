...
assert.match(migration,/manual_completion_rejection/)
assert.match(migration,/on conflict \(action,entity_type,entity_id,actor_id,idempotency_key\)[\s\S]*do nothing/)
assert.match(migration,/if p_mode='replay_probe' then[\s\S]*return jsonb_build_object\('replayed',false,'idempotent',false\)/)
assert.match(migration,/if coalesce\(\(v_result->>'replayed'\)::boolean,false\) is true then[\s\S]*Do not fabricate or backdate/)
