...
test("runner source has only authorized RPCs and no scheduling, table, autopost, or platform execution", () => {
  ...
  const core = readFileSync(corePath, "utf8")
  assert.equal((core.match(/creator_publishing_claim_due_scheduler_events/g) ?? []).length, 1)
  assert.equal((core.match(/creator_publishing_process_scheduler_event/g) ?? []).length, 1)
  assert.match(core, /CREATOR_PUBLISHING_SCHEDULER_CLAIM_LIMIT = 1 as const/)
  assert.match(core, /CREATOR_PUBLISHING_SCHEDULER_LOCK_MINUTES = 15 as const/)
  assert.match(core, /p_limit: CREATOR_PUBLISHING_SCHEDULER_CLAIM_LIMIT/)
  assert.match(core, /p_lock_minutes: CREATOR_PUBLISHING_SCHEDULER_LOCK_MINUTES/)
  assert.match(core, /try\s*{\s*claim = await admin\.rpc\("creator_publishing_claim_due_scheduler_events", { p_limit: CREATOR_PUBLISHING_SCHEDULER_CLAIM_LIMIT, p_lock_minutes: CREATOR_PUBLISHING_SCHEDULER_LOCK_MINUTES }\)\s*} catch {\s*return { ok: false, code: "CLAIM_RPC_FAILED", \.\.\.counts }\s*}/s)
  assert.match(core, /if \(claim\.error\) return { ok: false, code: "CLAIM_RPC_FAILED", \.\.\.counts }/)
  for (const forbidden of ["/api/autopost/run", "autopost_rules", "autopost_jobs", "autopost_job_logs", "onlyfans.com", "fansly.com", "fanvue.com", "api.onlyfans", "fetch(", ".insert(", ".upsert(", ".delete(", "due_at", "operator_due_at", "intended_publish_at", "timezone", "publication", "Intl.DateTimeFormat", "Date.parse", "setTimeout", "setInterval", "Promise.all", "console.", "JSON.stringify", ".catch("]) assert.equal(core.includes(forbidden), false, forbidden)
  assert.equal((core.match(/\bfor\s*\(/g) ?? []).length, 2)
  assert.equal((core.match(/\bwhile\s*\(/g) ?? []).length, 0)
  assert.doesNotMatch(core, /runCreatorPublishingSchedulerCore[\s\S]*runCreatorPublishingSchedulerCore\(/)
  assert.equal((core.match(/\.rpc\(/g) ?? []).length, 2)
