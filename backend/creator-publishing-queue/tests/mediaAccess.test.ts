...
await test("no upload, delete, list, move, public bucket, platform calls, or Fanvue autopost changes are introduced", () => {
  ...
  const allowedAutopost = new Set(["app/autopost/page.tsx", "app/autopost/Task14AutopostOrchestration.tsx"])
  const unsafe = (path: string) => path.includes("lib/autopost") || (path.includes("fanvue") && !path.includes("creator-publishing-queue/tests")) || (path.startsWith("app/autopost/") && !allowedAutopost.has(path))
