...
\echo TASK17A_SCENARIO_START: initial_schedule_active_claim_to_future_clears_claim
do $$
declare
  f jsonb;
  claim_result jsonb;
  ...
  cleanup jsonb;
begin
  f := task17a_test.reset_fixture(926402,'ready_for_handoff','draft',false);
  claim_result := public.creator_publishing_claim_onlyfans_operator_task((f->>'creator')::uuid,(f->>'task')::uuid,(f->>'job')::uuid,f->>'consent_version',f->>'consent_hash','initschedclaim01');
  perform task17a_test.assert((claim_result->>'ok')::boolean is true, 'initial schedule active claim setup succeeds');
  select claim_token into former_token from public.creator_publishing_queue_tasks where id=(f->>'task')::uuid;
  perform public.creator_publishing_update_onlyfans_operator_progress((f->>'creator')::uuid,(f->>'task')::uuid,(f->>'job')::uuid,former_token,'not_started',0,'preparing',f->>'consent_version',f->>'consent_hash','initschedprog01');
  perform public.creator_publishing_update_onlyfans_operator_progress((f->>'creator')::uuid,(f->>'task')::uuid,(f->>'job')::uuid,former_token,'preparing',1,'prepared',f->>'consent_version',f->>'consent_hash','initschedprog02');
  update public.creator_publishing_queue_tasks set assigned_operator_id=(f->>'operator_b')::uuid where id=(f->>'task')::uuid;
  select * into before_queue from public.creator_publishing_queue_tasks where id=(f->>'task')::uuid;
  select * into before_job from public.creator_publishing_platform_jobs where id=(f->>'job')::uuid;
  perform task17a_test.assert(before_job.job_state='draft' and before_job.schedule_revision is null and before_job.intended_publish_at is null and before_job.operator_due_at is null, 'initial schedule starts from unscheduled draft job');
  progress_audits_before := (select count(*) from public.creator_publishing_audit_events where action in ('operator_preparation_started','operator_package_prepared','operator_handoff_ready') and entity_id=(f->>'task')::uuid);
  progress_idempotency_before := (select count(*) from public.creator_publishing_operator_action_idempotency where action_type='progress_update' and queue_task_id=(f->>'task')::uuid);
  schedule_result := public.creator_publishing_schedule_plan((f->>'creator')::uuid,(f->>'plan')::uuid,new_intended,'UTC','initschedfuture01',f->>'consent_version',f->>'consent_hash',array[(f->>'job')::uuid],'{}'::jsonb,'schedule');
  cleanup := schedule_result->'jobs'->0->'operator_claim_cleanup';
  select * into after_queue from public.creator_publishing_queue_tasks where id=(f->>'task')::uuid;
  select * into after_job from public.creator_publishing_platform_jobs where id=(f->>'job')::uuid;
  perform task17a_test.assert((cleanup->>'performed')::boolean is true and cleanup->>'queue_task_id'=f->>'task' and cleanup->>'previous_status'='claimed' and cleanup->>'resulting_status'='scheduled_internally' and cleanup->>'reason'='scheduled_before_operator_due','initial schedule result reports operator claim cleanup');
  perform task17a_test.assert(schedule_result::text not like '%claim_token%', 'initial schedule result omits claim_token');
  perform task17a_test.assert(after_job.job_state='scheduled_internally' and after_job.schedule_revision=1 and after_job.intended_publish_at=new_intended and after_job.operator_due_at > clock_timestamp() and after_job.operator_due_at = after_job.intended_publish_at - interval '60 minutes', 'initial schedule updates schedule exactly once');
  perform task17a_test.assert(after_queue.status='scheduled_internally' and after_queue.claimed_by is null and after_queue.claimed_at is null and after_queue.claim_token is null and after_queue.claim_expires_at is null, 'initial schedule clears complete claim tuple');
  perform task17a_test.assert(after_queue.claim_attempt_count=before_queue.claim_attempt_count and after_queue.operator_progress_state=before_queue.operator_progress_state and after_queue.operator_progress_revision=before_queue.operator_progress_revision and after_queue.operator_progress_updated_by is not distinct from before_queue.operator_progress_updated_by and after_queue.operator_progress_updated_at is not distinct from before_queue.operator_progress_updated_at and after_queue.assigned_operator_id is not distinct from before_queue.assigned_operator_id, 'initial schedule preserves attempts progress and assignment');
  perform task17a_test.assert(after_queue.posted_by is not distinct from before_queue.posted_by and after_queue.posted_at is not distinct from before_queue.posted_at and after_queue.posted_confirmation is not distinct from before_queue.posted_confirmation and after_queue.final_post_url is not distinct from before_queue.final_post_url and after_queue.final_post_url_skip_reason is not distinct from before_queue.final_post_url_skip_reason and after_queue.proof_screenshot_storage_key is not distinct from before_queue.proof_screenshot_storage_key and after_queue.skip_or_fail_reason is not distinct from before_queue.skip_or_fail_reason, 'initial schedule preserves seven Task18 manual-result fields');
  select * into v_audit from public.creator_publishing_audit_events where action='operator_task_claim_cleared_by_schedule' and entity_id=(f->>'task')::uuid and idempotency_key='initschedfuture01';
  perform task17a_test.assert(found and (select count(*) from public.creator_publishing_audit_events where action='operator_task_claim_cleared_by_schedule' and entity_id=(f->>'task')::uuid and idempotency_key='initschedfuture01')=1, 'one initial schedule claim cleanup audit');
  perform task17a_test.assert(v_audit.before_state->>'status'='claimed' and (v_audit.before_state->>'claimed_by')::uuid=before_queue.claimed_by and (v_audit.before_state->>'claimed_at')::timestamptz=before_queue.claimed_at and (v_audit.before_state->>'claim_expires_at')::timestamptz=before_queue.claim_expires_at and (v_audit.before_state->>'claim_attempt_count')::integer=before_queue.claim_attempt_count and v_audit.before_state->>'operator_progress_state'=before_queue.operator_progress_state and (v_audit.before_state->>'operator_progress_revision')::integer=before_queue.operator_progress_revision and (v_audit.before_state->>'operator_progress_updated_by')::uuid=before_queue.operator_progress_updated_by and (v_audit.before_state->>'operator_progress_updated_at')::timestamptz=before_queue.operator_progress_updated_at and (v_audit.before_state->>'assigned_operator_id')::uuid=before_queue.assigned_operator_id, 'initial schedule cleanup audit before_state preserves safe prior claim progress assignment');
  perform task17a_test.assert(v_audit.after_state->>'resulting_status'='scheduled_internally' and (v_audit.after_state->>'schedule_revision')::integer=1 and (v_audit.after_state->>'operator_due_at')::timestamptz=after_job.operator_due_at and v_audit.after_state->>'reason'='scheduled_before_operator_due', 'initial schedule cleanup audit after_state records schedule result');
  perform task17a_test.assert(not (v_audit.before_state ? 'claim_token') and not (v_audit.after_state ? 'claim_token'), 'initial schedule cleanup audit omits claim_token');
  replay_result := public.creator_publishing_schedule_plan((f->>'creator')::uuid,(f->>'plan')::uuid,new_intended,'UTC','initschedfuture01',f->>'consent_version',f->>'consent_hash',array[(f->>'job')::uuid],'{}'::jsonb,'schedule');
  select * into replay_queue from public.creator_publishing_queue_tasks where id=(f->>'task')::uuid;
  select * into replay_job from public.creator_publishing_platform_jobs where id=(f->>'job')::uuid;
  perform task17a_test.assert((replay_result->>'idempotent')::boolean is true and (replay_result - 'idempotent')=(schedule_result - 'idempotent'), 'initial schedule replay returns stored cleanup result');
  perform task17a_test.assert(replay_job.schedule_revision=1 and replay_job.operator_due_at=after_job.operator_due_at and replay_queue.status='scheduled_internally' and replay_queue.claimed_by is null and (select count(*) from public.creator_publishing_audit_events where action='operator_task_claim_cleared_by_schedule' and entity_id=(f->>'task')::uuid and idempotency_key='initschedfuture01')=1, 'initial schedule replay no duplicate cleanup or audit');
  perform task17a_test.assert((select count(*) from public.creator_publishing_scheduler_idempotency where creator_id=(f->>'creator')::uuid and action_type='schedule' and idempotency_key='initschedfuture01')=1, 'one initial schedule idempotency row');
  perform task17a_test.expect_error('former token progress after initial schedule','OPERATOR_CLAIM_TOKEN_MISMATCH',format('select public.creator_publishing_update_onlyfans_operator_progress(%L,%L,%L,%L,%L,%s,%L,%L,%L,%L)',f->>'creator',f->>'task',f->>'job',former_token::text,'prepared',2,'handoff_ready',f->>'consent_version',f->>'consent_hash','initschedstale01'));
  perform task17a_test.assert((select count(*) from public.creator_publishing_audit_events where action in ('operator_preparation_started','operator_package_prepared','operator_handoff_ready') and entity_id=(f->>'task')::uuid)=progress_audits_before and (select count(*) from public.creator_publishing_operator_action_idempotency where action_type='progress_update' and queue_task_id=(f->>'task')::uuid)=progress_idempotency_before, 'former token after initial schedule writes no progress audit or idempotency');
  perform task17a_test.expect_error('before due claim after initial schedule','OPERATOR_NOT_DUE',format('select public.creator_publishing_claim_onlyfans_operator_task(%L,%L,%L,%L,%L,%L)',f->>'creator',f->>'task',f->>'job',f->>'consent_version',f->>'consent_hash','initschedclaimbefore01'));
  perform task17a_test.assert((select claimed_by is null and claimed_at is null and claim_token is null and claim_expires_at is null and claim_attempt_count=after_queue.claim_attempt_count from public.creator_publishing_queue_tasks where id=(f->>'task')::uuid), 'before due rejected claim after initial schedule leaves ownership empty and attempts unchanged');
  perform task17a_test.set_valid_schedule_phase((f->>'job')::uuid,'after_operator_due',after_job.schedule_revision);
  reclaim_result := public.creator_publishing_claim_onlyfans_operator_task((f->>'creator')::uuid,(f->>'task')::uuid,(f->>'job')::uuid,f->>'consent_version',f->>'consent_hash','initschedclaim02');
  select * into final_queue from public.creator_publishing_queue_tasks where id=(f->>'task')::uuid;
  perform task17a_test.assert((reclaim_result->>'ok')::boolean is true and final_queue.claim_token is not null and final_queue.claim_token <> former_token and final_queue.claimed_by=(f->>'creator')::uuid and final_queue.claimed_at is not null and final_queue.claim_expires_at is not null, 'after due reclaim after initial schedule succeeds with new ownership tuple');
  perform task17a_test.assert(final_queue.operator_progress_state=before_queue.operator_progress_state and final_queue.operator_progress_revision=before_queue.operator_progress_revision and final_queue.assigned_operator_id is not distinct from before_queue.assigned_operator_id, 'initial schedule reclaim preserves preparation progress and assignment');
end $$;
