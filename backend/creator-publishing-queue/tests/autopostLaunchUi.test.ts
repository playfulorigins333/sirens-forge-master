...
test("creator UI omits Fanvue internal-validation controls while preserving legacy safety", () => {
  ...
  assert.match(src, /function isFanvueScheduledPublishingActive\(platform: Platform\)/)
  assert.match(src, /platform\.launch_status === "available"/)
  assert.match(src, /platform\.public_selectable === true/)
  assert.match(src, /platform\.supports_real_posting === true/)
  assert.match(src, /"SCHEDULED PUBLISHING"/)
  assert.match(src, /"FINAL ACTIVATION PENDING"/)
  assert.match(src, /Direct scheduled Fanvue publishing is available for eligible connected accounts\./)
  assert.match(src, /Fanvue connection is ready; scheduled publishing is awaiting final launch activation\./)
