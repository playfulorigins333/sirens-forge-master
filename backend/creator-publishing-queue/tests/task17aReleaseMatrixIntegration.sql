...
update public.creator_publishing_platform_jobs set cancelled_at=clock_timestamp(), job_state='archived' where id=(:'release_cancelled_fixture'::jsonb->>'job')::uuid;
...
update public.creator_publishing_operator_authorizations set revoked_at=clock_timestamp(), updated_at=clock_timestamp() where creator_id=(:'release_revoked_fixture'::jsonb->>'creator')::uuid and operator_id=(:'release_revoked_fixture'::jsonb->>'operator_a')::uuid;
...
select public.creator_publishing_claim_onlyfans_operator_task((:'release_drift_intended_fixture'::jsonb->>'creator')::uuid,(:'release_drift_intended_fixture'::jsonb->>'task')::uuid,(:'release_drift_intended_fixture'::jsonb->>'job')::uuid,:'release_drift_intended_fixture'::jsonb->>'consent_version',:'release_drift_intended_fixture'::jsonb->>'consent_hash','reldriftclaim31') as release_drift_intended_claim \gset
select claim_token as release_drift_intended_token from public.creator_publishing_queue_tasks where id=(:'release_drift_intended_fixture'::jsonb->>'task')::uuid \gset
alter table public.creator_publishing_platform_jobs drop constraint if exists creator_publishing_jobs_scheduled_fields_required;
...
select task17a_test.assert_release_rejected('release_drift_missing_intended_publish_at','OPERATOR_TASK_INELIGIBLE',(:'release_drift_intended_fixture'::jsonb->>'creator')::uuid,(:'release_drift_intended_fixture'::jsonb->>'task')::uuid,(:'release_drift_intended_fixture'::jsonb->>'job')::uuid,:'release_drift_intended_token'::uuid,'reldrift31');
...
select task17a_test.assert_release_rejected('release_drift_missing_operator_due_at','OPERATOR_TASK_INELIGIBLE',(:'release_drift_operator_fixture'::jsonb->>'creator')::uuid,(:'release_drift_operator_fixture'::jsonb->>'task')::uuid,(:'release_drift_operator_fixture'::jsonb->>'job')::uuid,:'release_drift_operator_token'::uuid,'reldrift32');
...
select task17a_test.assert_release_rejected('release_drift_missing_timezone','OPERATOR_TASK_INELIGIBLE',(:'release_drift_timezone_fixture'::jsonb->>'creator')::uuid,(:'release_drift_timezone_fixture'::jsonb->>'task')::uuid,(:'release_drift_timezone_fixture'::jsonb->>'job')::uuid,:'release_drift_timezone_token'::uuid,'reldrift33');
...
select public.creator_publishing_claim_onlyfans_operator_task((:'release_drift_zero_revision_fixture'::jsonb->>'creator')::uuid,(:'release_drift_zero_revision_fixture'::jsonb->>'task')::uuid,(:'release_drift_zero_revision_fixture'::jsonb->>'job')::uuid,:'release_drift_zero_revision_fixture'::jsonb->>'consent_version',:'release_drift_zero_revision_fixture'::jsonb->>'consent_hash','reldriftclaim37') as release_drift_zero_revision_claim \gset
select claim_token as release_drift_zero_revision_token from public.creator_publishing_queue_tasks where id=(:'release_drift_zero_revision_fixture'::jsonb->>'task')::uuid \gset
alter table public.creator_publishing_platform_jobs drop constraint if exists creator_publishing_jobs_schedule_revision_positive;
...
select task17a_test.assert_release_rejected('release_drift_operator_offset_not_60_minutes','OPERATOR_TASK_INELIGIBLE',(:'release_drift_offset_fixture'::jsonb->>'creator')::uuid,(:'release_drift_offset_fixture'::jsonb->>'task')::uuid,(:'release_drift_offset_fixture'::jsonb->>'job')::uuid,:'release_drift_offset_token'::uuid,'reldrift39');
...
alter table public.creator_publishing_platform_jobs drop constraint if exists creator_publishing_jobs_unscheduled_fields_null;
...
select task17a_test.assert_release_rejected('release_drift_unscheduled_job_with_schedule_fields','OPERATOR_TASK_INELIGIBLE',(:'release_drift_unscheduled_fields_fixture'::jsonb->>'creator')::uuid,(:'release_drift_unscheduled_fields_fixture'::jsonb->>'task')::uuid,(:'release_drift_unscheduled_fields_fixture'::jsonb->>'job')::uuid,:'release_drift_unscheduled_fields_token'::uuid,'reldrift41');

select task17a_test.set_valid_schedule_phase((:'release_drift_intended_fixture'::jsonb->>'job')::uuid,'before_operator_due');
select task17a_test.set_valid_schedule_phase((:'release_drift_operator_fixture'::jsonb->>'job')::uuid,'before_operator_due');
select task17a_test.set_valid_schedule_phase((:'release_drift_timezone_fixture'::jsonb->>'job')::uuid,'before_operator_due');
select task17a_test.set_valid_schedule_phase((:'release_drift_blank_timezone_fixture'::jsonb->>'job')::uuid,'before_operator_due');
select task17a_test.set_valid_schedule_phase((:'release_drift_scheduled_at_fixture'::jsonb->>'job')::uuid,'before_operator_due');
select task17a_test.set_valid_schedule_phase((:'release_drift_scheduled_by_fixture'::jsonb->>'job')::uuid,'before_operator_due');
select task17a_test.set_valid_schedule_phase((:'release_drift_zero_revision_fixture'::jsonb->>'job')::uuid,'before_operator_due');
select task17a_test.set_valid_schedule_phase((:'release_drift_negative_revision_fixture'::jsonb->>'job')::uuid,'before_operator_due');
select task17a_test.set_valid_schedule_phase((:'release_drift_offset_fixture'::jsonb->>'job')::uuid,'before_operator_due');
update public.creator_publishing_platform_jobs set job_state='scheduled_internally' where id=(:'release_drift_job_state_fixture'::jsonb->>'job')::uuid;
select task17a_test.set_valid_schedule_phase((:'release_drift_job_state_fixture'::jsonb->>'job')::uuid,'before_operator_due');
update public.creator_publishing_platform_jobs set intended_publish_at=null, schedule_timezone=null, operator_due_at=null, scheduled_at=null, scheduled_by=null, rescheduled_at=null where id=(:'release_drift_unscheduled_fields_fixture'::jsonb->>'job')::uuid;
alter table public.creator_publishing_platform_jobs add constraint creator_publishing_jobs_schedule_revision_positive check (schedule_revision is null or schedule_revision > 0);
alter table public.creator_publishing_platform_jobs add constraint creator_publishing_jobs_unscheduled_fields_null check (schedule_revision is not null or (intended_publish_at is null and schedule_timezone is null and operator_due_at is null and scheduled_at is null and scheduled_by is null and rescheduled_at is null));
alter table public.creator_publishing_platform_jobs add constraint creator_publishing_jobs_scheduled_fields_required check (schedule_revision is null or (intended_publish_at is not null and schedule_timezone is not null and length(btrim(schedule_timezone)) > 0 and scheduled_at is not null and scheduled_by is not null));
