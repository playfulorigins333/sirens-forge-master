...
select claim_token as release_drift_unscheduled_fields_fixture_token from public.creator_publishing_queue_tasks where id=(:'release_drift_unscheduled_fields_fixture'::jsonb->>'task')::uuid \gset
select task17a_test.assert((select :'release_drift_unscheduled_fields_fixture_token'::uuid=claim_token and status='claimed' and claimed_by=(:'release_drift_unscheduled_fields_fixture'::jsonb->>'creator')::uuid and claim_expires_at is not null and claim_expires_at > clock_timestamp() from public.creator_publishing_queue_tasks where id=(:'release_drift_unscheduled_fields_fixture'::jsonb->>'task')::uuid), 'release_drift_unscheduled_job_with_schedule_fields token alias matches claimed queue');
