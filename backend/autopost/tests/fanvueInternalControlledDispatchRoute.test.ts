...
async function exercise(input: Record<string, any> = {}) {
  ...
  const response = await handleFanvueInternalControlledDispatchRoute({
    ...
    loadAccount: input.loadAccount ?? (async () => { loadAccountCalls++; return { user_id: userId, platform: 'fanvue', connection_status: 'CONNECTED', encrypted_access_token: 'encrypted-token-never-returned', encrypted_refresh_token: 'encrypted-refresh-token-never-returned', token_expires_at: new Date(now.getTime() + 3600000).toISOString(), scopes: input.scopes ?? ['write:post', 'read:media', 'write:media', 'write:creator'] } }),
