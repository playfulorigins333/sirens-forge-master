...

{
  ...
  assert.equal(typeof adapterCalls[0].lock_id, 'string')
  assert.deepEqual({ ...adapterCalls[0], lock_id: '<execution-lock>' }, { run_mode: 'autopost', user_id: 'user-success', rule_id: 'success', job_id: 'job-1', lock_id: '<execution-lock>', payload: { text: 'local text success' } })
