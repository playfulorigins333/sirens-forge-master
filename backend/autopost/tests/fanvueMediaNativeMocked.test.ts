...
async function run() {
  ...
  assert.match(adminRunner, /accessToken: string[\s\S]*accessToken = deps\.decryptToken\(String\(account\?\.encrypted_access_token\)\)[\s\S]*const config = \{ accessToken,/m, 'admin media upload path must use the decrypted stored autopost_accounts token')
