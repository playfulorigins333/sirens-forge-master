...
assert.match(runRoute, /schedulablePlatforms: AutopostProofPlatform\[\] = dispatchEnabled \? \["x"\] : getFoundationSchedulablePlatforms\(req\)/, 'Fanvue must not be a live dispatch or normally schedulable run platform')
assert.doesNotMatch(runRoute, /postFanvue|dispatchFanvue|createFanvue(?:Text|Media)?Post/i, 'run route must not perform live Fanvue posting or dispatch')
assert.doesNotMatch(runRoute, /persistAutopostJobResult\([^)]*fanvue|fanvue[^\n]*(advance|schedule|posted|success)/i, 'run route must not persist Fanvue posted results, advance Fanvue schedules, or fabricate success')
assert.doesNotMatch(`${fanvueRunDryRunVerifier}\n${fanvueRunDryRunBranch}`, /fetch\(|postFanvue|dispatchFanvue|createFanvue(?:Text|Media)?Post|persistAutopostJobResult|calculateNextRunAtAfterPostedProof/, 'Fanvue dry-run verification must not fetch externally, post, dispatch, persist results, or advance schedules')
