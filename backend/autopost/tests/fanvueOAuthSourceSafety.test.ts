...
assert.ok(defaultRequestedScopes.includes('write:creator'), 'Fanvue launch scopes must include write:creator')
...
assert.match(oauth.match(/export const FANVUE_DEFAULT_REQUESTED_SCOPES[\s\S]*?\] as const/)?.[0] ?? '', /write:creator/, 'normal launch OAuth must request write:creator')
...
assert.doesNotMatch(startRoute, /hasFanvueWriteCreatorScope\(configStatus\.scopes\)/, 'Generic Fanvue start must not block launch-required write:creator')
...
assert.doesNotMatch(callback, /operation === FANVUE_CONNECT_OPERATION && grantedWriteCreator/, 'ordinary launch callback must accept requested write:creator grant')
