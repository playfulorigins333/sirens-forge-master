...
const fanvueRunDryRunVerifier = readFileSync('lib/autopost/fanvueRunRouteDryRunVerifier.ts', 'utf8')
const fanvueRunDryRunBranch = readFileSync('lib/autopost/fanvueRunDryRunBranch.ts', 'utf8')
...
assert.match(rulesRoute, /posts_per_day: 1/, 'Fanvue draft persistence must satisfy the existing insert schedule contract')
assert.match(rulesRoute, /time_slots: \["00:00"\]/, 'Fanvue draft persistence must include an insert-safe placeholder time slot')
assert.match(rulesRoute, /next_run_at: null/, 'Fanvue draft persistence must not create runnable schedule slots')
assert.match(rulesRoute, /code: error\?\.code \?\? null/, 'rules route should return Supabase insert code for diagnosis')
assert.match(rulesRoute, /hint: error\?\.hint \?\? null/, 'rules route should return Supabase insert hint for diagnosis')
assert.match(approveRoute, /filterSelectableAutopostPlatformIds/, 'rule approval must still require selectable platforms')
assert.match(runRoute, /schedulablePlatforms: AutopostProofPlatform\[\] = dispatchEnabled \? \["x"\] : getFoundationSchedulablePlatforms\(req\)/, 'normal run execution must keep schedulable platforms restricted to X')
assert.doesNotMatch(runRoute, /postFanvue|dispatchFanvue|createFanvue(?:Text|Media)?Post/i, 'run route must not expose Fanvue live posting or dispatch')
assert.doesNotMatch(runRoute, /persistAutopostJobResult\([^)]*fanvue|fanvue[^\n]*(advance|schedule|posted|success)/i, 'run route must not persist Fanvue results, advance Fanvue schedules, or fabricate posted success')
assert.doesNotMatch(`${fanvueRunDryRunVerifier}\n${fanvueRunDryRunBranch}`, /fetch\(|postFanvue|dispatchFanvue|createFanvue(?:Text|Media)?Post|persistAutopostJobResult|calculateNextRunAtAfterPostedProof/, 'Fanvue dry-run verification must remain non-posting, non-dispatching, and unable to advance schedules or persist posted results')
...
assert.doesNotMatch(autopostPageClient, /saveFanvueInternalValidationDraftRule|Save Fanvue Validation Draft|Fanvue validation draft saved\./, 'creator-facing Fanvue validation creation and save controls must remain removed')
assert.match(autopostPageClient, /const details = j\?\.details \?/, 'rule creation errors should surface Supabase insert details for diagnosis')
assert.match(autopostPageClient, /function isFanvueInternalValidationDraft/, 'My Rules should identify Fanvue-only validation draft rules')
assert.match(autopostPageClient, /canApprove: state === "DRAFT" && !fanvueInternalValidationDraft/, 'Fanvue-only validation drafts must not expose an active Approve action')
assert.match(autopostPageClient, /Internal validation only/, 'Fanvue validation draft rules should show internal validation copy')
assert.match(autopostPageClient, /Approval disabled/, 'Fanvue validation draft rules should show approval disabled copy')
assert.match(autopostPageClient, /Non-runnable draft/, 'Fanvue validation draft rules should show non-runnable draft copy')
assert.match(autopostPageClient, /Native posting disabled · Scheduling disabled · Media upload disabled/, 'Fanvue validation draft rules should show disabled native posting, scheduling, and media upload copy')
assert.match(autopostPageClient, /canApprove: state === "DRAFT"/, 'X/manual draft approval UI should remain available for non-Fanvue drafts')
assert.match(autopostPageClient, /NO_AVAILABLE_PLATFORMS/, 'Approval errors for non-selectable platform actions should use safe selectable-platform copy')
...
assert.doesNotMatch(`${rulesRoute}\n${approveRoute}\n${runRoute}\n${contentHelper}\n${autopostPageClient}`, /FANVUE_RUN_DISPATCH_ENABLED|FANVUE_POST_VERIFY_ENABLED|write:creator|media upload endpoint|fanvue_media_uuid|fanvue_post_uuid/i, 'Fanvue source safety must not add env flags, write scope, media upload, or posting proof')
