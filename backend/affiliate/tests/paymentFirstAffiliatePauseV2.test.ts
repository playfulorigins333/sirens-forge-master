...
equal(summary.pending, 12, "historical pending total remains visible"); equal(summary.clicks, 3, "usage remains visible"); equal(summary.payouts.length, 1, "payout history remains visible"); equal(summary.stripe_connect_onboarded, true, "Connect boolean remains visible");
...
response = await createStripeConnectResponse(new Request("https://local.test", { method: "POST" }), {
  cutoverEnabled: () => true, getAuthenticatedUserId: async () => { effects.auth++; return "u"; }, getAdminClient: () => { effects.admin++; return {}; },
  getConfiguration: () => { effects.config++; return {}; }, createStripeClient: () => { effects.stripe++; return {} as any; },
});
equal(response.status, 503, "paused Connect is 503"); equal(await response.json(), { error: "Affiliate program is paused", code: "AFFILIATE_PROGRAM_PAUSED" }, "paused Connect contract is exact"); equal(effects, { auth: 0, admin: 0, config: 0, stripe: 0 }, "paused Connect precedes all effects");
...
check(dashboard.includes('fetch("/api/affiliate/summary"'), "dashboard uses authenticated summary"); check(dashboard.includes("Affiliate program paused"), "dashboard contains pause notice"); check(dashboard.includes("affiliateActive && referralCode"), "referral sharing requires confirmed legacy mode");
const pricing = files("app/pricing/PricingClient.tsx"); check(pricing.includes('removeItem("sf_referral_code")'), "Payment V2 clears stored referral"); check(pricing.includes("if (!publicPurchase) return"), "unresolved mode captures no referral"); check(pricing.includes('? { tierName }'), "V2 request remains tier-only"); check(pricing.includes("Referral codes are not accepted or tracked"), "Pricing pause copy is truthful");
const home = files("app/page.tsx"); check(home.includes('checkoutMode === "payment_v2"') && home.includes("Affiliate referrals are currently paused"), "homepage conditionally shows pause copy");
const terms = files("app/affiliate-terms/page.tsx"); check(terms.includes('lastUpdated="August 6, 2026"') && terms.includes("Program Pause During Payment-First Launch"), "terms date and pause section are present");
...
console.log(`PFC-CORE-02B affiliate pause tests passed (${assertions} assertions; local fakes only, zero external calls).`);
